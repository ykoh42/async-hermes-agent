...
async def _iter_python_files(root: Path):
    try:
        names = sorted(await aiofiles.os.listdir(root))
    except OSError:
        return
    for name in names:
        ...
        if await aiofiles.os.path.isdir(candidate):
            if not await aiofiles.os.path.islink(candidate):
                async for descendant in _iter_python_files(candidate):
                    yield descendant
        elif (
            candidate.suffix.lower() == ".py"
            and await aiofiles.os.path.isfile(candidate)
        ):
            yield candidate
...
async def ast_scan_path(path: Path) -> List[Finding]:
    ...
    async for py in _iter_python_files(path):
        try:
            async with aiofiles.open(
                py, encoding="utf-8", errors="replace"
            ) as handle:
                content = await handle.read()
        except OSError:
            continue
