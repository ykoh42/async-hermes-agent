...
async def _validate_file_path(file_path: str, working_dir: str) -> Optional[str]:
    ...
    abs_workdir = await _normalize_path(working_dir)
    resolved = await aiofiles.os.wrap(Path.resolve)(abs_workdir / file_path)
    try:
...
async def _normalize_path(path_value: str) -> Path:
    """Return a canonical absolute path for checkpoint operations."""
    return await aiofiles.os.wrap(Path.resolve)(Path(path_value).expanduser())


async def _project_hash(working_dir: str) -> str:
    """Deterministic per-project hash: sha256(abs_path)[:16]."""
    abs_path = str(await _normalize_path(working_dir))
...
async def _git_env(
    ...
    * ``GIT_CONFIG_NOSYSTEM=1`` — legacy belt-and-suspenders for older git.
    ...
    """
    normalized_working_dir = await _normalize_path(working_dir)
...
async def _repair_bare_repo_dirs(store: Path) -> None:
...
async def _run_git(
    ...
    """
    normalized_working_dir = await _normalize_path(working_dir)
    if not await aiofiles.os.path.exists(normalized_working_dir):
        msg = f"working directory not found: {normalized_working_dir}"
        logger.error("Git command skipped: %s (%s)", " ".join(["git"] + list(args)), msg)
        return False, "", msg
    if not await aiofiles.os.path.isdir(normalized_working_dir):
        msg = f"working directory is not a directory: {normalized_working_dir}"
        logger.error("Git command skipped: %s (%s)", " ".join(["git"] + list(args)), msg)
        return False, "", msg

    env = await _git_env(store, str(normalized_working_dir), index_file=index_file)
    cmd = ["git"] + list(args)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.debug("Git cleanup after cancellation failed", exc_info=True)
            raise
        except TimeoutError:
            if process.returncode is None:
                try:
        ...
        if not ok and process.returncode not in allowed_returncodes:
            logger.error(
...
async def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    for child_name in await aiofiles.os.listdir(base):
        ...
        if legacy_root is None:
            ...
            try:
                await aiofiles.os.makedirs(legacy_root, exist_ok=True)
            except OSError as exc:
                logger.warning("Could not create legacy archive dir: %s", exc)
                return None
        dest = legacy_root / name
        try:
            await aiofiles.os.wrap(shutil.move)(str(child), str(dest))
        except OSError as exc:
...
async def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    if not await aiofiles.os.path.exists(store):
        try:
        ...
        # our own v2 layout.
        await _migrate_legacy_store(base)

    if await aiofiles.os.path.exists(store / "HEAD"):
        return None

    await aiofiles.os.makedirs(store, exist_ok=True)
    await aiofiles.os.makedirs(store / _INDEXES_DIRNAME, exist_ok=True)
    await aiofiles.os.makedirs(store / _PROJECTS_DIRNAME, exist_ok=True)
    ...
    cfg_wd = str(base)
    await _run_git(["config", "user.email", "hermes@local"], store, cfg_wd)
    await _run_git(["config", "user.name", "Hermes Checkpoint"], store, cfg_wd)
    await _run_git(["config", "commit.gpgsign", "false"], store, cfg_wd)
    await _run_git(["config", "tag.gpgSign", "false"], store, cfg_wd)
    await _run_git(["config", "gc.auto", "0"], store, cfg_wd)

    info_dir = store / "info"
    await aiofiles.os.makedirs(info_dir, exist_ok=True)
    async with aiofiles.open(info_dir / "exclude", "w", encoding="utf-8") as handle:
...
async def _volume_evidence(workdir: Path) -> Dict:
    ...
    """
    try:
        if not await aiofiles.os.path.exists(workdir):
            return {}
        st = await aiofiles.os.stat(workdir.parent)
...
async def _register_project(store: Path, working_dir: str) -> None:
    """Create or update ``projects/<hash>.json`` with workdir + timestamps."""
    dir_hash = await _project_hash(working_dir)
    meta_path = _project_meta_path(store, dir_hash)
    now = time.time()
    normalized_working_dir = await _normalize_path(working_dir)
    meta: Dict = {"workdir": str(normalized_working_dir),
                  "created_at": now, "last_touch": now}
    evidence = await _volume_evidence(normalized_working_dir)
    if evidence:
        meta.update(evidence)
    if await aiofiles.os.path.exists(meta_path):
        try:
            async with aiofiles.open(meta_path, encoding="utf-8") as handle:
...
async def _touch_project(store: Path, working_dir: str) -> None:
    ...
    if not isinstance(meta, dict):
        meta = {}
    normalized_working_dir = await _normalize_path(working_dir)
    meta["workdir"] = str(normalized_working_dir)
...
async def _list_projects(store: Path) -> List[Dict]:
    """Return all registered projects under the store."""
    projects_dir = store / _PROJECTS_DIRNAME
    if not await aiofiles.os.path.exists(projects_dir):
        return []
    out: List[Dict] = []
    for name in await aiofiles.os.listdir(projects_dir):
        if not name.endswith(".json"):
            continue
        meta_path = projects_dir / name
...
async def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child_name in await aiofiles.os.listdir(base):
        ...
        if child.name == _STORE_DIRNAME or child.name.startswith(_LEGACY_PREFIX):
            continue
        if not await aiofiles.os.path.exists(child / "HEAD"):
            continue
        workdir: Optional[str] = None
        marker_unreadable = False
        wd_marker = child / "HERMES_WORKDIR"
        if await aiofiles.os.path.exists(wd_marker):
            try:
                async with aiofiles.open(wd_marker, encoding="utf-8") as handle:
                    workdir = (await handle.read()).strip()
        ...
        out.append({
            "path": child,
            "workdir": workdir,
            "exists": bool(workdir) and await aiofiles.os.path.exists(workdir),
            "marker_unreadable": marker_unreadable,
        })
    return out


async def _dir_file_count(path: str) -> int:
        ...
        while pending:
            ...
            for name in await aiofiles.os.listdir(directory):
                ...
                if count > _MAX_FILES:
                    return count
                if (
                    await aiofiles.os.path.isdir(entry)
                    and not await aiofiles.os.path.islink(entry)
                ):
            ...
            for name in await aiofiles.os.listdir(directory):
                ...
                try:
                    if await aiofiles.os.path.isdir(entry):
                        if not await aiofiles.os.path.islink(entry):
                            pending.append(entry)
                    elif await aiofiles.os.path.isfile(entry):
                        total += (await aiofiles.os.stat(entry)).st_size
                except OSError:
                    continue
    except OSError:
        pass
...
async def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    on success.
    """
    err = await _init_store(shadow_repo, working_dir)
    if err:
        return err
    await _register_project(shadow_repo, working_dir)
    # Compat marker for tests that look at HERMES_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        async with aiofiles.open(
            shadow_repo / "HERMES_WORKDIR", "w", encoding="utf-8"
        ) as handle:
            await handle.write(str(await _normalize_path(working_dir)) + "\n")
    except OSError:
        pass
    return None
...
class CheckpointManager:
    ...
    async def ensure_checkpoint(self, working_dir: str, reason: str = "auto") -> bool:
        ...
        if self._git_available is None:
            self._git_available = (
                await aiofiles.os.wrap(shutil.which)("git") is not None
            )
        ...
        async with self._checkpoint_lock:
            ...
            try:
                return await self._take(abs_dir, reason)
    ...
    async def list_checkpoints(self, working_dir: str) -> List[Dict]:
        ...
        ref = _ref_name(await _project_hash(abs_dir))
        ok, stdout, _ = await _run_git(
        ...
        for line in stdout.splitlines():
            ...
            if len(parts) == 4:
                ...
                stat_ok, stat_out, _ = await _run_git(
    ...
    async def diff(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        await _run_git(["read-tree", ref], store, abs_dir,
    ...
    async def session_diff(self, working_dir: str) -> Dict:
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        """
        checkpoints = await self.list_checkpoints(working_dir)
        if not checkpoints:
            return {"success": True, "stat": "", "diff": "", "empty": True}

        baseline = checkpoints[-1].get("hash") or ""
        result = await self.diff(working_dir, baseline)
    ...
    async def restore(
        ...
        if file_path:
            path_err = await _validate_file_path(file_path, abs_dir)
            if path_err:
        ...
        ok2, reason_out, _ = await _run_git(
    ...
    async def get_working_dir_for_path(self, file_path: str) -> str:
        """Resolve a file path to its working directory for checkpointing."""
        path = await _normalize_path(file_path)
        ...
        while check != check.parent:
            for marker in markers:
                if await aiofiles.os.path.exists(check / marker):
                    return str(check)
            check = check.parent
    ...
    async def _take(self, working_dir: str, reason: str) -> bool:
        ...
        await _touch_project(store, working_dir)

        # Quick size guard — don't try to snapshot enormous directories
        if await _dir_file_count(working_dir) > _MAX_FILES:
        ...
        if await aiofiles.os.path.exists(index_file):
            ...
            ok_ref, ref_commit, _ = await _run_git(
            ...
            if ok_ref and ref_commit:
                ...
                try:
                    await aiofiles.os.unlink(index_file)
                except OSError:
                    pass
        else:
            # First snapshot for this project.
            await aiofiles.os.makedirs(index_file.parent, exist_ok=True)
        ...
        if self.max_file_size_mb > 0:
            await self._drop_oversize_from_index(store, working_dir, index_file)
        ...
        ok_tree, tree_sha, err = await _run_git(
        ...
        ok_commit, new_sha, err = await _run_git(
        ...
        ok_update, _, err = await _run_git(
        ...
        await self._prune(store, working_dir, ref)

        # Enforce global size cap.
        await self._enforce_size_cap(store)

        return True

    async def _drop_oversize_from_index(
        ...
        for rel in paths:
            try:
                size = (await aiofiles.os.stat(abs_workdir / rel)).st_size
            except OSError:
                continue
            if size > cap:
                oversize.append(rel)
    ...
    async def _prune(self, store: Path, working_dir: str, ref: str) -> None:
        ...
        ok_list, list_out, _ = await _run_git(
        ...
        for sha in keep:
            ok_tree, tree_sha, _ = await _run_git(
                ["rev-parse", f"{sha}^{{tree}}"], store, working_dir,
            )
            if not ok_tree or not tree_sha:
                return
            ok_msg, msg, _ = await _run_git(
            ...
            ok_commit, new_sha, _ = await _run_git(args, store, working_dir)
            if not ok_commit or not new_sha:
                return
            new_parent = new_sha
        ...
        if new_parent is None:
            return
        await _run_git(["update-ref", ref, new_parent], store, working_dir)
        ...
        await _repair_bare_repo_dirs(store)

    async def _enforce_size_cap(self, store: Path) -> None:
        ...
        size = await _dir_size_bytes(store)
        if size <= cap_bytes:
            return
        logger.info(
        ...
        for _ in range(20):  # hard upper bound to avoid pathological loops
            ...
            if size <= cap_bytes:
                break
            for ref in refs:
                ok_count, count_out, _ = await _run_git(
                ...
                for sha in keep:
                    ...
                    if new_parent is not None:
                        args = ["commit-tree", tree_sha, "-p", new_parent,
                                "-m", commit_msg, "--no-gpg-sign"]
                    ok_commit, new_sha, _ = await _run_git(
...
async def _delete_ref(store: Path, ref: str) -> bool:
    """Delete a ref from the store.  Returns True on success."""
    ok, _, _ = await _run_git(
...
async def _workdir_is_observably_gone(
    workdir: str,
    ...
    try:
        ...
        if parent_dev is not None and parent_ino is not None:
            st = await aiofiles.os.stat(parent)
            ...
            return False
        if await _dir_has_any_entry(parent):
            return True
        ...
        return await aiofiles.os.path.ismount(parent)
    except OSError:
        # Probe failed (permission, I/O error) — not evidence of deletion.
        return False


async def _dir_has_any_entry(directory: Path) -> bool:
    """True when ``directory`` contains at least one entry."""
    return bool(await aiofiles.os.listdir(directory))
...
async def prune_checkpoints(
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    size_before = await _dir_size_bytes(base)
    ...
    for child_name in await aiofiles.os.listdir(base):
        ...
        if child.name.startswith(_LEGACY_PREFIX):
            ...
            if retention_days <= 0:
                continue
            try:
                m = (await aiofiles.os.stat(child)).st_mtime
            except OSError:
                continue
            if m >= cutoff:
                continue
            try:
                size = await _dir_size_bytes(child)
                await aiofiles.os.wrap(shutil.rmtree)(child)
    ...
    for repo in await _pre_v2_shadow_repos(base):
        child = repo["path"]
        result["scanned"] += 1
        ...
        if (
            ...
            and (
                ...
                or await _workdir_is_observably_gone(
            ...
            try:
                ...
                while pending:
                    ...
                    for name in await aiofiles.os.listdir(directory):
                        entry = directory / name
                        try:
                            stat_result = await aiofiles.os.stat(entry)
    ...
    store = _store_path(base)
    if await aiofiles.os.path.exists(store / "HEAD"):
        for meta in await _list_projects(store):
            ...
            if reason is None:
                continue
            ref = _ref_name(dir_hash)
            await _delete_ref(store, ref)
            # Drop per-project index and metadata.
            try:
                idx = _index_path(store, dir_hash)
                if await aiofiles.os.path.exists(idx):
                    await aiofiles.os.unlink(idx)
            except OSError:
                pass
            try:
                mp = _project_meta_path(store, dir_hash)
                if await aiofiles.os.path.exists(mp):
                    await aiofiles.os.unlink(mp)
            except OSError:
                pass
            if reason == "orphan":
                result["deleted_orphan"] += 1
        ...
        if max_total_size_mb > 0:
            ...
            for _i in range(20):
                ...
                any_drop = False
                for ref in refs:
                    ok_c, count_out, _ = await _run_git(
                    ...
                    for sha in keep:
                        ...
                        if new_parent is not None:
                            args = ["commit-tree", tsha, "-p", new_parent,
                                    "-m", msg, "--no-gpg-sign"]
                        ok_cm, new_sha, _ = await _run_git(
                            args, store, str(base)
                        )
                    ...
                    if fail or new_parent is None:
                        continue
                    await _run_git(
                        ["update-ref", ref, new_parent], store, str(base)
                    )
                    any_drop = True
                if not any_drop:
                    break
    ...
    size_after = await _dir_size_bytes(base)
...
async def maybe_auto_prune_checkpoints(
    retention_days: int = 7,
    ...
    try:
        ...
        if await aiofiles.os.path.exists(marker):
            try:
                async with aiofiles.open(marker, encoding="utf-8") as handle:
                    last_ts = float((await handle.read()).strip())
        ...
        try:
            async with aiofiles.open(marker, "w", encoding="utf-8") as handle:
                await handle.write(str(now))
        except OSError as exc:
...
async def store_status(checkpoint_base: Optional[Path] = None) -> Dict:
    ...
    store = _store_path(base)
    if await aiofiles.os.path.exists(store):
        out["store_size_bytes"] = await _dir_size_bytes(store)
        if await aiofiles.os.path.exists(store / "HEAD"):
            for meta in await _list_projects(store):
                dir_hash = meta.get("_hash") or ""
                workdir = meta.get("workdir") or ""
                ref = _ref_name(dir_hash)
                ok, count_out, _ = await _run_git(
                    ["rev-list", "--count", ref], store, str(base),
                    allowed_returncodes={128},
                )
                try:
                ...
                out["projects"].append({
                    ...
                    "workdir": workdir,
                    "exists": bool(workdir)
                    and await aiofiles.os.path.exists(workdir),
    ...
    out["pre_v2_projects"] = [
        ...
        for r in await _pre_v2_shadow_repos(base)
    ]

    for child_name in await aiofiles.os.listdir(base):
        child = base / child_name
        if (
            await aiofiles.os.path.isdir(child)
            and child.name.startswith(_LEGACY_PREFIX)
        ):
            try:
                size = await _dir_size_bytes(child)
            except OSError:
            ...
            try:
                mt = (await aiofiles.os.stat(child)).st_mtime
            except OSError:
                mt = 0
    ...
    out["total_size_bytes"] = await _dir_size_bytes(base)
    return out


async def clear_all(checkpoint_base: Optional[Path] = None) -> Dict[str, int]:
    ...
    if not await aiofiles.os.path.exists(base):
        return out
    size = await _dir_size_bytes(base)
    try:
        await aiofiles.os.wrap(shutil.rmtree)(base)
...
async def clear_legacy(checkpoint_base: Optional[Path] = None) -> Dict[str, int]:
    ...
    for child_name in await aiofiles.os.listdir(base):
        child = base / child_name
        if (
            not await aiofiles.os.path.isdir(child)
            or not child.name.startswith(_LEGACY_PREFIX)
        ):
            continue
        try:
            size = await _dir_size_bytes(child)
