...
async def _validate_operations(
    ...
    async def _read(path: str):
        ...
        r = await file_ops.read_file_raw(path)
        if r.error:
            return None, r.error
    ...
    for op in operations:
        if op.operation != OperationType.UPDATE:
            real_change_count += 1
        if op.operation == OperationType.UPDATE:
            content, read_err = await _read(op.file_path)
            if read_err:
            ...
            _content, read_err = await _read(op.file_path)
            if read_err:
            ...
            if not op.new_path:
                ...
                continue
            src_content, src_err = await _read(op.file_path)
            if src_err:
                errors.append(f"{op.file_path}: source file not found for move")
            dst_content, dst_err = await _read(op.new_path)
            if not dst_err:
                errors.append(
...
async def apply_v4a_operations(operations: List[PatchOperation],
    ...
    # ---- Phase 1: validate ----
    validation_errors = await _validate_operations(operations, file_ops)
    ...
    for op in operations:
        try:
            if op.operation == OperationType.ADD:
                result = await _apply_add(op, file_ops)
                if result[0]:
                ...
                result = await _apply_delete(op, file_ops)
                if result[0]:
                ...
                result = await _apply_move(op, file_ops)
                if result[0]:
                ...
                result = await _apply_update(op, file_ops)
                if result[0]:
    ...
    for f in files_modified + files_created:
        if hasattr(file_ops, '_check_lint'):
            lint_result = await file_ops._check_lint(f)
...
async def _apply_add(op: PatchOperation, file_ops: Any) -> Tuple[bool, str, Optional[str]]:
...
async def _apply_delete(op: PatchOperation, file_ops: Any) -> Tuple[bool, str]:
    ...
    read_result = await file_ops.read_file_raw(op.file_path)
    if read_result.error:
        return False, f"Cannot delete {op.file_path}: file not found"

    result = await file_ops.delete_file(op.file_path)
    if result.error:
...
async def _apply_move(op: PatchOperation, file_ops: Any) -> Tuple[bool, str]:
    """Apply a move file operation."""
    result = await file_ops.move_file(op.file_path, op.new_path)
    if result.error:
        return False, result.error
...
async def _apply_update(op: PatchOperation, file_ops: Any) -> Tuple[bool, str, Optional[str]]:
