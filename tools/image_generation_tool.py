...
# ---------------------------------------------------------------------------
async def _resolve_fal_model() -> tuple:
...
async def _upscale_image(
    ...
    try:
        ...
        result = await _submit_fal_request(
            UPSCALER_MODEL,
...
async def image_generate_tool(
    ...
    try:
        ...
        result = await _submit_fal_request(endpoint, arguments=arguments)
        ...
        for img in images:
            ...
            if should_upscale:
                upscaled_image = await _upscale_image(
                    img["url"],
                    prompt.strip(),
                )
                if upscaled_image:
...
async def check_image_generation_requirements() -> bool:
    """True if FAL or the explicitly configured image backend is available."""
    try:
        if await check_fal_api_key():
    ...
    configured = await _read_configured_image_provider()
    ...
    try:
        ...
        return bool(provider and await provider.is_available())
    except Exception:
        return False
...
async def _read_configured_image_model():
...
async def _read_configured_image_provider():
...
async def _dispatch_to_plugin_provider(
    ...
    configured_model = await _read_configured_image_model()
    ...
    if configured_provider and configured_provider != "fal":
        try:
            ...
            if provider is not None:
                caps = {}
                try:
                    caps = await provider.capabilities() or {}
                except Exception:  # noqa: BLE001
                    caps = {}
                info["provider"] = provider.display_name
                info["model"] = (
                    await _read_configured_image_model()
...
async def _build_dynamic_image_schema() -> Dict[str, Any]:
    ...
    try:
        info = await _active_image_capabilities()
    except Exception:  # noqa: BLE001
