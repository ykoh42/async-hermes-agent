...
# Resolve the checkout root once, while module import is still outside any
# request event loop.  The terminal guard is invoked from an awaited command
# path; repeating ``Path.resolve()`` there would perform synchronous stat I/O
# on the loop thread (and is rejected by the native-async runtime checks).
try:
    _RUNNING_SOURCE_ROOT_FILE = __file__
    _RUNNING_SOURCE_ROOT = Path(__file__).resolve().parent.parent
    if not (_RUNNING_SOURCE_ROOT / ".git").exists():
        _RUNNING_SOURCE_ROOT = None
...
def get_running_source_root() -> Path | None:
    ...
    # A few synchronous guard tests intentionally monkeypatch ``__file__`` to
    # exercise worktree layouts.  Recompute only for that explicit test seam;
    # normal awaited command paths keep the import-time, non-blocking result.
    if __file__ != _RUNNING_SOURCE_ROOT_FILE:
        try:
            root = Path(__file__).resolve().parent.parent
        except (OSError, RuntimeError, ValueError):
            return None
        return root if (root / ".git").exists() else None
    return _RUNNING_SOURCE_ROOT
...
def _nested_shell_scripts(command: str) -> list[str]:
    """Extract command substitutions/backticks while respecting quotes."""
    scripts: list[str] = []
    quote: str | None = None
    index = 0
    while index < len(command):
        ...
        if command.startswith("$(", index):
            ...
            if depth == 0:
                scripts.append(command[index + 2 : end - 1])
                index = end
                continue
        index += 1
    return scripts
...
def _find_mutation(command: str, cwd: Path, root: Path, depth: int = 0) -> str | None:
    ...
    for script in heredoc_scripts:
        ...
        if operation:
            return operation
    for script in _nested_shell_scripts(masked_command):
        operation = _find_mutation(script, cwd, root, depth + 1)
        if operation:
            return operation
...
def detect_self_repo_git_mutation(
    ...
    using_cached_root = source_root is None
    root = source_root if source_root is not None else get_running_source_root()
    if root is None or not command:
        return False, None

    if using_cached_root:
        # ``terminal_tool`` supplies an absolute environment cwd.  Keep this
        # hot path lexical so the guard remains non-blocking under
        # BlockBuster; explicit source roots retain the fully canonical test
        # and library path semantics below.
