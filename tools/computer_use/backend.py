...
must return the shape described below. I/O methods are native coroutines.
"""
...
class ComputerUseBackend(ABC):
    """Lifecycle: await `start()` before first use, await `stop()` at shutdown."""
    ...
    async def wait(self, seconds: float) -> ActionResult:
        """Default implementation: non-blocking event-loop sleep."""
        import asyncio
