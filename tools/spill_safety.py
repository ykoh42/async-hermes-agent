...
async def _write_text_exclusive_async(
    path: Path,
    text: str,
    *,
    private: bool = True,
    ...
    """Async spill writer with the same no-follow/exclusive contract.

    ``aiofiles`` still delegates regular-file operations to its configured
    executor (Python has no native regular-file async primitive), but the
    security-sensitive open itself remains ``O_EXCL|O_NOFOLLOW``.  This keeps
    retained async callers from reintroducing an unsafe ``open(..., 'w')``
    rewrite while preserving the existing public synchronous helper.
    """
    path = Path(path)
    if overwrite:
        try:
            stat_result = await aiofiles.os.stat(path, follow_symlinks=False)
        except FileNotFoundError:
            pass
        else:
    ...
    def _opener(raw_path: str, flags: int) -> int:
        return os.open(raw_path, flags | _O_NOFOLLOW, mode)
