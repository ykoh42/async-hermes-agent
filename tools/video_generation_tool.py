...
async def check_video_generation_requirements() -> bool:
...
async def _handle_video_generate(args: Dict[str, Any], **_kw: Any) -> str:
    ...
    configured = await _read_configured_video_provider()
    ...
    model = (
        model_override
        or await _read_configured_video_model()
    ...
    try:
        result = await provider.generate(prompt=prompt, **kwargs)
...
async def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    active_model = configured_model or await provider.default_model()
    model_meta = next(
    ...
    if provider.name == "xai":
        ...
        try:
            ...
            notice = await xai_storage_notice_text("video_gen")
        except Exception:
            notice = ""
        if notice:
