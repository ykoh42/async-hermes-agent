...
async def _is_write_denied(path: str) -> bool:
    """Return True if path is on the write deny list."""
    return await _shared_is_write_denied(path)
...
class FileOperations(ABC):
    ...
    @abstractmethod
    async def patch_replace(self, path: str, old_string: str, new_string: str,
    ...
    async def delete_path(self, path: str, recursive: bool = False) -> WriteResult:
...
class ShellFileOperations(FileOperations):
    ...
    async def _exec(self, command: str, cwd: str | None = None, timeout: int | None = None,  # noqa: ASYNC109 - upstream API names timeout
        ...
        result = await self.env.execute(command, cwd=effective_cwd, **kwargs)
    ...
    async def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
    ...
    async def delete_file(self, path: str) -> WriteResult:
        ...
        """
        return await self._python_delete(path, recursive=False)
    ...
    async def delete_path(self, path: str, recursive: bool = False) -> WriteResult:
        ...
        """
        return await self._python_delete(path, recursive=recursive)

    async def _python_delete(self, path: str, recursive: bool) -> WriteResult:
        path = await self._expand_path(path)
        denied = await get_write_denied_error(path, verb="Delete")
        if denied:
    ...
    async def move_file(self, src: str, dst: str) -> WriteResult:
        """Move a file via mv."""
        src = await self._expand_path(src)
        dst = await self._expand_path(dst)
        for p in (src, dst):
            denied = await get_write_denied_error(p, verb="Move")
            if denied:
                return WriteResult(error=denied)
        result = await self._exec(
    ...
    async def write_file(
        ...
        denied = await get_write_denied_error(path)
        if denied:
            return WriteResult(error=denied)
        ...
        original_ending = await self._detect_file_line_ending(path, pre_content)
        if original_ending == "\r\n":
        ...
        if await self._file_has_bom(path, pre_content) and not _has_bom(content):
        ...
        write_result = await self._atomic_write(path, content)
        ...
        lint_result = await self._check_lint_delta(path, pre_content=pre_content, post_content=content)
    ...
    async def patch_replace(self, path: str, old_string: str, new_string: str,
        ...
        verify_result = await self._exec(verify_cmd)
        ...
        lint_result = await self._check_lint_delta(path, pre_content=content, post_content=new_content)
    ...
    async def _check_lint(self, path: str, content: str | None = None) -> LintResult:
        ...
        if not await self._has_command(base_cmd):
            return LintResult(skipped=True, message=f"{base_cmd} not available")

        # Run linter
        cmd = linter_cmd.replace("{file}", self._escape_shell_arg(path))
        result = await self._exec(cmd, timeout=30)
    ...
    async def _check_lint_delta(self, path: str, pre_content: str | None,
        ...
        Returns:
            ...
            refinement applied).
        """
        post = await self._check_lint(path, content=post_content)
        ...
        pre = await self._check_lint(path, content=pre_content)
    ...
    async def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        check = await self._exec(f"test -e {self._escape_shell_arg(path)} && echo exists || echo not_found")
        if "not_found" in check.stdout:
            ...
            multi = await self._try_multi_path_search(
        ...
        if target == "files":
            ...
            return await self._search_content(pattern, path, file_glob, limit, offset,
                                        output_mode, context)
    ...
    async def _try_multi_path_search(self, pattern: str, path: str, target: str,
        ...
        existing, missing = [], []
        for p in parts:
            expanded = await self._expand_path(p)
            chk = await self._exec(
        ...
        for p in existing:
            if target == "files":
                sub = await self._search_files(pattern, p, limit, offset)
            else:
                sub = await self._search_content(pattern, p, file_glob, limit, offset,
                                           output_mode, context)
            if sub.error:
                continue
    ...
    async def _search_files_rg(self, pattern: str, path: str, limit: int, offset: int) -> SearchResult:
        ...
        result = await self._exec(cmd_sorted, timeout=60)
        stdout, limit_reason = _search_stdout_and_limit(result)
        ...
        if not all_files and not limit_reason:
            ...
            result = await self._exec(cmd_plain, timeout=60)
            stdout, limit_reason = _search_stdout_and_limit(result)
            all_files = [f for f in stdout.strip().split('\n') if f]
    ...
    async def _search_content(self, pattern: str, path: str, file_glob: str | None,
        ...
        if await self._has_command('rg'):
            used_rg = True
            result = await self._search_with_rg(pattern, path, file_glob, limit, offset,
                                          output_mode, context)
        elif await self._has_command('grep'):
            result = await self._search_with_grep(pattern, path, file_glob, limit, offset,
                                            output_mode, context)
        else:
        ...
        if (not result.error and result.total_count == 0
            ...
            try:
                hint = await self._zero_match_probe(pattern, path, file_glob)
            except Exception:
                hint = None
            if hint:
