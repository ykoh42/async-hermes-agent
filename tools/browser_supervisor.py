...
snapshot object that tool handlers consume on the same event loop.
...
class CDPSupervisor:
    ...
    Lifecycle:
      * ``start()`` — kicked off by ``SupervisorRegistry.get_or_start``; starts
        a long-lived task, connects the WebSocket, attaches to the first page
        target, enables domains, and auto-attaches to child targets.
      * ``snapshot()`` — reads state between await points.
      * ``respond_to_dialog(action, ...)`` — awaits the CDP acknowledgement.
      * ``stop()`` — cancels and awaits the long-lived task.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Supervisor task machinery — populated in start().
    ...
    async def start(self, timeout: float = 15.0) -> None:
        """Launch the supervisor task and wait until attachment is complete.
        ...
        self._run_task = asyncio.create_task(
            self._run(), name=f"cdp-supervisor-{self.task_id}"
        )
        try:
    ...
    def _start_background_task(
        ...
        """Start and retain a task until it completes or the supervisor stops."""
    ...
    async def respond_to_dialog(
        ...
        *,
        ...
        """Accept or dismiss a pending dialog and await the CDP acknowledgement.
    ...
    async def evaluate_runtime(
        ...
        *,
        ...
        try:
            response = await _do_eval(return_by_value)
        except Exception as exc:
        ...
        if self.dialog_policy == DIALOG_POLICY_AUTO_DISMISS:
            ...
            handle = loop.call_later(
                ...
                lambda: self._start_background_task(
                    ...
                    name=f"cdp-dialog-timeout-{dialog.id}",
                ),
            )
    ...
    async def _on_fetch_paused(
        ...
        if self.dialog_policy == DIALOG_POLICY_AUTO_DISMISS:
            ...
            self._start_background_task(
                ...
                name=f"cdp-dialog-bridge-{dialog.id}",
            )
        elif self.dialog_policy == DIALOG_POLICY_AUTO_ACCEPT:
            self._archive_dialog_locked(dialog, "auto_policy")
            self._start_background_task(
    ...
    async def _on_target_attached(self, params: Dict[str, Any]) -> None:
        ...
        self._start_background_task(
            self._enable_child_domains(sid), name=f"cdp-child-{sid[:16]}"
        )
        ...
        async with self._lock:
            ...
            if existing is not None:
                if existing.cdp_url == cdp_url:
                    ...
                    task_ok = run_task is not None and not run_task.done()
                    if task_ok:
                        return existing
