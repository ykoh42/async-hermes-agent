...
async def _call_gateway(
    ...
    headers = await managed_gateway_auth_headers(url)
    if not headers:
...
async def _warm_nous_token() -> None:
...
async def _download_video(url: str, save_to, started: float) -> tuple:
    ...
    target = await _resolve_destination(save_to, _filename_from_url(url))
    ...
    try:
        client = await create_ssrf_safe_client(timeout=timeout, follow_redirects=True)
        async with client:
        ...
        size = (await aiofiles.os.stat(partial)).st_size
        if size < _MIN_PLAUSIBLE_VIDEO_BYTES:
            raise ValueError(f"the download returned only {size} bytes, which is not a video")
        await aiofiles.os.replace(partial, target)
    except BaseException:
        try:
            await aiofiles.os.remove(partial)
...
async def _default_directory():
    ...
    if _delivers_as_an_attachment():
        try:
            ...
            return await get_hermes_dir("cache/videos", "video_cache")
        except Exception:
            logger.debug("Could not resolve the video cache dir; using Downloads", exc_info=True)
    downloads = Path.home() / "Downloads"
    return downloads if await aiofiles.os.path.isdir(downloads) else Path.cwd()


async def _resolve_destination(save_to, filename: str):
    ...
    if isinstance(save_to, str) and save_to.strip():
        ...
        if await aiofiles.os.path.isdir(requested) or save_to.rstrip().endswith(("/", "\\")):
            directory, name = requested, filename
        else:
            directory, name = requested.parent, requested.name
    else:
        directory, name = await _default_directory(), filename

    await aiofiles.os.makedirs(directory, exist_ok=True)
    return await _free_path(directory / name)


async def _free_path(candidate):
    """`name.mp4` -> `name-2.mp4` -> `name-3.mp4` … so nothing is clobbered."""
    if not await aiofiles.os.path.exists(candidate):
        return candidate
    for suffix in range(2, _MAX_FILENAME_ATTEMPTS + 2):
        sibling = candidate.with_name(f"{candidate.stem}-{suffix}{candidate.suffix}")
        if not await aiofiles.os.path.exists(sibling):
            return sibling
...
async def _has_nous_credential() -> bool:
    ...
    """
    if await peek_nous_access_token():
        return True
    try:
...
async def check_bfl_requirements() -> bool:
    ...
    try:
        if _endpoints() is None:
            return False
        return await _has_nous_credential()
    except Exception:
        return False
