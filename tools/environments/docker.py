...
async def _run_docker_command(
    ...
    *,
    ...
    """Run one Docker CLI command and preserve ``subprocess.run`` semantics."""
    if _output_collector is not None and not merge_stderr:
        raise ValueError("bounded Docker capture requires merge_stderr=True")
    ...
    if _output_collector is None:
        ...
        async def communicate_bounded() -> tuple[bytes, bytes | None]:
            ...
            return _output_collector.render().encode("utf-8"), None
    ...
    if timed_out or cancellation is not None:
        ...
        async def _finish() -> tuple[bytes, bytes | None]:
            await _terminate_process(process)
            return await communication

        cleanup_task = asyncio.create_task(_finish())
        try:
            stdout, stderr = await _await_owned(cleanup_task)
        except asyncio.CancelledError as exc:  # noqa: ASYNC103 - raised below
            cancellation = exc
    decoded_stdout = (stdout or b"").decode("utf-8", errors="replace")
    decoded_stderr = (stderr or b"").decode("utf-8", errors="replace")
...
async def _load_hermes_env_vars() -> dict[str, str]:
    ...
    try:
        ...
        values: dict[str, str] = {}
        for raw_line in _sanitize_env_lines(raw_lines):
            line = raw_line.strip()
...
async def _get_active_profile_name() -> str:
    ...
    try:
        ...
        return await get_active_profile_name() or "default"
    except Exception:
        return "default"


async def reap_orphan_containers(
    *,
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    pair.
    """
    docker = docker_exe or await find_docker() or "docker"
    ...
    try:
        listing = await _run_docker_command(
...
async def _container_finished_at(docker_exe: str, container_id: str):
...
async def find_docker() -> str | None:
    ...
    if (
        override
        and await aiofiles.os.path.isfile(override)
        and await aiofiles.os.access(override, os.X_OK)
    ):
        _docker_executable = override
        logger.info("Using HERMES_DOCKER_BINARY override: %s", override)
        return override

    # 2-3. docker/podman on PATH. Resolve candidates with native async stat
    # calls; ``shutil.which`` itself performs synchronous filesystem probes.
    search_path = os.getenv("PATH", os.defpath)
    for executable in ("docker", "podman"):
        for directory in search_path.split(os.pathsep):
            candidate = os.path.join(directory or ".", executable)
            if await aiofiles.os.path.isfile(candidate) and await aiofiles.os.access(
                candidate, os.X_OK
            ):
                _docker_executable = candidate
                if executable == "podman":
                    logger.info("Using podman as container runtime: %s", candidate)
                return candidate

    # 4. Well-known macOS Docker Desktop locations
    for path in _DOCKER_SEARCH_PATHS:
        if await aiofiles.os.path.isfile(path) and await aiofiles.os.access(path, os.X_OK):
...
async def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    try:
        ...
        from agent.proxy_sources import iron_proxy as ip  # type: ignore[import-not-found]
    ...
    if status.ca_cert_path is None or not await aiofiles.os.path.exists(
        status.ca_cert_path
    ):
    ...
    mappings = await ip.load_mappings()
    if not mappings:
        msg = (
...
async def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
...
class DockerEnvironment(BaseEnvironment):
    ...
    async def init_session(self) -> None:
        """Lazily create the container and capture its shell snapshot."""
    ...
    async def _ensure_initialized(self) -> None:
        ...
        async with self._initialize_lock:
            ...
            try:
                ...
                self._init_env_args = await self._build_init_env_args()
                await BaseEnvironment.init_session(self)
            except asyncio.CancelledError:
                if self._container_id is not None:
                    if self._reused_container:
                        ...
                        await _await_owned(cleanup_task)  # noqa: ASYNC120
                raise

    async def _initialize_container(self) -> None:
        parameters = self._docker_parameters
        ...
        host_cwd = parameters["host_cwd"]
        auto_mount_cwd = parameters["auto_mount_cwd"]
        run_as_host_user = parameters["run_as_host_user"]
        extra_args = parameters["extra_args"]
        shm_size = parameters["shm_size"]
        ...
        # cached host-wide.
        resource_args = []
        if cpu > 0 and await _cgroup_limits_available(image):
            resource_args.extend(["--cpus", str(cpu)])
        if memory > 0 and await _cgroup_limits_available(image):
            resource_args.extend(["--memory", f"{memory}m"])
        if await _cgroup_limits_available(image):
        ...
        if disk > 0 and sys.platform != "darwin":
            if await self._storage_opt_supported():
        ...
        host_cwd_abs = (
            await aiofiles.os.path.abspath(await expanduser(host_cwd))
            if host_cwd
            else ""
        )
        bind_host_cwd = (
            ...
            and await aiofiles.os.path.isdir(host_cwd_abs)
            and not workspace_explicitly_mounted
        )
        if (
            auto_mount_cwd
            and host_cwd
            and not await aiofiles.os.path.isdir(host_cwd_abs)
        ):
            logger.debug("Skipping docker cwd mount: host_cwd is not a valid directory: %s", host_cwd)

        writable_args = []
        if self._persistent:
            sandbox = await get_sandbox_dir() / "docker" / self._task_id
            self._home_dir = str(sandbox / "home")
            await aiofiles.os.makedirs(self._home_dir, exist_ok=True)
            writable_args.extend([
                "-v", f"{self._home_dir}:/root",
            ])
            if not bind_host_cwd and not workspace_explicitly_mounted:
                self._workspace_dir = str(sandbox / "workspace")
                await aiofiles.os.makedirs(self._workspace_dir, exist_ok=True)
                writable_args.extend([
        ...
        try:
            ...
            for mount_entry in await get_credential_file_mounts():
            ...
            for skills_mount in await get_skills_directory_mount():
            ...
            for cache_mount in await get_cache_directory_mounts():
        ...
        egress_volume_args, egress_env_overrides, egress_host_args = (
            await _egress_proxy_args_for_docker()
        )
        egress_label = _egress_reuse_fingerprint(
            egress_volume_args, egress_env_overrides, egress_host_args,
        )
        _enforce_egress = await _egress_enforce_on_docker()
        ...
        if egress_env_overrides:
            try:
                from hermes_cli.config import (
                    load_config_readonly as _load_cfg_for_collision,
                )
                _proxy_cfg = ((await _load_cfg_for_collision()).get("proxy") or {})
            except (ImportError, OSError):
                _proxy_cfg = {}
            ...
            try:
                from agent.proxy_sources import (  # type: ignore[import-not-found]
                    iron_proxy as _ip_for_mappings,
                )
                _critical_provider_keys = {
                    m.real_env_name
                    for m in await _ip_for_mappings.load_mappings()
                }
        ...
        try:
            from hermes_cli.config import (
                load_config_readonly as _load_cfg_for_precedence,
            )
            _enforce_egress_merge = bool(
                ((await _load_cfg_for_precedence()).get("proxy") or {})
        ...
        self._docker_exe = await find_docker() or "docker"
        ...
        image_uses_s6_init = await _image_uses_init_entrypoint(
        ...
        profile_name = _sanitize_label_value(await _get_active_profile_name())
        task_label = _sanitize_label_value(self._task_id)
        label_args = [
        ...
        if self._persist_across_processes:
            existing = await self._find_reusable_container(
            ...
            if existing is not None:
                ...
                actual_mode = None
                if not network:
                    actual_mode = await self._container_network_mode(container_id)
        ...
        if not reused:
            ...
            run_failure: subprocess.CalledProcessError | subprocess.TimeoutExpired | None = None
            ...
            if run_failure is not None or run_cancellation is not None:
                ...
                logger.warning(
                    ...
                    container_name, run_failure or run_cancellation,
                )
                async def _remove_orphan() -> None:
                    try:
                        await _run_docker_command(
                        ...
                        logger.debug(
                            "Failed to clean up orphaned container %s",
                            container_name,
                            exc_info=True,
                        )
                ...
                cleanup_task = asyncio.create_task(_remove_orphan())
        ...
        self._reused_container = reused

    async def _build_init_env_args(self) -> list[str]:
        ...
        """
        passthrough_env, unset_names = await self._resolve_passthrough_env()
    ...
    async def _build_passthrough_env(self) -> dict[str, str]:
        """Resolve forwarded host variables through the active profile scope."""
        return (await self._resolve_passthrough_env())[0]

    async def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        try:
            ...
            passthrough_keys = set(await get_all_passthrough())
        except Exception:
            pass
        ...
        hermes_env = await _load_hermes_env_vars() if forward_keys else {}
    ...
    async def _build_runtime_env_args_with_unsets(self) -> tuple[list[str], tuple[str, ...]]:
    ...
    async def _build_runtime_env_args(self) -> list[str]:
        """Build only dynamic forwarded values for a non-login command."""
        return (await self._build_runtime_env_args_with_unsets())[0]
    ...
    async def _run_bash(
        ...
        *,
        ...
        """Run a bash process inside the Docker container."""
        ...
        if login:
            ...
            runtime_args, unset_names = (
                await self._build_runtime_env_args_with_unsets()
            )
        ...
        if unset_names:
            ...
            command = f"unset {quoted_names} 2>/dev/null || true\n{command}"

        cmd.extend([self._container_id])

        if login:
            cmd.extend(["bash", "-l", "-c", command])
        else:
        ...
        try:
            # Give the newly-created transport task the same opportunity to
            # start/finish that upstream's already-spawned ProcessHandle has
    ...
    async def execute(self, command: str, cwd: str = "", **kwargs) -> dict:
        ...
        """
        result = await super().execute(command, cwd, **kwargs)
        if (
            result.get("returncode", 0) != 0
            and self._is_container_gone(result.get("output", ""))
            and self._persist_across_processes
        ):
            if await self._recreate_container():
                result = await super().execute(command, cwd, **kwargs)
        return result

    @staticmethod
    async def _storage_opt_supported() -> bool:
        ...
        try:
            docker = await find_docker() or "docker"
            result = await _run_docker_command(
            ...
            probe = await _run_docker_command(
    ...
    async def _find_reusable_container(
        self,
        task_label: str,
        profile_label: str,
        egress_label: str,
    ...
    async def cleanup(self, *, force_remove: bool = False):
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Cleanup awaits bounded Docker subprocesses directly. If the caller is
        cancelled, the owned stop/remove sequence finishes before cancellation
        is re-raised so no child process or half-removed container is leaked.
        """
        ...
        if not container_id:
            ...
            if not self._persistent:
                for d in (self._workspace_dir, self._home_dir):
                    if d:
                        from tools.credential_files import _remove_tree

                        await _remove_tree(Path(d))
            return
        ...
        async def _teardown() -> None:
            ...
            # container (the dirs are the container's filesystem state;
            # keeping them around with no container would orphan the data).
            if should_remove and not self._persistent:
                for directory in (self._workspace_dir, self._home_dir):
                    if directory:
                        from tools.credential_files import _remove_tree

                        await _remove_tree(Path(directory))

        from tools.environments.file_sync import _await_owned

        await _await_owned(asyncio.create_task(_teardown()))

    async def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        """Return immediately after the awaited cleanup boundary."""
        return True
