...
async def _configured_for_xai_video() -> bool:
    try:
...
async def _check_xai_video_requirements() -> bool:
...
async def _handle_xai_video_edit(args: dict[str, Any], **_kw: Any) -> str:
    ...
    if not await _configured_for_xai_video():
        return _provider_not_configured_error()

    result = await run_xai_video_edit(
        prompt=prompt,
        video_url=video_url,
        model=model,
    )
...
async def _handle_xai_video_extend(args: dict[str, Any], **_kw: Any) -> str:
    ...
    result = await run_xai_video_extend(
        prompt=prompt,
        video_url=video_url,
        duration=duration,
        model=model,
