...
async def _handle_list_entities(args: dict, **kw) -> str:
    """Handler for ha_list_entities tool."""
    domain = args.get("domain")
    area = args.get("area")
    try:
        result = await _async_list_entities(domain=domain, area=area)
...
async def _handle_get_state(args: dict, **kw) -> str:
    ...
    try:
        result = await _async_get_state(entity_id)
        return json.dumps({"result": result})
    except Exception as e:
...
async def _handle_call_service(args: dict, **kw) -> str:
    ...
    try:
        result = await _async_call_service(domain, service, entity_id, data)
        return json.dumps({"result": result})
    except Exception as e:
...
async def _handle_list_services(args: dict, **kw) -> str:
    """Handler for ha_list_services tool."""
    domain = args.get("domain")
    try:
        result = await _async_list_services(domain=domain)
        return json.dumps({"result": result})
    except Exception as e:
