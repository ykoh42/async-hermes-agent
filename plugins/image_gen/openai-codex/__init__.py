...
async def _local_image_to_data_url(value: str) -> str:
    """Read a local image path and return a validated data:image URL."""
    try:
        from agent.file_safety import get_read_block_error

        blocked = await get_read_block_error(value)
        if blocked:
...
async def _to_input_image_part(value: str) -> dict[str, str]:
    ...
    if lowered.startswith("http://") or lowered.startswith("https://"):
        ...
        image_url = await _local_image_to_data_url(candidate)
    return {"type": "input_image", "image_url": image_url}


async def _normalize_input_images(
    image_url: str | None,
    ...
    return [await _to_input_image_part(value) for value in values]
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    async def is_available(self) -> bool:
        if not await _read_codex_access_token():
            return False
        try:
    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        tier_id, meta = await _resolve_model()
        size = _SIZES.get(aspect, _SIZES["square"])

        token = await _read_codex_access_token()
        if not token:
            return error_response(
                error=(
                    "No Codex/ChatGPT OAuth credentials available. Run "
        ...
        try:
            input_images = await _normalize_input_images(
                image_url,
