...
async def _resolve_resolution() -> str:
...
async def _xai_image_field(source: str) -> Dict[str, str]:
    ...
    await raise_if_read_blocked(source)
    async with aiofiles.open(_os.path.expanduser(source), "rb") as fh:
        raw = await fh.read()
...
class XAIImageGenProvider(ImageGenProvider):
    ...
    async def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # xAI service.
        storage_notice = await xai_storage_notice_text("image_gen")
        tag = (
