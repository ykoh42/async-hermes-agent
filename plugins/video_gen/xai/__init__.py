...
async def _raise_if_blocked_local_input(ref: str) -> None:
    ...
    try:
        ...
        return
    await raise_if_read_blocked(ref)


async def _image_ref_to_xai_url(value: str) -> str:
    ...
    await _raise_if_blocked_local_input(ref)
    ...
    async with aiofiles.open(path, "rb") as image_file:
        encoded = base64.b64encode(await image_file.read()).decode("ascii")
...
async def _video_ref_to_xai_url(value: str) -> str:
    ...
    async with aiofiles.open(path, "rb") as video_file:
        encoded = base64.b64encode(await video_file.read()).decode("ascii")
    return f"data:{mime};base64,{encoded}"
...
async def _video_input_from_public_url(
    ...
    *,
    ...
    if await aiofiles.os.path.isfile(path):
        data_ref = await _video_ref_to_xai_url(ref)
...
async def _normalize_reference_images(
    ...
    for url in reference_image_urls or []:
        ...
        if not cleaned:
            continue
        normalized = await _image_ref_to_xai_input(cleaned)
        if not normalized:
            return None, (
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    async def get_setup_schema(self) -> Dict[str, Any]:
        ...
        try:
            ...
            storage_notice = await xai_storage_notice_text("video_gen")
        except Exception:
            storage_notice = ""
        tag = (
...
async def has_xai_video_credentials() -> bool:
    api_key, _ = await _resolve_xai_credentials()
    return bool(api_key)
...
async def run_xai_video_extend(
    *,
    prompt: str,
    video_url: str,
    duration: Optional[int] = None,
...
async def _generate_xai_video(
    *,
    ...
    image_input = None
    image_url_value = (image_url or "").strip()
    if image_url_value:
        image_input = await _image_ref_to_xai_input(image_url_value)
        if not image_input:
            return error_response(
                error=(
    ...
    refs, refs_error = await _normalize_reference_images(reference_image_urls)
    if refs_error:
        return error_response(
            error=refs_error,
...
async def _submit_xai_video_payload(
    *,
    ...
    try:
        ...
        storage_options = await build_xai_storage_options(
            "video_gen",
            filename_prefix="hermes-xai-video",
            extension="mp4",
        )
        storage_notice = await maybe_mark_xai_storage_notice_seen("video_gen")
        storage_cfg = await read_xai_imagine_storage_config("video_gen")
    except Exception:
