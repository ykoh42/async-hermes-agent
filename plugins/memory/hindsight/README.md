...
- Install the native async client with `pip install "async-hermes-agent[hindsight]"`.
- **Cloud:** API key from [ui.hindsight.vectorize.io](https://ui.hindsight.vectorize.io)
- **Local Embedded:** additionally install `hindsight-all`; an API key for a supported LLM provider is required (OpenAI, Anthropic, Gemini, Groq, OpenRouter, MiniMax, Ollama, or any OpenAI-compatible endpoint). Embeddings and reranking run locally — no additional API keys needed.
- **Local External:** A running Hindsight instance (Docker or self-hosted) reachable over HTTP.

## Setup

Select `hindsight` as `memory.provider` in the application configuration, set
`HINDSIGHT_API_KEY` in the profile's environment, and write provider settings
to `$HERMES_HOME/hindsight/config.json`. For example:

```bash
export HINDSIGHT_API_KEY=your-key
mkdir -p "$HERMES_HOME/hindsight"
printf '%s\n' '{"mode":"cloud","bank_id":"hermes"}' > "$HERMES_HOME/hindsight/config.json"
```
...
Provide the LLM API key to the application environment as `HINDSIGHT_LLM_API_KEY`.
...
Requires the distribution-pinned `hindsight-client == 0.6.1`. Missing or incompatible optional dependencies fail with an installation hint; the library never installs or upgrades packages at runtime.
