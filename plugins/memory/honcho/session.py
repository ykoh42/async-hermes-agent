...
class HonchoSessionManager:
    ...
    def __init__(
        ...
        self._remote_cache_lock = asyncio.Lock()
        ...
        self._prefetch_cache_lock = asyncio.Lock()
        ...
        # Async write queue — the writer task starts lazily on first enqueue
    ...
    def honcho(self) -> Honcho:
        """Return the initialized Honcho client without performing I/O."""
        if self._honcho is None:
            raise RuntimeError("Honcho client has not been initialized")
        return self._honcho

    async def _get_honcho(self) -> Honcho:
        if self._honcho is None:
            self._honcho = await get_honcho_client(self._config)
        return self._honcho

    async def _get_or_create_peer(self, peer_id: str) -> Any:
        """
    ...
    async def _get_or_create_honcho_session(
        ...
        try:
            ...
            await session.aio.add_peers([(user_peer, user_config), (assistant_peer, ai_config)])
            ...
            try:
                server_user = await session.aio.get_peer_configuration(user_peer)
                server_ai = await session.aio.get_peer_configuration(assistant_peer)
        ...
        existing_messages = []
        try:
            ctx = await session.aio.context(summary=True, tokens=self._context_tokens)
        ...
        async with self._remote_cache_lock:
            cached = self._sessions_cache.setdefault(session_id, session)
        return cached, existing_messages
    ...
    async def get_or_create(self, key: str) -> HonchoSession:
        """
        ...
        session = await self._build_session(key)

        # Write to cache under lock — only one writer wins
        async with self._cache_lock:
            return self._cache.setdefault(key, session)

    async def _build_session(self, key: str) -> HonchoSession:
        """Build a session from Honcho without reading or writing the local cache."""
        ...
        user_peer = await self._get_or_create_peer(user_peer_id)
        assistant_peer = await self._get_or_create_peer(assistant_peer_id)
        honcho_session, existing_messages = await self._get_or_create_honcho_session(
    ...
    async def _flush_session(self, session: HonchoSession) -> bool:
        """Internal: write unsynced messages to Honcho."""
        if not session.messages:
            return True

        user_peer = await self._get_or_create_peer(session.user_peer_id)
        assistant_peer = await self._get_or_create_peer(session.assistant_peer_id)
        honcho_session = self._sessions_cache.get(session.honcho_session_id)

        if not honcho_session:
            honcho_session, _ = await self._get_or_create_honcho_session(
        ...
        try:
            await honcho_session.aio.add_messages(honcho_messages)
            for msg in new_messages:
                msg["_synced"] = True
    ...
    async def _async_writer_loop(self) -> None:
        """Background task: drain the async write queue in FIFO order."""
        ...
        while True:
            ...
            try:
                ...
                try:
                    success = await self._flush_session(item)
                except Exception as e:
                    success = False
                    first_error = e
                ...
                try:
                    retry_success = await self._flush_session(item)
    ...
    async def save(self, session: HonchoSession) -> None:
        ...
        if wf == "async":
            if self._async_queue is not None:
                await self._ensure_async_writer()
                await self._async_queue.put(session)
        elif wf == "turn":
    ...
    async def flush_all(self) -> None:
        ...
        """
        if self._async_queue is not None and self._async_task is not None:
    ...
    async def _ensure_async_writer(self) -> None:
        """Start the async writer on first enqueue (idempotent)."""
    ...
    async def shutdown(self) -> None:
        """Gracefully flush and shut down the async writer task."""
        ...
        try:
            ...
            if task is not None and not task.done():
                await self._async_queue.put(_ASYNC_SHUTDOWN)
                await task
        except asyncio.CancelledError:
            if task is not None and not task.done():
                task.cancel()
    ...
    async def new_session(self, key: str) -> HonchoSession:
        """
        ...
        # Keep the raw key unavailable until the replacement is fully built,
        # matching upstream's reentrant-lock atomicity without blocking the loop.
        async with self._cache_lock:
            ...
            session = await self._build_session(new_key)
    ...
    async def dialectic_query(
        ...
        try:
            if self._ai_observe_others:
                ...
                ai_peer_obj = await self._get_or_create_peer(session.assistant_peer_id)
                if target_peer_id == session.assistant_peer_id:
                    result = await ai_peer_obj.aio.chat(query, reasoning_level=level) or ""
                else:
                    result = await ai_peer_obj.aio.chat(
                        query,
                ...
                target_peer = await self._get_or_create_peer(target_peer_id)
                result = await target_peer.aio.chat(query, reasoning_level=level) or ""
    ...
    async def prefetch_context(self, session_key: str, user_message: str | None = None) -> None:
        ...
        """
        result = await self.get_prefetch_context(session_key, user_message)
        if result:
            await self.set_context_result(session_key, result)

    async def set_context_result(self, session_key: str, result: dict[str, str]) -> None:
        """Store a prefetched context result in a thread-safe way."""
        if not result:
            return
        async with self._prefetch_cache_lock:
            self._context_cache[session_key] = result

    async def pop_context_result(self, session_key: str) -> dict[str, str]:
        """
    ...
    async def get_prefetch_context(self, session_key: str, user_message: str | None = None) -> dict[str, str]:
        """
        ...
        try:
            honcho_session = self._sessions_cache.get(session.honcho_session_id)
            if honcho_session:
                ctx = await honcho_session.aio.context(summary=True)
        ...
        try:
            ...
            user_ctx = await self._fetch_peer_context(observer_peer_id, search_query=user_message or None, target=target_peer_id or session.user_peer_id)
        ...
        try:
            ai_ctx = await self._fetch_peer_context(session.assistant_peer_id, target=session.assistant_peer_id)
    ...
    async def migrate_local_history(self, session_key: str, messages: list[dict[str, Any]]) -> bool:
        """
        ...
        try:
            await honcho_session.aio.upload_file(
    ...
    async def migrate_memory_files(self, session_key: str, memory_dir: str) -> bool:
        """
        ...
        for filename, upload_name, description, target_peer, target_kind in files:
            ...
            if not await aiofiles.os.path.exists(filepath):
                continue
            async with aiofiles.open(filepath, encoding="utf-8") as handle:
    ...
    async def _fetch_peer_card(self, peer_id: str, *, target: str | None = None) -> list[str]:
        ...
        """
        peer = await self._get_or_create_peer(peer_id)
        getter = getattr(peer.aio, "get_card", None)
        if callable(getter):
            value = await (getter(target=target) if target is not None else getter())
            return self._normalize_card(value)

        legacy_getter = getattr(peer.aio, "card", None)
    ...
    async def _fetch_peer_context(
        ...
        *,
        ...
        try:
            ...
            ctx = (
                await peer.aio.context(**context_kwargs)
                if context_kwargs
                else await peer.aio.context()
            )
            representation = (
        ...
        if not representation:
            try:
                representation = (
                    await peer.aio.representation(target=target)
                    if target is not None
                    else await peer.aio.representation()
                ) or ""
            except Exception as e:
        ...
        if not card:
            try:
                card = await self._fetch_peer_card(peer_id, target=target)
            except Exception as e:
                logger.debug("Direct peer card fetch failed for '%s': %s", peer_id, e)

        return {"representation": representation, "card": card}

    async def get_session_context(self, session_key: str, peer: str = "user") -> dict[str, Any]:
        ...
        if not honcho_session:
            ...
            return await self._fetch_peer_context(peer_id, target=peer_id)

        try:
            observer_peer_id, target_peer_id = self._resolve_observer_target(session, peer)
            ctx = await honcho_session.aio.context(
                summary=True,
    ...
    async def get_peer_card(self, session_key: str, peer: str = "user") -> list[str]:
        """
        ...
        try:
            observer_peer_id, target_peer_id = self._resolve_observer_target(session, peer)
            card = await self._fetch_peer_card(observer_peer_id, target=target_peer_id)
            if card:
                return card
            ...
            if target_peer_id:
                return await self._fetch_peer_card(target_peer_id)
            return []
        except Exception as e:
    ...
    async def search_context(
        ...
        try:
            honcho = await self._get_honcho()
            messages = await honcho.aio.search(
                q,
            ...
            try:
                peer_obj = await self._get_or_create_peer(peer_id)
                messages = await peer_obj.aio.search(q, limit=limit)
            except Exception as e2:
    ...
    async def _conclusions_scope(self, session: Any, target_peer_id: str) -> Any:
        ...
        """
        if target_peer_id == session.assistant_peer_id:
            observer = await self._get_or_create_peer(session.assistant_peer_id)
    ...
    async def create_conclusion(self, session_key: str, content: str, peer: str = "user") -> bool:
        ...
        try:
            ...
            conclusions_scope = await self._conclusions_scope(session, target_peer_id)
            await conclusions_scope.aio.create([{
    ...
    async def delete_conclusion(self, session_key: str, conclusion_id: str, peer: str = "user") -> bool:
        ...
        try:
            target_peer_id = self._resolve_peer_id(session, peer)
            scope = await self._conclusions_scope(session, target_peer_id)
            await scope.aio.delete(conclusion_id)
    ...
    async def list_conclusions(
        ...
        try:
            ...
            scope = await self._conclusions_scope(session, target_peer_id)
            if query:
                conclusions = await scope.aio.query(query, top_k=limit)
            else:
                conclusions = (await scope.aio.list(size=limit)).items
            return [{"id": c.id, "content": c.content} for c in conclusions]
        except Exception as e:
            logger.debug("Honcho list_conclusions failed: %s", e)
            return []

    async def set_peer_card(self, session_key: str, card: list[str], peer: str = "user") -> list[str] | None:
        ...
        try:
            ...
            if observer_peer_id is None:
                ...
                return None
            peer_obj = await self._get_or_create_peer(observer_peer_id)
            result = (
                await peer_obj.aio.set_card(card, target=target_peer_id)
                if target_peer_id is not None
                else await peer_obj.aio.set_card(card)
            )
            logger.info(
    ...
    async def seed_ai_identity(self, session_key: str, content: str, source: str = "manual") -> bool:
        """
        ...
        try:
            ...
            await honcho_session.aio.add_messages([assistant_peer.message(wrapped)])
    ...
    async def get_ai_representation(self, session_key: str) -> dict[str, str]:
        """
        ...
        try:
            ctx = await self._fetch_peer_context(session.assistant_peer_id, target=session.assistant_peer_id)
            return {
