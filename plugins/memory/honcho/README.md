...
- Install the package extra: `uv sync --extra honcho` for a source checkout or
  `pip install "async-hermes-agent[honcho]"` for the distribution
- A Honcho Cloud API key from [app.honcho.dev](https://app.honcho.dev), or a
  self-hosted instance

## Configuration

Enable the `memory` toolset when constructing `AIAgent` and select Honcho in
...
`HERMES_HOME` defaults to `~/.hermes`. Put provider settings in
`$HERMES_HOME/honcho.json`, and put `HONCHO_API_KEY` in the process environment
or `$HERMES_HOME/.env`. A minimal cloud configuration is:
...
For self-hosting, set `baseUrl` in the same file. All provider, OAuth refresh,
session, peer, search, and conclusion I/O uses the pinned SDK's native async
transport. Close the owning `AIAgent` with `await agent.close()` or its async
context manager so pending writes and the HTTP client are finalized.
...
cycle (not per pass). Select a fast, inexpensive auxiliary model for the
`memory_query_rewrite` task in `config.yaml`; its request timeout is
`auxiliary.memory_query_rewrite.timeout` (default 8s). The
...
### Identity Mapping (Downstream Multi-User Services)

Downstream services may supply a platform-native runtime user ID. These three
keys control how those IDs map to Honcho peers. The resolver is config-driven
and deterministic — no automatic merging or runtime inference.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `pinUserPeer` | bool | `false` | When `true`, every runtime user collapses to `peerName`. Useful for single-operator services that intentionally share one peer |
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read
> for back-compat (`pinUserPeer` wins where both are set).
...
6. peerName                      → no runtime ID supplied
...
Flipping `pinUserPeer` from `true` to `false` does not migrate existing data.
Memory accumulated under `peerName` remains there; use `userPeerAliases` to map
the operator's runtime IDs back to that peer when continuity is required.
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `writeFrequency` | string/int | `"async"` | `"async"` (owned background task), `"turn"` (await each turn), `"session"` (awaited batch on end), or integer N (every N turns) |
...
| 2 | `session_title` supplied by the caller | `"refactor-auth"` |
| 3 | Downstream service session key | `"agent-main-telegram-dm-8439114563"` |
...
Downstream service calls that provide a session key always resolve via priority
3 regardless of `sessionStrategy`. The strategy applies when no downstream key
is supplied.
...
- **`per-directory`** — basename of `$PWD`. Running an agent in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Hermes session ID (timestamp + hex). Every agent session starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
...
Host key is derived from the active Hermes profile: `hermes` (default) or
`hermes_<profile>` (for example, profile `coder` uses `hermes_coder`). Older
`hermes.<profile>` host blocks are still read for compatibility.
