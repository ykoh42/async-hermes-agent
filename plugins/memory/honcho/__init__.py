...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def __init__(
        ...
        self._prefetch_task_started_at: float = 0.0
    ...
    async def is_available(self) -> bool:
        ...
        try:
            ...
            cfg = await HonchoClientConfig.from_global_config()
    ...
    def _task_done(self, task: asyncio.Task[None]) -> None:
        ...
        try:
            ...
            logger.debug("Honcho background task failed", exc_info=True)
        else:
            if error is not None:
                logger.debug(
                    "Honcho background task failed: %s", error,
                    exc_info=error,
                )
    ...
    async def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            self._session_key = await self._resolve_session_key(cfg, session_id, **kwargs)
            ...
            await self._start_session_init_background(wait_timeout=0.1)
    ...
    async def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        """Start Honcho session initialization in an owned task.
        ...
        async with self._init_lock:
            ...
            async def _run() -> None:
                try:
                    await self._do_session_init(cfg, init_session_id, **init_kwargs)
            ...
            self._init_task = self._track_task(asyncio.create_task(
                _run(),
                name="honcho-session-init",
            ))
        if wait_timeout > 0 and self._init_task is not None:
            try:
                await asyncio.wait_for(
                    asyncio.shield(self._init_task), timeout=wait_timeout
                )
            except TimeoutError:
                pass

    async def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        session = await self._manager.get_or_create(self._session_key)
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                await self._manager.migrate_memory_files(self._session_key, mem_dir)
        ...
        if self._recall_mode in {"context", "hybrid"}:
            if self._query_rewriter is None or not self._query_rewrite_enabled:
                ...
                async def _prewarm_dialectic() -> None:
                    try:
                        r = await self._run_dialectic_depth(
                ...
                self._prefetch_task_started_at = time.monotonic()
                self._prefetch_task = self._track_task(asyncio.create_task(
                    _prewarm_dialectic(),
    ...
    def _session_ready(self) -> bool:
        ...
        if self._session_initialized:
            return True
        return not (self._init_task and not self._init_task.done())
    ...
    async def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        if not self._session_ready():
            ...
            await self._start_session_init_background()
                ...
                if init_task is not None:
                    try:
                        await asyncio.wait_for(
                            ...
                            timeout=max(
                                0.0,
                                first_turn_base_deadline - time.monotonic(),
                            ),
                        )
                    except TimeoutError:
                        pass
        ...
        if self._is_trivial_prompt(query):
            ready = await self._consume_pending_dialectic()
        ...
        if not _skip_base:
            ...
            async with self._base_context_lock:
            ...
            if _first_base_fetch and self._manager:
                ...
                async def _fetch_base() -> None:
                    try:
                        ctx = await self._manager.get_prefetch_context(
                            self._session_key, query or None
                        ) or {}
                        _ctx_holder["ctx"] = ctx
                        if ctx:
                            await self._manager.set_context_result(self._session_key, ctx)
                    except Exception as e:
                        logger.debug("Honcho first-turn base context failed: %s", e)

                self._base_task = self._track_task(asyncio.create_task(
                    _fetch_base(), name="honcho-base-first"
                ))
                _base_wait = (
                ...
                try:
                    await asyncio.wait_for(
                        asyncio.shield(self._base_task), timeout=_base_wait
                    )
                except TimeoutError:
                    pass
                _ctx = _ctx_holder.get("ctx")
                if _ctx:
                    await self._manager.pop_context_result(self._session_key)
                    formatted = self._format_first_turn_context(_ctx)
                    if formatted:
                        async with self._base_context_lock:
                            self._base_context_cache = formatted
                        base_context = formatted
                elif self._base_task and not self._base_task.done():
                    logger.debug(
            ...
            if not _first_base_fetch and self._manager:
                fresh_ctx = await self._manager.pop_context_result(self._session_key)
                if fresh_ctx:
        ...
        if self._last_dialectic_turn == -999 and query:
            ...
            if self._thread_is_live():
                _live = self._prefetch_task
                if _live is not None:
                    try:
                        await asyncio.wait_for(
                            asyncio.shield(_live), timeout=_dia_wait
                        )
                    except TimeoutError:
                        pass
            else:
                ...
                async def _run_first_turn() -> None:
                    try:
                        r = await self._run_dialectic_depth(query)
                    except Exception as exc:
        ...
        dialectic_result = await self._consume_pending_dialectic()
    ...
    async def _consume_pending_dialectic(self) -> str:
    ...
    async def queue_prefetch(self, query: str, *, session_id: str = "") -> None:
        ...
        """
        if self._cron_skipped or self._shutdown:
            return
        ...
        async def _run():
            try:
                result = await self._run_dialectic_depth(query)
            except Exception as e:
    ...
    def _thread_is_live(self) -> bool:
        """Preserved liveness guard name, now backed by the owned async task."""
        ...
        age = time.monotonic() - self._prefetch_task_started_at
    ...
    def liveness_snapshot(self) -> dict:
        ...
        thread_age = None
        if self._prefetch_task and not self._prefetch_task.done():
            thread_age = time.monotonic() - self._prefetch_task_started_at
        return {
    ...
    async def _run_dialectic_depth(self, query: str, *, use_query_rewrite: bool = True) -> str:
        ...
        if use_query_rewrite and self._query_rewrite_enabled and self._query_rewriter:
            try:
                rewritten_query = (await self._query_rewriter(query)).strip()
            except Exception as exc:
        ...
        for i in range(self._dialectic_depth):
            ...
            result = await self._manager.dialectic_query(
    ...
    async def sync_turn(
        ...
        *,
        ...
        try:
            ...
            await self._manager._flush_session(session)
    ...
    async def on_memory_write(
        ...
        try:
            await self._manager.create_conclusion(self._session_key, content)
        except asyncio.CancelledError:
            raise
        except Exception as e:
    ...
    async def on_session_end(self, messages: List[Dict[str, Any]]) -> None:
        ...
        if not self._manager:
            return
        if not self._session_initialized and self._init_task and not self._init_task.done():
            return
        try:
    ...
    async def handle_tool_call(self, tool_name: str, args: dict, **kwargs) -> str:
        ...
        try:
            if tool_name == "honcho_profile":
                ...
                if card_update:
                    result = await self._manager.set_peer_card(self._session_key, card_update, peer=peer)
                    if result is None:
                        return tool_error("Failed to update peer card.")
                    return json.dumps({"result": f"Peer card updated ({len(result)} facts).", "card": result})
                card = await self._manager.get_peer_card(self._session_key, peer=peer)
                if not card:
                ...
                peer = args.get("peer", "user")
                result = await self._manager.search_context(
                ...
                peer = args.get("peer", "user")
                ctx = await self._manager.get_session_context(self._session_key, peer=peer)
                if not ctx:
                ...
                if list_mode:
                    conclusions = await self._manager.list_conclusions(
                        self._session_key, query=query or None, peer=peer
                    )
                    return json.dumps({"conclusions": conclusions})
                if has_delete_id:
                    ok = await self._manager.delete_conclusion(self._session_key, delete_id, peer=peer)
                    if ok:
                        return json.dumps({"result": f"Conclusion {delete_id} deleted."})
                    return tool_error(f"Failed to delete conclusion {delete_id}.")
                ok = await self._manager.create_conclusion(self._session_key, conclusion, peer=peer)
                if ok:
