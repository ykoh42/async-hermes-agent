...
- `uv sync --extra supermemory` for a source checkout, or install the
  `supermemory` package extra from the distribution
...
Enable the `memory` toolset when constructing `AIAgent` and select Supermemory
...
`HERMES_HOME` defaults to `~/.hermes`. Put the API key in
...
Add the local endpoint to `$HERMES_HOME/supermemory.json`:
...
The provider resolves the endpoint before constructing its native async SDK
client, so SDK operations and session ingest remain on the configured server.
...
for SDK operations, connection probes, and full-session conversation ingest.
...
- buffer the full conversation and ingest it as **one session** at session end (or on session switch, branch, compression, or shutdown)
