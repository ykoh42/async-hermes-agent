...
class FactRetriever:
    ...
    async def probe(
        ...
        if not hrr._HAS_NUMPY:
            ...
            return await self.search(entity, category=category, limit=limit)

        conn, lock = await self.store._ready()
        ...
        if category:
            ...
            async with lock:
                bank_row = await self.store._fetchone(
                    conn,
            ...
            if bank_row:
                ...
                return await self._score_facts_by_vector(
    ...
    async def contradict(
        ...
        async with lock:
            for row in rows:
                fid = row["fact_id"]
                entity_rows = await conn.execute_fetchall(
                    """
    ...
    async def _score_facts_by_vector(
        self,
