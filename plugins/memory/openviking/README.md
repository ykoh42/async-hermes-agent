...
Select the provider in the active `config.yaml`:
...
Secrets and environment overrides belong in the active profile's `.env` file.
For the default profile that is `~/.hermes/.env`; for a named profile use
...
To reuse an existing OpenViking client config, set
`memory.openviking.use_ovcli_config: true` and optionally
`memory.openviking.ovcli_config_path`. The default path is
`~/.openviking/ovcli.conf`.

The provider lifecycle is coroutine-only. `AIAgent` initializes and shuts it
down at awaited boundaries. Direct provider integrations use the same public
names without a synchronous wrapper:

```python
from plugins.memory.openviking import OpenVikingMemoryProvider

provider = OpenVikingMemoryProvider()
await provider.initialize("session-id", hermes_home="/path/to/.hermes")
try:
    context = await provider.prefetch("What should I recall?", session_id="session-id")
finally:
...
Provider config is read from `config.yaml`, the linked `ovcli.conf`, and
environment variables. Environment values take precedence:
