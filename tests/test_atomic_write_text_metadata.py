"""``atomic_write_text`` metadata preservation, ported from upstream.

The retained utility is synchronous by design: it is a small atomic primitive
used at file-write boundaries.  Its callers may remain async while preserving
the upstream mode/owner contract.
"""
...
class TestPreserveMode:
    ...
    def test_no_owner_calls_without_opt_in(
        ...
        monkeypatch.setattr("utils._preserve_file_owner", lambda _p: (123, 456))
        monkeypatch.setattr(
            "utils.os.chown", lambda *args: chown_calls.append(args)
        )
