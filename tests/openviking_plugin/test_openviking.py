...
async def wait_prefetch(provider, query="What should we recall?", session_id="session-test"):
    return await provider.prefetch(query, session_id=session_id)


class TestOpenVikingSummaryUriNormalization:
    async def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
...
class TestOpenVikingSkillQuerySafety:
    async def test_derive_returns_empty_string_for_non_string_input(self):
    ...
    async def test_skill_markers_match_hermes_scaffolding(self, tmp_path, monkeypatch):
        ...
        single = await skill_commands.build_skill_invocation_message(
            "/example",
        ...
        await skill_bundles.scan_bundles()
        bundle_result = await skill_bundles.build_bundle_invocation_message(
            "/demo",
            user_instruction="hello",
        )
    ...
    async def test_prefetch_searches_only_slash_skill_user_instruction(self, monkeypatch):
        ...
        await provider.prefetch(skill_message)
    ...
    async def test_sync_turn_stores_only_slash_skill_user_instruction(self, monkeypatch):
        RecordingVikingClient.calls = []
        monkeypatch.setattr(openviking_plugin, "_VikingClient", RecordingVikingClient)
        provider = OpenVikingMemoryProvider()
        provider._client = cast(Any, object())
        ...
        await provider.sync_turn(skill_message, "Done.")
        assert await provider._drain_writers("session-1", timeout=5.0)
...
class TestOpenVikingConfigSchema:
    async def test_recall_policy_options_are_exposed_in_setup_schema(self):
        provider = OpenVikingMemoryProvider()
        ...
        assert await provider._recall_config() == {
            "limit": 6,
    ...
    async def test_recall_config_reads_from_config_yaml(self, monkeypatch, tmp_path):
...
""",
        ...
        provider = OpenVikingMemoryProvider()
        cfg = await provider._recall_config()
        ...
        assert await provider._profile_token_budget() == 7000

    async def test_recall_config_env_overrides_config_yaml(self, monkeypatch, tmp_path):
...
""",
    ...
    async def test_recall_config_partial_config_yaml(self, monkeypatch, tmp_path):
...
""",
    ...
    async def test_dashboard_shaped_string_values_are_typed(self, monkeypatch):
        for key in list(os.environ):
        ...
        assert cfg["resources"] is True
        assert await provider._profile_token_budget() == 7500

    async def test_invalid_recall_values_fall_back_without_type_errors(self, monkeypatch):
        for key in list(os.environ):
            if key.startswith("OPENVIKING_RECALL_") or key == "OPENVIKING_PROFILE_TOKEN_BUDGET":
                monkeypatch.delenv(key, raising=False)
        monkeypatch.setattr(
        ...
        assert cfg["prefer_abstract"] is False
        assert await provider._profile_token_budget() == 6000

    async def test_recall_env_overrides_string_config_with_native_types(self, monkeypatch):
        monkeypatch.setattr(
            openviking_plugin,
            "_load_hermes_openviking_config",
            AsyncMock(return_value={
        ...
        cfg = await OpenVikingMemoryProvider()._recall_config()
...
class TestOpenVikingTurnConversion:
    async def test_extract_current_turn_anchors_on_latest_matching_user_and_assistant(self):
        messages = [
    ...
    async def test_messages_to_openviking_batch_coalesces_tool_results(self):
        turn = [
            {"role": "user", "content": "Please inspect the repository for assemble hooks."},
            {
                "role": "assistant",
...
class TestOpenVikingRead:
    async def test_overview_read_normalizes_uri_and_unwraps_result(self):
        provider = OpenVikingMemoryProvider()
        ...
        result = json.loads(await provider._tool_read({"uri": "viking://user/hermes/.overview.md", "level": "overview"}))
    ...
    async def test_read_accepts_uri_batch_and_caps_batch_full_content(self):
        provider = OpenVikingMemoryProvider()
        uris = [
        ...
        result = json.loads(await provider._tool_read({"uris": uris, "level": "full"}))
    ...
    async def test_summary_uri_error_does_not_fallback_and_raises(self):
        provider = OpenVikingMemoryProvider()
        provider._client = FakeVikingClient(
            {
                (
        ...
        try:
            await provider._tool_read({"uri": "viking://user/hermes/.overview.md", "level": "overview"})
...
class TestOpenVikingAutoRecallPrefetch:
    async def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch):
        ...
        provider = OpenVikingMemoryProvider()
        try:
            await provider.initialize("e2e-session")
            block = await provider.prefetch("What should we recall?", session_id="e2e-session")
        finally:
        ...
        profile_listings = records["listings"][1:]
        assert all(listing["output"] == "agent" for listing in profile_listings)
        assert all(listing["recursive"].lower() == "true" for listing in profile_listings)
        assert all(listing["abs_limit"] == "512" for listing in profile_listings)
        assert all(listing["node_limit"] == "512" for listing in profile_listings)
...
class TestOpenVikingBrowse:
    async def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        provider = OpenVikingMemoryProvider()
        provider._client = FakeVikingClient(
            {
                (
        ...
        result = json.loads(await provider._tool_browse({"action": "list", "path": "viking://user/hermes"}))
...
class TestOpenVikingMemoryUriBuilder:
    ...
    async def test_uri_layout_includes_peer_segment(self):
...
class TestEnsureClientReloadsEnv:
    ...
    async def test_ensure_client_rebuilds_when_api_key_changes(self, monkeypatch):
        constructions = []
    ...
    async def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        instances = []
        ...
        out = json.loads(await provider.handle_tool_call(
            "viking_remember",
            {"content": "stable fact"},
        ))
    ...
    async def test_concurrent_refresh_does_not_return_stale_client(self, monkeypatch):
        ...
        class _StubClient:
            ...
            async def health(self):
                if self.endpoint == "https://new.example":
                    refresh_entered.set()
                    await asyncio.wait_for(release_refresh.wait(), timeout=2.0)
                    return False
                return True
        ...
        completed_during_refresh = second.done()
        release_refresh.set()
        results = await asyncio.gather(first, second)
        assert completed_during_refresh is False
        assert results == [None, None]
        assert all(client is not stale_client for client in results)


    async def test_concurrent_local_runtime_recovery_starts_once(self, monkeypatch):
        provider = OpenVikingMemoryProvider()
        ...
        monkeypatch.setattr(
            ...
            lambda **kwargs: setattr(
                provider,
                "_runtime_start_task",
                asyncio.create_task(waiter_release.wait()),
            ),
            raising=False,
        )

        first = asyncio.create_task(provider._handle_runtime_openviking_unreachable())
        await asyncio.wait_for(first_start_entered.wait(), timeout=2.0)
        second = asyncio.create_task(provider._handle_runtime_openviking_unreachable())
        await asyncio.sleep(0)
        release_start.set()
        await asyncio.gather(first, second)
        assert start_calls == ["http://127.0.0.1:31933"]
        provider._runtime_start_task.cancel()
        await asyncio.gather(provider._runtime_start_task, return_exceptions=True)
...
class TestEnsureClientFailureHardening:
    ...
    async def test_failed_config_probes_once_then_cools_down(self, monkeypatch):
        ...
        assert await provider._ensure_client() is None
        assert await provider._ensure_client() is None
        assert await provider._ensure_client() is None
    ...
    async def test_conn_snapshot_published_only_on_healthy(self, monkeypatch):
        class _StubClient:
            def __init__(self, endpoint, api_key="", account="", user="", agent=""):
                self.endpoint = endpoint
        ...
        provider = OpenVikingMemoryProvider()
        provider._env_refresh_enabled = True
        assert await provider._ensure_client() is not None
...
class TestUnavailableWarningsPromiseRetry:
    ...
    async def test_local_autostart_refused_warning(self, monkeypatch):
        monkeypatch.setattr(
            openviking_plugin,
    ...
    async def test_still_unhealthy_after_autostart_warning(self, monkeypatch):
        ...
        await provider._finish_runtime_openviking_start(warning_callback=warnings.append)

        assert provider._client is None
        assert len(warnings) == 1
        self._assert_promises_retry(warnings[0])

    async def test_attach_failure_after_autostart_warning(self, monkeypatch):
    ...
    async def test_initialize_responded_unhealthy_warning(self, monkeypatch, tmp_path):
    ...
    async def test_startup_failure_really_does_reconnect_on_a_later_access(
