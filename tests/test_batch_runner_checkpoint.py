...
class TestResumePreservesProgress:
    ...
    @pytest.mark.asyncio
    async def test_incomplete_persisted_row_is_not_treated_as_completed(
        self, tmp_path, monkeypatch
    ):
        dataset = tmp_path / "dataset.jsonl"
        dataset.write_text(json.dumps({"prompt": "retry me"}) + "\n")
        batch_runner = BatchRunner(
            dataset_file=str(dataset),
            batch_size=1,
            run_name="retry-incomplete",
        )
        batch_runner.output_dir = tmp_path / "data" / "retry-incomplete"
        batch_runner.checkpoint_file = batch_runner.output_dir / "checkpoint.json"
        batch_runner.stats_file = batch_runner.output_dir / "statistics.json"
        batch_runner.output_dir.mkdir(parents=True)
        (batch_runner.output_dir / "batch_0.jsonl").write_text(
            json.dumps(
                {
                    "prompt_index": 0,
                    "completed": False,
                    "conversations": [
                        {"from": "human", "value": "retry me"},
                    ],
                }
            )
            + "\n"
        )
        batch_runner.checkpoint_file.write_text(
            json.dumps(
                {
                    "run_name": "retry-incomplete",
                    "completed_prompts": [0],
        ...
        incomplete = set()
        completed = await batch_runner._scan_completed_prompts_by_content(incomplete)
        ...
        async def process_prompt(prompt_index, prompt_data, *_args, **_kwargs):
            return {
                ...
                "trajectory": [
                    ...
                    {"from": "gpt", "value": "<think>retry</think>done"},
                ],
                "reasoning_stats": {
        ...
        await batch_runner.run(resume=True)

        merged = [
            json.loads(line)
            for line in (batch_runner.output_dir / "trajectories.jsonl")
            .read_text()
            .splitlines()
        ]
        checkpoint = json.loads(batch_runner.checkpoint_file.read_text())
        assert [row["prompt_index"] for row in merged] == [0]
