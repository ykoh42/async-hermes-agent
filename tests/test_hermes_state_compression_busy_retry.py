...
async def db(tmp_path: Path) -> SessionDB:
    ...
    await d.create_session("sess1", source="test")
    yield d
    await d.close()


@pytest.mark.asyncio
async def test_append_waits_out_a_live_compression_lock(db: SessionDB) -> None:
    ...
    async def _release_soon():
        ...
        await db.release_compression_lock("sess1", "compressor")
        released.set()
    ...
    try:
        ...
        await db.append_message(
            "sess1", role="user", content="steered mid-compression"
        )
    ...
    rows = await db.get_messages("sess1")
...
@pytest.mark.asyncio
async def test_append_still_gives_up_when_the_lock_never_clears(
    ...
    with pytest.raises(CompressionSessionBusyError):
        await db.append_message("sess1", role="user", content="never lands")
    elapsed = time.monotonic() - started
...
@pytest.mark.asyncio
async def test_the_lock_owner_is_never_delayed_by_its_own_lock(db: SessionDB) -> None:
    assert await db.try_acquire_compression_lock("sess1", "compressor") is True
