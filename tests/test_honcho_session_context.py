...
@pytest.mark.asyncio
async def test_session_context_user_alias_uses_assistant_observer_when_ai_can_observe_others():
    mgr, fake = _manager_with_cached_session(ai_observe_others=True)

    result = await mgr.get_session_context("test-session", peer="user")
...
@pytest.mark.asyncio
async def test_session_context_explicit_user_peer_matches_user_alias():
    mgr, fake = _manager_with_cached_session(ai_observe_others=True)

    await mgr.get_session_context("test-session", peer="chris")
...
@pytest.mark.asyncio
async def test_session_context_user_alias_uses_user_self_observer_when_ai_cannot_observe_others():
    mgr, fake = _manager_with_cached_session(ai_observe_others=False)

    await mgr.get_session_context("test-session", peer="user")
