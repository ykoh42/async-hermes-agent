...
class TestDefaults:
    ...
    @pytest.mark.asyncio
    async def test_get_limits_returns_defaults_when_load_config_raises(self):
        async def _boom():
...
class TestOverrides:
    @pytest.mark.asyncio
    async def test_user_config_overrides_all_three(self):
        cfg = {
            "tool_output": {
                "max_bytes": 100_000,
                "max_lines": 5000,
...
class TestShortcuts:
    @pytest.mark.asyncio
    async def test_individual_accessors_delegate_to_get_tool_output_limits(self):
        cfg = {
            "tool_output": {
                "max_bytes": 111,
                "max_lines": 222,
...
class TestIntegrationReadPagination:
    ...
    @pytest.mark.asyncio
    async def test_pagination_limit_clamped_by_config_value(self):
