...
class TestCronApprovalModeParsing:
    def test_default_is_deny(self):
        ...
        with mock_patch.object(approval_module, "_approval_config_snapshot", {}):
            assert _get_cron_approval_mode() == "deny"

    def test_explicit_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch.object(approval_module, "_approval_config_snapshot", {"cron_mode": "deny"}):
            assert _get_cron_approval_mode() == "deny"

    def test_explicit_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch.object(approval_module, "_approval_config_snapshot", {"cron_mode": "approve"}):
    ...
    def test_off_maps_to_approve(self):
        ...
        from unittest.mock import patch as mock_patch
        with mock_patch.object(approval_module, "_approval_config_snapshot", {"cron_mode": "off"}):
            assert _get_cron_approval_mode() == "approve"

    def test_allow_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch.object(approval_module, "_approval_config_snapshot", {"cron_mode": "allow"}):
            assert _get_cron_approval_mode() == "approve"

    def test_yes_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch.object(approval_module, "_approval_config_snapshot", {"cron_mode": "yes"}):
            assert _get_cron_approval_mode() == "approve"
    ...
    def test_unknown_value_defaults_to_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch.object(approval_module, "_approval_config_snapshot", {"cron_mode": "maybe"}):
            assert _get_cron_approval_mode() == "deny"
    ...
    def test_yaml_boolean_false_maps_to_deny(self):
        ...
        from unittest.mock import patch as mock_patch
        with mock_patch.object(approval_module, "_approval_config_snapshot", {"cron_mode": False}):
