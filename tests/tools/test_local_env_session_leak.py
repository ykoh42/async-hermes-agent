...
@pytest.mark.asyncio
async def test_local_environment_child_receives_task_local_session(monkeypatch, tmp_path):
    monkeypatch.setenv("HERMES_SESSION_KEY", "foreign-global-session")
    tokens = set_session_vars(
        session_key="agent:library:local:real-session",
        platform="local",
        chat_id="real-session",
    )
    try:
        environment = LocalEnvironment(cwd=str(tmp_path))
        result = await environment.execute('printf "%s" "$HERMES_SESSION_KEY"')
    finally:
        clear_session_vars(tokens)

    assert result == {
        "output": "agent:library:local:real-session",
        "returncode": 0,
    }
