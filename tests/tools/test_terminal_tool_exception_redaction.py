...
async def test_generic_exception_result_redacts_error_and_traceback(monkeypatch):
    _force_exception(monkeypatch, RuntimeError(f"connect failed OPENAI_API_KEY={SECRET}"))
    result = json.loads(await terminal_tool.terminal_tool("echo hi"))
...
async def test_degraded_fail_mode_result_redacts_error_and_traceback(monkeypatch):
