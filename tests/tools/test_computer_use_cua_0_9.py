"""Native-async parity contracts for cua-driver 0.9-era behavior."""
...
async def _fixture():
    async with aiofiles.open(FIXTURE, encoding="utf-8") as stream:
...
@pytest.mark.asyncio
async def test_normalized_fixture_is_sanitized_and_records_the_selected_contract():
    ...
    assert all(
        "input.delivery_mode" not in tool["capabilities"]
...
@pytest.mark.asyncio
async def test_foreground_support_is_discovered_from_tool_input_schema():
    ...
    await session._populate_capabilities(_McpSession())
...
@pytest.mark.asyncio
async def test_foreground_focus_is_a_separate_call_before_action():
    session = _FakeSession(input_properties={"click": {"delivery_mode"}})
    result = await _make_backend(session).click(
        element=3, delivery_mode="foreground", bring_to_front=True
    )
    assert result.ok is True
    assert [name for name, _ in session.calls] == ["bring_to_front", "click"]
    assert session.calls[0][1] == {"pid": 42, "window_id": 7}
    assert session.calls[1][1]["delivery_mode"] == "foreground"
    assert "bring_to_front" not in session.calls[1][1]


@pytest.mark.asyncio
async def test_foreground_refuses_only_when_schema_lacks_delivery_property():
...
@pytest.mark.asyncio
async def test_invalid_delivery_mode_is_rejected_before_driver_call():
    session = _FakeSession(input_properties={"type_text": {"delivery_mode"}})
    result = await _make_backend(session).type_text(
        "hello", delivery_mode="sideways"
    )
    assert result.ok is False
...
@pytest.mark.asyncio
async def test_backends_are_isolated_by_hermes_session_and_reused_within_it():
    ...
    with patch("tools.computer_use.cua_backend.CuaDriverBackend", _Backend):
        first = await computer_use._get_backend(session_id="conversation-a")
        again = await computer_use._get_backend(session_id="conversation-a")
        second = await computer_use._get_backend(session_id="conversation-b")
...
@pytest.mark.asyncio
async def test_release_seam_stops_exact_backend_and_clears_session_state():
    from tools.computer_use import tool as computer_use
    ...
    computer_use._backend_call_locks.update(
        {"conversation-a": asyncio.Lock(), "conversation-b": asyncio.Lock()}
    )
    computer_use._session_auto_approve["conversation-a"] = True
    computer_use._always_allow["conversation-a"] = {("click", "background")}
    assert await computer_use.release_computer_use_session("conversation-a") is True
    assert await computer_use.release_computer_use_session("conversation-a") is False
...
@pytest.mark.asyncio
async def test_release_seam_evicts_state_even_when_backend_stop_fails():
    from tools.computer_use import tool as computer_use

    backend = AsyncMock()
    backend.stop.side_effect = RuntimeError("driver teardown failed")
    computer_use._backends["failed-run"] = backend
    computer_use._backend_call_locks["failed-run"] = asyncio.Lock()
    computer_use._session_auto_approve["failed-run"] = True
    assert await computer_use.release_computer_use_session("failed-run") is True
...
@pytest.mark.asyncio
async def test_release_seam_waits_for_in_flight_action_before_stopping_backend():
    from tools.computer_use import tool as computer_use
    ...
    released = asyncio.create_task(
        computer_use.release_computer_use_session("cancelled-run")
    )
    await asyncio.sleep(0)
    assert released.done() is False
    backend.stop.assert_not_awaited()
    call_lock.release()
    assert await released is True
    backend.stop.assert_awaited_once_with()


@pytest.mark.asyncio
async def test_concurrent_hermes_sessions_do_not_share_backend_state():
    from tools.computer_use import tool as computer_use
    ...
    class _Backend:
        ...
        async def typed_browser_state(self, **kwargs):
            return {"marker": self.marker, "pid": kwargs.get("pid")}

    async def invoke(session_id):
        return json.loads(
            await computer_use.handle_computer_use(
    ...
    with patch("tools.computer_use.cua_backend.CuaDriverBackend", _Backend):
        markers = await asyncio.gather(
            *(invoke(sid) for sid in ["conversation-a", "conversation-b"] * 4)
        )
...
@pytest.mark.asyncio
async def test_persistent_focus_has_a_separate_approval_scope():
    from tools.computer_use import tool as computer_use
...
async def _bind_and_snapshot(route) -> str:
    bound = await route.observe(pid=101, window_id=202)
    assert bound["exact_binding"] is True
    snapshot = await route.observe(tab_id="opaque-tab")
    assert snapshot["fresh_state"] is True
    return next(iter(route.state.refs))


@pytest.mark.asyncio
async def test_exact_browser_binding_injects_hermes_session_capability():
    driver = _BrowserDriver()
    payload = await _browser_route(
        driver, session_id="hermes-owned-session"
    ).observe(pid=101, window_id=202)
...
@pytest.mark.asyncio
async def test_browser_mutation_requires_driver_granted_mutation_capability():
    driver = _BrowserDriver(mutation_allowed=False)
    route = _browser_route(driver)
    await route.observe(pid=101, window_id=202)
    result = await route.mutate(
        "browser_navigate",
        tab_id="opaque-tab",
...
@pytest.mark.asyncio
async def test_browser_bind_requires_fresh_tab_state_before_first_mutation():
    driver = _BrowserDriver()
    route = _browser_route(driver)
    await route.observe(pid=101, window_id=202)
    result = await route.mutate(
...
@pytest.mark.asyncio
async def test_browser_mutation_enforces_current_ref_and_fresh_verification():
    driver = _BrowserDriver()
    route = _browser_route(driver)
    current_ref = await _bind_and_snapshot(route)
    stale = await route.mutate(
        "browser_click", tab_id="opaque-tab", args={"ref": "p0:stale"}
    )
    assert stale["code"] == "browser_ref_stale"
    first = await route.mutate(
        "browser_click", tab_id="opaque-tab", args={"ref": current_ref}
    )
    assert first["next_step"] == "fresh_browser_state"
    assert first["verification_required"] is True
    chained = await route.mutate(
        "browser_navigate",
        tab_id="opaque-tab",
        args={"url": "about:blank"},
    )
    assert chained["code"] == "browser_verification_required"
    fresh_ref = next(iter((await route.observe(tab_id="opaque-tab"))["refs"]))
    second = await route.mutate(
        "browser_type",
        tab_id="opaque-tab",
...
@pytest.mark.asyncio
async def test_dom_event_is_forwarded_only_when_explicitly_requested():
    driver = _BrowserDriver()
    route = _browser_route(driver)
    current_ref = await _bind_and_snapshot(route)
    result = await route.mutate(
...
@pytest.mark.asyncio
async def test_trust_route_is_rejected_for_tools_without_a_live_route_property():
    driver = _BrowserDriver()
    route = _browser_route(driver)
    current_ref = await _bind_and_snapshot(route)
    result = await route.mutate(
...
@pytest.mark.asyncio
async def test_scope_ref_must_come_from_this_routes_latest_snapshot():
    driver = _BrowserDriver()
    route = _browser_route(driver)
    await _bind_and_snapshot(route)
    result = await route.observe(tab_id="opaque-tab", scope_ref="other-session:1")
    assert result["code"] == "browser_ref_stale"
    assert len(driver.calls) == 2


@pytest.mark.asyncio
async def test_typed_browser_refs_do_not_cross_route_sessions():
    driver = _BrowserDriver()
    first = _browser_route(driver, session_id="hermes-a")
    second = _browser_route(driver, session_id="hermes-b")
    first_ref = await _bind_and_snapshot(first)
    await _bind_and_snapshot(second)
    result = await second.mutate(
        "browser_click", tab_id="opaque-tab", args={"ref": first_ref}
    )
    assert result["code"] == "browser_ref_stale"


@pytest.mark.asyncio
async def test_trusted_browser_refusal_does_not_silently_change_route():
    driver = _BrowserDriver()
    ...
    calls = [args for name, args in driver.calls if name == "browser_click"]
    assert len(calls) == 1
    assert calls[0].get("input_route") is None
...
@pytest.mark.asyncio
async def test_typed_mutation_disarms_refs_before_transport_failure():
    driver = _BrowserDriver()
    route = _browser_route(driver)
    current_ref = await _bind_and_snapshot(route)

    async def fail_transport(name, args):
...
@pytest.mark.asyncio
async def test_read_only_dialog_inspection_does_not_invalidate_page_state():
    driver = _BrowserDriver()
    route = _browser_route(driver)
    current_ref = await _bind_and_snapshot(route)
    inspected = await route.mutate(
        "browser_dialog", tab_id="opaque-tab", args={"action": "inspect"}
    )
...
@pytest.mark.asyncio
async def test_missing_typed_browser_tool_returns_native_fallback_refusal():
    from tools.computer_use.browser_route import CuaTypedBrowserRoute

    call = AsyncMock()
    route = CuaTypedBrowserRoute(
        session_id="hermes-a", call_tool=call, has_tool=lambda name: False
    )
    result = await route.observe(pid=101, window_id=202)
...
@pytest.mark.asyncio
async def test_existing_profile_prepare_delegates_to_driver_permission_mode():
    driver = _BrowserDriver()
...
@pytest.mark.asyncio
async def test_namespaced_state_and_prepare_actions_use_typed_backend_wrappers():
    from tools.computer_use.tool import _dispatch

    backend = Mock()
    backend.typed_browser_state = AsyncMock(return_value={"status": "ok"})
    backend.typed_browser_prepare = AsyncMock(return_value={"status": "ok"})
    await _dispatch(backend, "cua_browser_state", {"pid": 101, "window_id": 202})
    await _dispatch(
        backend,
        "cua_browser_prepare",
        {
    ...
    backend.typed_browser_state.assert_awaited_once_with(pid=101, window_id=202)
    backend.typed_browser_prepare.assert_awaited_once_with(
        pid=101,
        window_id=202,
        profile_mode="isolated_new",
        profile_name=None,
...
@pytest.mark.asyncio
async def test_namespaced_outer_browser_actions_map_to_exact_driver_tools(
    outer_action, driver_tool, args
):
    from tools.computer_use.tool import _dispatch

    backend = Mock()
    backend.typed_browser_action = AsyncMock(return_value={"status": "ok"})
    await _dispatch(backend, outer_action, {"tab_id": "opaque-tab", **args})
    backend.typed_browser_action.assert_awaited_once_with(
        driver_tool, tab_id="opaque-tab", args=args
    )
...
def test_driver_verdict_fields_are_preserved_and_surfaced_additively():
    ...
    assert json.loads(_text_response(ActionResult(ok=True, action="click"))) == {
        "ok": True,
        "action": "click",
...
@pytest.mark.asyncio
async def test_call_tool_restarts_a_dead_session():
    from tools.computer_use.cua_backend import _CuaDriverSession

    session = _CuaDriverSession.__new__(_CuaDriverSession)
    session._started = False
    session._call_lock = asyncio.Lock()
    starts = []
    ...
    await session.call_tool("click", {"pid": 1})
    assert starts == [True]
