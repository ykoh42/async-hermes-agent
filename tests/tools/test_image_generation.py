...
class TestManagedGatewayErrorTranslation:
    ...
    @pytest.mark.asyncio
    async def test_4xx_translates_to_value_error_with_remediation(self, image_tool, monkeypatch):
        ...
        monkeypatch.setattr(image_tool, "_load_fal_client", MagicMock())
        ...
        mock_managed_client.submit = AsyncMock(side_effect=bad_request)
        mock_managed_client.close = AsyncMock()
        monkeypatch.setattr(
            image_tool, "_ManagedFalClient", MagicMock(return_value=mock_managed_client)
        )
        monkeypatch.setattr(
            image_tool,
        ...
        with pytest.raises(ValueError) as exc_info:
            await image_tool._submit_fal_request(
                "fal-ai/nano-banana-pro", {"prompt": "x"}
            )
    ...
    @pytest.mark.asyncio
    async def test_non_http_exception_from_managed_bubbles_up(self, image_tool, monkeypatch):
        ...
        mock_managed_client = MagicMock()
        mock_managed_client.submit = AsyncMock(side_effect=conn_error)
        mock_managed_client.close = AsyncMock()
        monkeypatch.setattr(
            image_tool, "_ManagedFalClient", MagicMock(return_value=mock_managed_client)
        )
...
class TestManagedKreaRouting:
    ...
    async def test_no_route_when_model_not_krea(self, image_tool, monkeypatch):
        monkeypatch.setattr(
            image_tool, "_read_configured_image_provider", AsyncMock(return_value=None)
        )
        monkeypatch.setattr(
            image_tool,
    ...
    @pytest.mark.asyncio
    async def test_routes_native_krea_model_to_krea_plugin_in_managed_mode(
        ...
        fake_provider.generate = AsyncMock(
            return_value={"success": True, "image": "/tmp/x.png"}
        )
        monkeypatch.setattr(
            "agent.image_gen_registry.get_provider", lambda name: fake_provider
        )
        monkeypatch.setattr(
            "hermes_cli.plugins._ensure_plugins_discovered", AsyncMock(return_value=None)
        )
