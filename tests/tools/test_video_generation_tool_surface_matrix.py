...
async def matrix_env(tmp_path, monkeypatch):
...
async def _invoke_tool(home, cfg: dict, args: dict, tool_name: str = "video_generate") -> dict:
...
@pytest.mark.asyncio
async def test_fal_text_only_routes_to_text_endpoint(matrix_env, family_id):
...
@pytest.mark.asyncio
async def test_xai_text_only_via_tool_surface(matrix_env):
...
@pytest.mark.asyncio
async def test_tool_model_arg_overrides_config(matrix_env):
...
@pytest.mark.asyncio
async def test_tool_model_arg_with_image_url_routes_to_override_image_endpoint(matrix_env):
