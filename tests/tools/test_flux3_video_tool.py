...
def _fake_download(body, status_code=200):
    ...
    Patched at `create_ssrf_safe_client` rather than at httpx, which both
    ...
    async def _create_client(**_kw):
        return _Client()

    with patch.object(url_safety, "create_ssrf_safe_client", _create_client):
        yield fetched
...
async def _call(handler, args, response, headers=None):
...
class TestGating:
    async def test_hidden_without_a_reachable_mount(self):
        with patch.object(flux3, "managed_vendor_endpoints", return_value=None):
            assert await flux3.check_bfl_requirements() is False

    async def test_visible_to_any_signed_in_account_whatever_its_entitlement(self):
        ...
        with patch.object(flux3, "peek_nous_access_token", return_value="nous-token"), \
            ...
            assert await flux3.check_bfl_requirements() is True

    async def test_hidden_without_a_nous_credential(self):
    ...
    async def test_a_profile_sees_a_credential_held_at_the_global_root(self, tmp_path, monkeypatch):
        ...
        assert await flux3.peek_nous_access_token() is None
        assert await flux3.check_bfl_requirements() is True

    async def test_the_credential_probe_never_forces_a_token_refresh(self, monkeypatch):
...
class TestSubmitTransport:
    ...
    async def test_each_generate_tool_sends_its_own_mode(self):
        for handler, args, mode in [
            ...
            _parsed, requests = await _call(handler, args, _FakeResponse(200, {"id": "j", "status": "submitted", "guidance": "ok"}))
            assert requests[0]["json"]["mode"] == mode

    async def test_urls_pass_through_without_an_upload(self):
        # Forwarding a URL is cheaper than downloading and re-uploading it.
        _parsed, requests = await _call(
    ...
    async def test_a_refusal_is_surfaced_as_the_tools_result_text(self):
        ...
        parsed, _requests = await _call(flux3._handle_text_to_video, {"prompt": "a"}, response)
    ...
    async def test_a_401_asks_for_a_nous_sign_in(self):
        parsed, _requests = await _call(flux3._handle_text_to_video, {"prompt": "a"}, _FakeResponse(401, {"error": {"code": "AUTH_ERROR"}}))

        assert parsed["needs_reauth"] is True
        assert "sign in" in parsed["error"].lower()

    async def test_missing_credentials_ask_for_a_sign_in_without_calling_out(self):
        parsed, requests = await _call(flux3._handle_text_to_video, {"prompt": "a"}, _FakeResponse(200, {}), headers={})

        assert requests == []
        assert "sign in" in parsed["error"].lower()

    async def test_a_transport_failure_reports_the_cause(self):
        parsed, _requests = await _call(
            flux3._handle_text_to_video,
            {"prompt": "a"},
    ...
    async def test_an_unreadable_body_does_not_masquerade_as_success(self):
        parsed, _requests = await _call(flux3._handle_text_to_video, {"prompt": "a"}, _FakeResponse(502, None, text="upstream exploded"))
...
class TestPollTransport:
    async def test_a_terminal_status_returns_at_once_without_waiting(self):
        response = _FakeResponse(200, {"id": "bfl_job_1", "status": "Error", "guidance": "The job is over."})

        parsed, requests = await _call(flux3._handle_get_result, {"id": "bfl_job_1"}, response)
    ...
    async def test_a_running_job_is_waited_out_inside_the_call(self, monkeypatch):
        ...
        with patch.object(flux3.asyncio, "sleep", new=_record_sleep(slept)):
            parsed, requests = await _call(flux3._handle_get_result, {"id": "bfl_job_1"}, running)

        assert len(requests) == 2, "should look again after waiting"
        assert sum(slept) == 45.0
        assert parsed["details"]["status"] == "Generating"

    async def test_the_loop_keeps_looking_until_its_budget_is_spent(self, monkeypatch):
    ...
    async def test_the_wait_is_answerable_to_a_stop(self, monkeypatch):
    ...
    async def test_the_call_returns_as_soon_as_the_job_finishes(self):
        ...
        parsed, requests = await _call(flux3._handle_get_result, {"id": "bfl_job_1"}, [running, done])

        assert len(requests) == 2
        assert parsed["result"] == "That job failed."

    async def test_a_refusal_without_a_stated_wait_is_returned_immediately(self):
    ...
    async def test_a_throttle_is_waited_out_inside_the_call(self, monkeypatch):
        ...
        slept = []
        with patch.object(flux3.asyncio, "sleep", new=_record_sleep(slept)):
            parsed, requests = await _call(flux3._handle_get_result, {"id": "bfl_job_1"}, [throttled, done])
    ...
    async def test_a_throttle_never_polls_faster_than_the_loop_s_own_cadence(self, monkeypatch):
        ...
        slept = []
        with patch.object(flux3.asyncio, "sleep", new=_record_sleep(slept)):
            _parsed, requests = await _call(flux3._handle_get_result, {"id": "bfl_job_1"}, [throttled, done])

        assert len(requests) == 2
        assert sum(slept) == 10.0, "the loop's own gap, not the sliver it was offered"

    async def test_a_slow_poll_spends_the_budget_it_actually_took(self, monkeypatch):
        ...
        _parsed, requests = await _call(flux3._handle_get_result, {"id": "bfl_job_1"}, running)

        assert len(requests) == 2, "two twenty-second looks exhaust a thirty-second budget"

    async def test_a_poll_outwaits_the_gateways_own_poll_budget(self):
    ...
    async def test_a_blip_costs_a_look_rather_than_the_rest_of_the_call(self, monkeypatch):
    ...
    async def test_a_gateway_answering_in_html_counts_as_unreachable(self, monkeypatch):
    ...
    async def test_a_gateway_that_stays_down_is_reported_rather_than_retried_out(self, monkeypatch):
    ...
    async def test_the_tolerance_counts_consecutive_failures_only(self, monkeypatch):
    ...
    async def test_a_throttle_longer_than_the_budget_is_handed_back(self, monkeypatch):
    ...
    async def test_the_backstop_answers_rather_than_letting_the_bridge_kill_the_call(self, monkeypatch):
        ...
        parsed, _requests = await _call(flux3._handle_get_result, {"id": "bfl_job_1"}, _FakeResponse(200, {}))
    ...
    async def test_a_poll_does_not_inherit_the_submit_read_timeout(self):
        ...
        with patch.object(flux3, "managed_gateway_auth_headers", return_value={"Authorization": "Bearer t"}), \
            ...
            await flux3._handle_get_result({"id": "j"})
            await flux3._handle_text_to_video({"prompt": "a"})
    ...
    async def test_the_pacing_stays_clear_of_the_agents_per_tool_ceiling(self):
    ...
    async def test_download_timeout_never_outlives_the_backstop(self):
    ...
    async def test_ready_saves_the_clip_and_never_returns_the_signed_url(self, tmp_path):
    ...
    async def test_ready_never_overwrites_an_existing_file(self, tmp_path):
        ...
        with _fake_download(b"y" * (128 * 1024)):
            parsed, _requests = await _call(flux3._handle_get_result, {"id": "bfl_job_1", "save_to": str(tmp_path)}, response)
    ...
    async def test_on_messaging_the_clip_lands_where_the_gateway_may_send_it(self, monkeypatch):
        ...
        try:
            with _fake_download(b"x" * (128 * 1024)):
                parsed, _requests = await _call(flux3._handle_get_result, {"id": "bfl_job_1"}, response)
        finally:
        ...
        assert Path(saved).parent == await get_hermes_dir("cache/videos", "video_cache")
        assert Path(saved).is_file()
        # The exact line to copy, so the path is never retyped from memory.
        assert f"\nMEDIA:{saved}\n" in parsed["result"]

    async def test_the_offered_tag_is_one_the_gateway_actually_delivers(self, monkeypatch):
        ...
        assert offered[0] == f"MEDIA:{parsed['details']['saved_path']}"

    @pytest.mark.parametrize("platform", ["", "cli", "tui", "desktop"])
    async def test_off_messaging_the_clip_stays_a_file_and_no_tag_is_offered(self, tmp_path, monkeypatch, platform):
    ...
    async def test_platforms_without_an_attachment_channel_are_offered_no_tag(self, tmp_path, monkeypatch, platform):
    ...
    async def test_a_cli_session_is_recognised_by_its_source(self, tmp_path, monkeypatch):
    ...
    async def test_a_rejected_download_fails_loudly_and_leaves_no_file(self, tmp_path):
    ...
    async def test_poll_requires_an_id(self):
        parsed, requests = await _call(flux3._handle_get_result, {}, _FakeResponse(200, {}))

        assert "id is required" in parsed["error"]
        assert requests == []

    async def test_poll_url_encodes_the_job_id(self):
        _parsed, requests = await _call(
            flux3._handle_get_result,
            {"id": "weird/../id"},
...
class TestMediaDelivery:
    ...
    async def test_a_local_path_is_uploaded_and_replaced_with_a_reference(self):
    ...
    async def test_video_fields_permit_video_only(self):
        async def fake_uploader(data, mime):
    ...
    async def test_every_keyframe_path_is_uploaded(self):
        uploads = []
    ...
    async def test_a_list_valued_input_image_is_still_uploaded(self):
    ...
    async def test_media_fields_are_sanitized_whatever_the_mode_expects(self):
    ...
    async def test_text_to_video_strips_media_fields_instead_of_uploading_them(self):
    ...
    async def test_an_over_long_image_list_is_refused_before_any_upload(self):
        def _must_not_upload(*_args, **_kwargs):
    ...
    async def test_an_upload_refusal_becomes_the_tools_error(self):
    ...
    async def test_an_unreadable_file_is_reported_without_dumping_the_value(self):
...
class TestLocalPathDetection:
    ...
    async def test_rooted_paths_are_read_off_disk(self, value):
    ...
    async def test_ambiguous_and_remote_values_are_forwarded(self, value):
        assert flux3._looks_like_local_path(value) is False

    async def test_a_short_base64_lookalike_path_is_still_a_path(self):
        assert flux3._looks_like_local_path("/tmp/frames/a1") is True


class TestSchemas:
    async def test_every_tool_is_registered_under_the_bfl_toolset(self):
    ...
    async def test_generate_tools_point_at_the_guide_and_the_poll_tool(self):
    ...
    async def test_the_guide_covers_the_methodology_without_pinning_server_policy(self):
    ...
    async def test_the_guide_tool_takes_no_arguments_and_calls_nothing(self):
        assert flux3.PROMPTING_GUIDE_SCHEMA["parameters"]["properties"] == {}
        assert await flux3._handle_prompting_guide({}) == flux3.FLUX3_PROMPTING_GUIDE
