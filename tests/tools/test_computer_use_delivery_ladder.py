...
async def test_suspected_noop_carries_escalation():
    out = {
        "isError": False, "data": {},
        "structuredContent": {
...
async def test_degraded_capture_signal_preserved():
    ...
    be = _make_backend(_FakeSession(out))
    res = await be.scroll(direction="down", element=1)
    assert res.degraded is True
    assert res.escalation["recommended"] == "px"


async def test_old_driver_without_structured_content_is_clean():
...
async def test_text_response_surfaces_fields_additively():
...
async def test_foreground_refused_on_old_driver():
    ...
    sess = _FakeSession(out)
    be = _make_backend(sess)
    res = await be.click(element=1, delivery_mode="foreground")
...
async def test_bad_delivery_mode_rejected():
    out = {"isError": False, "data": {}, "structuredContent": {}}
    sess = _FakeSession(out, input_properties={"type_text": {"delivery_mode"}})
    be = _make_backend(sess)
    res = await be.type_text("hi", delivery_mode="sideways")
    assert res.ok is False
    assert res.code == "bad_delivery_mode"


async def test_dispatcher_threads_delivery_mode_to_backend():
    """End-to-end through the tool dispatcher with the noop backend."""
    from tools.computer_use import tool as cu
    with patch.dict(os.environ, {"HERMES_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        await cu.reset_backend_for_tests()
        be = await cu._get_backend()
        await cu.handle_computer_use({"action": "click", "element": 5,
...
async def test_background_approval_does_not_authorize_foreground():
    ...
    try:
        ...
        assert await cu._request_approval("click", {}, "sess-A") is None
        ...
        assert await cu._request_approval("click", {"delivery_mode": "foreground"}, "sess-A") is None
        assert len(seen) == 2
...
async def test_approval_state_is_session_scoped():
    from tools.computer_use import tool as cu
    ...
    try:
        ...
        await cu._request_approval("click", {"delivery_mode": "foreground"}, "run-A")
        # Run B has NOT — it must prompt independently.
        n_before = len(calls)
        await cu._request_approval("click", {"delivery_mode": "foreground"}, "run-B")
        assert len(calls) == n_before + 1
    finally:
        cu.set_approval_callback(None)


async def test_always_approve_covers_foreground():
    from tools.computer_use import tool as cu
    ...
    try:
        ...
        await cu._request_approval("click", {}, "run-C")
        # Foreground now sails through without another prompt.
        await cu._request_approval("click", {"delivery_mode": "foreground"}, "run-C")
        assert len(calls) == 1
    finally:
        cu.set_approval_callback(None)


async def test_foreground_summary_warns_about_focus_change():
...
async def test_lifecycle_finally_resets_started_for_reentry():
...
async def test_call_tool_restarts_a_dead_session(monkeypatch):
    """A dead native-async session is restarted once before the tool call."""
    from tools.computer_use.cua_backend import _CuaDriverSession

    sess = _CuaDriverSession()
    started = {"count": 0}
    ...
    async def fake_direct(name, args, timeout):
        del name, args, timeout
        return {"isError": False, "data": {}, "structuredContent": {}}

    monkeypatch.setattr(sess, "start", fake_start)
    monkeypatch.setattr(sess, "_call_tool_direct", fake_direct)

    result = await sess.call_tool("click", {"pid": 1})
