...
async def _empty_async_handler(args, **kwargs):
    return "{}"
...
class TestAssembly:
    ...
    def _register_mcp(name):
        ...
        async def _handler(args, task_id=None, **kw):
    ...
    @pytest.mark.asyncio
    async def test_idempotent_when_bridge_already_present(self):
...
class TestBridgeDispatch:
    @pytest.mark.asyncio
    async def test_tool_search_requires_query(self):
        from tools.tool_search import dispatch_tool_search
        result = await dispatch_tool_search({}, current_tool_defs=[])
        assert "error" in json.loads(result)

    @pytest.mark.asyncio
    async def test_empty_search_keeps_connected_sources_discoverable(self):
        from tools.registry import registry
        from tools.tool_search import dispatch_tool_search
        ...
        registry.register(
            name=name,
            handler=_empty_async_handler,
            schema=tool_def,
            toolset="mcp-recovery-catalog",
        )

        result = json.loads(await dispatch_tool_search(
...
class TestHandleFunctionCallIntegration:
    @pytest.mark.asyncio
    async def test_tool_search_dispatch_through_handle_function_call(self):
...
class TestRegression_OpenClawCron84141:
    ...
    @pytest.mark.asyncio
    async def test_core_tool_survives_alongside_many_mcp_tools(self):
...
class TestRegression_ToolsetScoping:
    ...
    @pytest.mark.asyncio
    async def test_search_catalog_is_scoped_to_session_toolsets(self):
        import model_tools
    ...
    @pytest.mark.asyncio
    async def test_scoped_deferrable_names_helper(self):
        from tools.tool_search import scoped_deferrable_names

        self._register("mcp_helper_op", "mcp-helper")
        import model_tools
        defs = await model_tools.get_tool_definitions(
...
class TestCatalogListing:
    ...
    @pytest.mark.asyncio
    async def test_default_listing_cap_bounds_fixed_catalog_overhead(self):
        ...
        cfg = ToolSearchConfig.from_raw(None)
        result = await assemble_tool_defs(
            defs, context_length=1_000_000, config=cfg
        )
        search = next(
    ...
    @pytest.mark.asyncio
    async def test_assembly_listing_off_keeps_legacy_description(self):
        from tools.tool_search import assemble_tool_defs, ToolSearchConfig
        for i in range(30):
...
class TestDeferredCallSchemaProbe:
    ...
    @pytest.mark.asyncio
    async def test_valid_tool_call_still_dispatches(self):
        import model_tools

        self._register("mcp_probe_valid_op", "mcp-probe-valid")
        result = json.loads(await model_tools.handle_function_call(
