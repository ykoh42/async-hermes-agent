...
class TestResolveActiveContextLengthProviderAware:
    @pytest.mark.asyncio
    async def test_passes_provider_base_url_and_key_from_runtime(self):
        """Resolved runtime credentials must reach get_model_context_length."""
        import model_tools

        captured = {}

        async def fake_get_ctx(model_id, base_url="", api_key="", config_context_length=None, provider=""):
            captured.update(
        ...
        with patch("hermes_cli.config.load_config_readonly", new=AsyncMock(return_value=_model_cfg())), \
             patch("hermes_cli.runtime_provider.resolve_runtime_provider",
                   new=AsyncMock(return_value={"base_url": "https://chatgpt.com/backend-api/codex",
                                               "api_key": "tok-live"})) as mock_rt, \
             patch("agent.model_metadata.get_model_context_length", side_effect=fake_get_ctx):
            ctx = await model_tools._resolve_active_context_length()
    ...
    @pytest.mark.asyncio
    async def test_offline_credential_failure_degrades_to_config_values(self):
        ...
        with patch("hermes_cli.config.load_config_readonly",
                   new=AsyncMock(return_value=_model_cfg(base_url="https://chatgpt.com/backend-api/codex"))), \
             patch("hermes_cli.runtime_provider.resolve_runtime_provider",
    ...
    @pytest.mark.asyncio
    async def test_no_provider_configured_skips_runtime_resolution(self):
        ...
        with patch("hermes_cli.config.load_config_readonly",
                   new=AsyncMock(return_value={"model": {"model": "some-model"}})), \
    ...
    @pytest.mark.asyncio
    async def test_config_context_length_still_short_circuits(self):
        ...
        with patch("hermes_cli.config.load_config_readonly",
                   new=AsyncMock(return_value=_model_cfg(context_length=150_000))), \
             patch("hermes_cli.runtime_provider.resolve_runtime_provider",
                   new=AsyncMock(return_value={"base_url": "https://chatgpt.com/backend-api/codex",
                                               "api_key": "tok"})), \
             patch("agent.model_metadata.get_model_context_length", side_effect=fake_get_ctx):
