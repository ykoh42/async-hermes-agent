...
class TestIngestWindows:
    async def test_skips_window_with_null_pid(self):
    ...
    async def test_preserves_fields_capture_relies_on(self):
        from tools.computer_use.cua_backend import _ingest_windows
...
def _backend_with_windows(raw_windows):
    ...
    async def _call_tool(name, args, *a, **k):
...
async def test_capture_vision_survives_null_pid_window():
    raw = [
        {"app_name": "Desktop", "pid": None, "window_id": 1, "z_index": 0},
    ...
    cap = await backend.capture(mode="vision")
