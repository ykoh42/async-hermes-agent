...
class TestApprovalHookSessionId:
    async def test_session_id_forwarded_when_bound(self):
        captured = []
        ...
        try:
            with patch(
                ...
                await approval_mod._fire_approval_hook(
    ...
    async def test_explicit_session_id_not_clobbered(self):
        captured = []
        tokens = approval_mod.set_current_observability_context(
    ...
    async def test_absent_when_unbound(self):
        captured = []
        with patch(
            "hermes_cli.lifecycle.invoke_hook",
            side_effect=_capture_hook(captured),
