...
@pytest.mark.asyncio
async def test_ensure_tab_sends_list_item_id():
    ...
    with (
        patch.object(mod, "_get_session", new=AsyncMock(return_value=fake_session)),
        patch.object(mod, "get_camofox_url", return_value="http://localhost:9377"),
        patch("tools.browser_camofox.httpx.AsyncClient") as client_cls,
    ):
        client = client_cls.return_value.__aenter__.return_value
        client.post = AsyncMock(return_value=mock_response)
        result = await mod._ensure_tab("test-task", url="https://example.com")
...
@pytest.mark.asyncio
async def test_ensure_tab_skips_creation_when_tab_exists():
    ...
    with (
        ...
        result = await mod._ensure_tab("test-task")
