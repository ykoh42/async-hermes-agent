...
async def test_parse_xprop_net_active_window_standard_output():
...
async def test_default_capture_prefers_x11_active_window_when_z_index_tied():
    ...
    with patch("tools.computer_use.cua_backend.sys.platform", "linux"), patch(
        ...
        new=AsyncMock(return_value=84043449),
    ):
        target = await _select_capture_target(windows, app_requested=False)

    assert target["title"] == "xr@10:~/hermes"
    assert target["window_id"] == 84043449


async def test_default_capture_skips_desktop_helper_when_active_window_unknown():
...
async def test_linux_null_is_on_screen_is_treated_as_unknown_not_offscreen():
...
async def test_explicit_app_capture_preserves_filtered_target_order():
    """When the caller filters first, target selection should not skip the match."""
    from tools.computer_use.cua_backend import _select_capture_target

    chrome = _normalized_windows(LINUX_LIST_WINDOWS)[1]

    assert await _select_capture_target([chrome], app_requested=True) == chrome
