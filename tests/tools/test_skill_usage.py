...
def skills_home(tmp_path, monkeypatch):
    ...
    async def prune_builtins_disabled():
        return False

    monkeypatch.setattr(mod, "_prune_builtins_enabled", prune_builtins_disabled)
    return home
...
async def test_empty_usage_returns_empty_dict(skills_home):
    from tools.skill_usage import load_usage
    assert await load_usage() == {}


async def test_save_and_load_roundtrip(skills_home):
    from tools.skill_usage import load_usage, save_usage
    data = {"skill-a": {"use_count": 3, "state": "active"}}
    await save_usage(data)
    loaded = await load_usage()
    assert loaded["skill-a"]["use_count"] == 3
    assert loaded["skill-a"]["state"] == "active"


async def test_get_record_missing_returns_empty_record(skills_home):
    from tools.skill_usage import get_record
    rec = await get_record("nonexistent")
...
async def test_load_usage_handles_corrupt_file(skills_home):
...
async def test_bump_view_increments_and_timestamps(skills_home):
    from tools.skill_usage import bump_view, get_record
    await bump_view("my-skill")
    await bump_view("my-skill")
    rec = await get_record("my-skill")
    assert rec["view_count"] == 2
    assert rec["last_viewed_at"] is not None


async def test_bumps_do_not_corrupt_other_skills(skills_home):
    from tools.skill_usage import bump_view, bump_use, get_record
    await bump_view("skill-a")
    await bump_use("skill-b")
    await bump_view("skill-a")
    assert (await get_record("skill-a"))["view_count"] == 2
    assert (await get_record("skill-a"))["use_count"] == 0
    assert (await get_record("skill-b"))["use_count"] == 1


async def test_concurrent_bump_view_preserves_all_updates(skills_home):
    from tools.skill_usage import get_record
    ...
    await asyncio.gather(
        *(
            bump_view("shared-skill")
            for _ in range(task_count * iterations)
        )
    )
    assert (await get_record("shared-skill"))["view_count"] == task_count * iterations
...
async def test_set_state_active(skills_home):
    from tools.skill_usage import set_state, get_record, STATE_ACTIVE
    await set_state("x", STATE_ACTIVE)
    assert (await get_record("x"))["state"] == "active"


async def test_restoring_from_archive_clears_timestamp(skills_home):
    from tools.skill_usage import set_state, get_record, STATE_ARCHIVED, STATE_ACTIVE
    await set_state("x", STATE_ARCHIVED)
    assert (await get_record("x"))["archived_at"] is not None
    await set_state("x", STATE_ACTIVE)
    assert (await get_record("x"))["archived_at"] is None


async def test_forget_removes_record(skills_home):
    from tools.skill_usage import bump_view, forget, load_usage
    await bump_view("x")
    assert "x" in await load_usage()
    await forget("x")
    assert "x" not in await load_usage()
...
async def test_agent_created_excludes_bundled(skills_home):
...
async def test_is_agent_created(skills_home):
    ...
    assert await is_agent_created("my-skill") is True
    assert await is_agent_created("bundled") is False
    assert await is_agent_created("hubbed") is False
...
async def test_end_to_end_telemetry_tracked_but_lifecycle_refused(skills_home):
    ...
    for name in ("bundled-one", "hub-one"):
        await bump_view(name)
        await bump_use(name)
        await bump_patch(name)
        await set_state(name, STATE_STALE)
        await set_state(name, STATE_ARCHIVED)
        await set_pinned(name, True)
        ok, _msg = await archive_skill(name)
        assert not ok, f"await archive_skill(\"{name}\") should refuse"
    ...
    await bump_view("mine")
    assert (await load_usage())["mine"]["view_count"] == 1
...
async def test_adopt_preserves_the_inactivity_clock(skills_home):
    ...
    before = latest_activity_at(await get_record("legacy"))

    ok, _msg = await adopt_skill("legacy")
    assert ok is True
    rec = await get_record("legacy")
...
async def test_adopt_refuses_skills_the_user_does_not_own(skills_home, monkeypatch, kind):
    ...
    async def prune_builtins_enabled():
        return True

    monkeypatch.setattr(
        skill_usage, "_prune_builtins_enabled", prune_builtins_enabled
    )
    ...
    ok, _msg = await adopt_skill(name)
    assert ok is False
    assert (await load_usage()).get(name, {}).get("created_by") != "agent"


async def test_adopt_rejects_empty_name(skills_home):
    from tools.skill_usage import adopt_skill

    assert (await adopt_skill(""))[0] is False
