...
class TestBrowserConsole:
    ...
    async def test_returns_console_messages_and_errors(self):
        ...
        with patch("tools.browser_tool._run_browser_command", new_callable=AsyncMock) as mock_cmd:
            mock_cmd.side_effect = [console_response, errors_response]
            result = json.loads(await browser_console(task_id="test"))
    ...
    async def test_passes_clear_flag(self):
        ...
        with patch("tools.browser_tool._run_browser_command", new_callable=AsyncMock, return_value=empty) as mock_cmd:
            await browser_console(clear=True, task_id="test")
    ...
    async def test_redacts_secrets_from_console_messages_and_errors(self):
        from tools.browser_tool import browser_console
    ...
    async def test_redacts_secrets_from_eval_result(self):
        ...
        with patch("tools.browser_tool._last_session_key", return_value="test"), \
             ...
             patch("tools.browser_tool._run_browser_command", new_callable=AsyncMock, return_value={"success": True, "data": {"result": fake_key}}):
            result = json.loads(await _browser_eval("document.body.innerText", task_id="test"))
    ...
    async def test_expression_allows_risky_eval_by_default(self):
        ...
        with patch("tools.browser_tool._browser_eval", new_callable=AsyncMock, return_value=json.dumps({"success": True, "result": "ok"})) as mock_eval:
            for expr in expressions:
                result = json.loads(await browser_console(expression=expr, task_id="test"))
                assert result == {"success": True, "result": "ok"}, expr

        assert mock_eval.await_count == len(expressions)

    async def test_expression_blocks_cookie_access_before_eval(self):
        from tools.browser_tool import browser_console

        with patch("tools.browser_tool._restrict_browser_evaluate", new_callable=AsyncMock, return_value=True), \
             patch("tools.browser_tool._allow_unsafe_browser_evaluate", new_callable=AsyncMock, return_value=False), \
             patch("tools.browser_tool._browser_eval", new_callable=AsyncMock) as mock_eval:
            result = json.loads(await browser_console(expression="document.cookie", task_id="test"))
    ...
    async def test_expression_blocks_storage_and_network_access_before_eval(self):
        from tools.browser_tool import browser_console
    ...
    async def test_restrict_evaluate_reads_browser_config(self):
        from tools.browser_tool import _restrict_browser_evaluate

        with patch("tools.browser_tool.load_config_readonly", new_callable=AsyncMock, return_value={"browser": {"restrict_evaluate": "true"}}):
            assert await _restrict_browser_evaluate() is True
        with patch("tools.browser_tool.load_config_readonly", new_callable=AsyncMock, return_value={"browser": {"restrict_evaluate": False}}):
            assert await _restrict_browser_evaluate() is False
...
class TestBrowserConsoleSchema:
    ...
    async def test_schema_in_browser_schemas(self):
        from tools.browser_tool import BROWSER_TOOL_SCHEMAS

        names = [s["name"] for s in BROWSER_TOOL_SCHEMAS]
        assert "browser_console" in names

    async def test_schema_has_clear_param(self):
        from tools.browser_tool import BROWSER_TOOL_SCHEMAS
...
class TestBrowserConsoleToolsetWiring:
    ...
    async def test_in_browser_toolset(self):
        from toolsets import TOOLSETS
        assert "browser_console" in TOOLSETS["browser"]["tools"]


    async def test_in_registry(self):
...
class TestBrowserVisionAnnotate:
    ...
    async def test_schema_has_annotate_param(self):
        from tools.browser_tool import BROWSER_TOOL_SCHEMAS
    ...
    async def test_annotate_true_adds_flag(self):
        """With annotate=True, screenshot command includes --annotate."""
        from tools.browser_tool import browser_vision

        with (
            patch("tools.browser_tool._run_browser_command", new_callable=AsyncMock) as mock_cmd,
            patch("tools.browser_tool._lazy_call_llm", new_callable=AsyncMock) as mock_call_llm,
            patch("tools.browser_tool._get_vision_model", return_value="test-model"),
        ):
            mock_cmd.return_value = {"success": True, "data": {}}
            try:
                await browser_vision("test", annotate=True, task_id="test")
            except Exception:
                pass
...
class TestBrowserVisionConfig:
    ...
    async def test_browser_vision_uses_configured_temperature_and_timeout(self, tmp_path):
        from tools.browser_tool import browser_vision
        ...
        with (
            ...
            patch("tools.browser_tool._cleanup_old_screenshots", new_callable=AsyncMock),
            patch("tools.browser_tool._run_browser_command", new_callable=AsyncMock, return_value={"success": True, "data": {"path": str(screenshot)}}),
            patch("tools.browser_tool._get_vision_model", return_value="test-model"),
            patch("tools.browser_tool.load_config_readonly", new_callable=AsyncMock, return_value={"auxiliary": {"vision": {"temperature": 1, "timeout": 45}}}),
            patch("tools.browser_tool._lazy_call_llm", new_callable=AsyncMock, return_value=mock_response) as mock_llm,
        ):
            result = json.loads(await browser_vision("what is on the page?", task_id="test"))
    ...
    async def test_browser_vision_native_fast_path_returns_multimodal(self, tmp_path):
        ...
        try:
            with (
                ...
                patch("tools.browser_tool._lazy_call_llm", new_callable=AsyncMock) as mock_llm,
            ):
                result = await browser_vision("what is on the page?", annotate=True, task_id="test")
        finally:
            clear_runtime_main()
    ...
    async def test_browser_vision_text_mode_blocks_native_fast_path(self, tmp_path):
...
class TestRecordSessionsConfig:
    ...
    async def test_default_config_has_record_sessions(self):
    ...
    async def test_maybe_stop_recording_noop_when_not_recording(self):
        ...
        with patch("tools.browser_tool._run_browser_command", new_callable=AsyncMock) as mock_cmd:
            await _maybe_stop_recording("test-task")
