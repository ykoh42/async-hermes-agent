...
class TestCheckLintBracePaths:
    ...
    async def test_normal_path(self, ops):
        ...
        with patch.object(ops, "_has_command", return_value=True), \
            ...
            result = await ops._check_lint("/tmp/test_file.js")
    ...
    async def test_path_with_curly_braces(self, ops):
        """Path containing ``{`` and ``}`` must not raise KeyError/ValueError."""
        with patch.object(ops, "_has_command", return_value=True), \
             patch.object(ops, "_exec") as mock_exec:
            mock_exec.return_value = MagicMock(exit_code=0, stdout="")
            # This would raise KeyError with .format() but works with .replace()
            result = await ops._check_lint("/tmp/{test}_file.js")

        assert result.success is True
        cmd_arg = mock_exec.call_args[0][0]
        assert "{test}" in cmd_arg

    async def test_path_with_nested_braces(self, ops):
        """Path with complex brace patterns like ``{{var}}`` should be safe."""
        with patch.object(ops, "_has_command", return_value=True), \
             patch.object(ops, "_exec") as mock_exec:
            mock_exec.return_value = MagicMock(exit_code=0, stdout="")
            result = await ops._check_lint("/tmp/{{var}}.js")

        assert result.success is True

    async def test_unsupported_extension_skipped(self, ops):
        """Extensions without a linter should return a skipped result."""
        result = await ops._check_lint("/tmp/file.unknown_ext")
        assert result.skipped is True

    async def test_missing_linter_skipped(self, ops):
        """When the linter binary is not installed, skip gracefully."""
        with patch.object(ops, "_has_command", return_value=False):
            result = await ops._check_lint("/tmp/test.js")
        assert result.skipped is True

    async def test_lint_failure_returns_output(self, ops):
        ...
        with patch.object(ops, "_has_command", return_value=True), \
            ...
            result = await ops._check_lint("/tmp/bad.js")
...
class TestCheckLintInproc:
    ...
    async def test_python_inproc_clean(self, ops):
        """Valid Python content passes in-process ast.parse."""
        result = await ops._check_lint("/tmp/ok.py", content="x = 1\n")
        assert result.success is True
        assert not result.skipped
        assert result.output == ""


    async def test_json_inproc_clean(self, ops):
        result = await ops._check_lint("/tmp/a.json", content='{"a": 1}')
        assert result.success is True


    async def test_toml_inproc_error(self, ops):
        result = await ops._check_lint("/tmp/b.toml", content='[section\nk = "v"')
        assert result.success is False
...
class TestCheckLintDelta:
    ...
    async def test_clean_post_no_pre_lint(self, ops):
        ...
        with patch.object(ops, "_check_lint", wraps=ops._check_lint) as wrapped:
            r = await ops._check_lint_delta("/tmp/a.py", pre_content="x = 0\n", post_content="x = 1\n")
            # Post-lint called exactly once (clean), pre-lint never called.
            assert wrapped.call_count == 1
        assert r.success is True


    async def test_pre_existing_remains_flagged_but_not_new(self, ops):
        ...
        pre = 'def a(:\n    pass\n'
        post = 'def a(:\n    pass\n\nprint(42)\n'  # still line 1 broken
        r = await ops._check_lint_delta("/tmp/d.py", pre_content=pre, post_content=post)
...
class TestPaginationBounds:
    ...
    async def test_read_file_clamps_offset_and_limit_before_building_sed_range(self):
        env = MagicMock()
        env.cwd = "/tmp"
        ...
        with patch.object(ops, "_exec", side_effect=fake_exec):
            result = await ops.read_file("notes.txt", offset=0, limit=0)
    ...
    async def test_search_clamps_offset_and_limit_before_building_head_pipeline(self):
        env = MagicMock()
        env.cwd = "/tmp"
        ops = ShellFileOperations(env)
        commands = []
        ...
        with patch.object(ops, "_has_command", side_effect=lambda cmd: cmd == "rg"), \
            ...
            result = await ops.search("*.py", target="files", path=".", offset=-4, limit=-2)
...
class TestSearchContextParsing:
    ...
    @pytest.mark.asyncio
    async def test_search_with_grep_context_handles_filename_with_dash_digits(self):
        env = MagicMock()
        env.cwd = "/tmp"
        ops = ShellFileOperations(env)
