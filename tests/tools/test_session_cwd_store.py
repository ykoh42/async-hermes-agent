...
def test_records_are_keyed_and_cleared_by_session(tmp_path):
...
def test_registered_override_seeds_and_updates_record(tmp_path):
    first = tmp_path / "first"
    second = tmp_path / "second"
    first.mkdir()
    second.mkdir()
...
def test_file_resolution_uses_each_sessions_record(tmp_path, monkeypatch):
    first = tmp_path / "first"
    second = tmp_path / "second"
    first.mkdir()
    second.mkdir()
    ...
    assert file_tools._resolve_path_for_task("f.py", "one") == first / "f.py"
    assert file_tools._resolve_path_for_task("f.py", "two") == second / "f.py"
...
def test_child_record_can_diverge_without_mutating_parent(tmp_path):
    parent = tmp_path / "parent"
    child = tmp_path / "child"
    parent.mkdir()
    child.mkdir()
    terminal.record_session_cwd("parent", str(parent))
    terminal.record_session_cwd("child", terminal.get_session_cwd("parent"))
