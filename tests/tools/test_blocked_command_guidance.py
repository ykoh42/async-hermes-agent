...
class TestParserLimitRecovery:
    async def test_parser_limit_block_saves_payload_and_names_it(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / ".hermes"))
        cmd = "python3 -c '" + "x = 1; " * 900 + "'"
        r = await _hardline_block_result(_PARSER_LIMIT_DESCRIPTION, cmd)
    ...
    async def test_save_failure_falls_back_to_manual_recipe(self, monkeypatch):
        import tools.approval as ap
        async def fail_save(_command):
            return None
        monkeypatch.setattr(ap, "_save_blocked_payload", fail_save)
        r = await _hardline_block_result(_PARSER_LIMIT_DESCRIPTION, "python3 -c 'x'")
        assert "write_file" in r["message"]
        assert "bash /path/script.sh" in r["message"]

    async def test_no_command_falls_back_to_manual_recipe(self):
        r = await _hardline_block_result(_PARSER_LIMIT_DESCRIPTION)
        assert "RECOVERY" in r["message"]
        assert "write_file" in r["message"]

    async def test_malformed_exec_block_has_recovery_recipe(self):
        r = await _hardline_block_result(_MALFORMED_EXEC_DESCRIPTION)
        assert "RECOVERY" in r["message"]

    async def test_real_hardline_blocks_unchanged(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / ".hermes"))
        r = await _hardline_block_result("recursive delete of root filesystem", "rm -rf --no-preserve-root /")
    ...
    async def test_old_saved_payloads_cleaned(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / ".hermes"))
        import os
        ...
        os.utime(stale, (1, 1))
        await _hardline_block_result(_PARSER_LIMIT_DESCRIPTION, "python3 -c 'y'")
