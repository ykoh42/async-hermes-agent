...
class TestExtractRelevantContentNoneGuard:
    ...
    async def test_none_content_falls_back_to_truncated(self, tmp_path, monkeypatch):
        """When LLM returns None content, should fall back to truncated snapshot."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        with patch("tools.browser_tool.call_llm", new=AsyncMock(return_value=_make_response(None))), \
            ...
            result = await _extract_relevant_content(
    ...
    async def test_empty_string_content_falls_back(self, tmp_path, monkeypatch):
        """Empty string content should also fall back to truncated."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        with patch("tools.browser_tool.call_llm", new=AsyncMock(return_value=_make_response("   "))), \
             patch("tools.browser_tool._get_extraction_model", return_value="test-model"):
            from tools.browser_tool import _extract_relevant_content
            result = await _extract_relevant_content(
                "This is a long snapshot text", "task"
            )
