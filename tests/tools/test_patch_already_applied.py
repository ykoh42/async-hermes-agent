...
async def _patch_tool(**kwargs):
    from tools.file_tools import patch_tool
    return json.loads(await patch_tool(**kwargs))


@pytest.mark.asyncio
class TestPatchReplaceAlreadyApplied:
    async def test_identical_old_new_present_is_success_noop(self, workdir):
        f = workdir / "a.py"
        f.write_text("value = compute_total(items)\n")
        r = await _patch_tool(path=str(f), old_string="value = compute_total(items)",
    ...
    async def test_replay_of_landed_edit_is_success_noop(self, workdir):
        ...
        r = await _patch_tool(path=str(f), old_string="TIMEOUT_WINDOW_MS = 9000",
                        new_string="RETRY_LIMIT_SECONDS = 30", task_id="t-applied")
        assert r["success"] is True
        assert r.get("no_change") is True

    async def test_genuine_no_match_still_errors(self, workdir):
        f = workdir / "c.py"
        f.write_text("something_else = 1\n")
        r = await _patch_tool(path=str(f), old_string="def missing_function():",
                        new_string="def replacement_function():", task_id="t-applied")
        assert "error" in r

    async def test_identical_but_absent_still_errors(self, workdir):
        f = workdir / "d.py"
        f.write_text("unrelated = True\n")
        r = await _patch_tool(path=str(f), old_string="def not_here_function():",
                        new_string="def not_here_function():", task_id="t-applied")
        assert "error" in r

    async def test_half_applied_rename_still_errors(self, workdir):
...
class TestV4AAlreadyApplied:
    async def test_already_applied_hunk_skipped_in_multi_hunk_patch(self, workdir):
        f = workdir / "mod.py"
        f.write_text(
        ...
        r = await _patch_tool(mode="patch", patch=patch_content, task_id="t-v4a")
        assert r["success"] is True, r
        text = f.read_text()
        assert "return y + 2" in text            # live hunk applied
        assert "already_renamed_helper" in text  # no-op hunk left intact

    async def test_fully_applied_patch_is_noop_success(self, workdir):
        f = workdir / "done.py"
