...
class TestUtf16Read:
    async def test_utf16le_bom(self, fops, tmp_path):
    ...
    async def test_crlf_normalized(self, fops, tmp_path):
    ...
    async def test_pagination(self, fops, tmp_path):
        text = "\ufeff" + "\n".join(f"line{i}" for i in range(1, 21)) + "\n"
        path = str(tmp_path / "paged.txt")
        (tmp_path / "paged.txt").write_bytes(text.encode("utf-16-le"))
        result = await fops.read_file(path, offset=5, limit=3)
        assert result.error is None
    ...
    async def test_real_binary_still_refused(self, fops, tmp_path):
        # Zero bytes at BOTH parities → not UTF-16 → stays binary.
        p = tmp_path / "blob.dat"
        p.write_bytes(bytes([0x00, 0x01, 0x02, 0x00, 0xFF, 0x00, 0x00, 0xFE]) * 40)
        result = await fops.read_file(str(p))
        assert result.is_binary is True
        assert result.error is not None

    async def test_binary_extension_not_rescued(self, fops, tmp_path):
    ...
    async def test_utf8_file_unaffected(self, fops, tmp_path):
