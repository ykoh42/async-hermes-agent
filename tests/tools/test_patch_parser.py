...
*** End Patch"""
    ...
    async def test_multiple_hunks(self):
        patch = """\
*** Begin Patch
*** Update File: f.py
@@ first @@
...
class TestBoundaryMarkersInContent:
    ...
    async def test_end_patch_marker_in_add_content_does_not_truncate(self):
        patch = """\
*** Begin Patch
*** Add File: notes.md
+doc line one
...
*** End Patch"""
    ...
    async def test_begin_patch_marker_in_content_does_not_discard_operations(self):
        patch = """\
*** Begin Patch
*** Add File: first.md
+first file content
...
*** End Patch"""
    ...
    async def test_context_line_end_patch_marker_does_not_truncate(self):
        patch = """\
*** Begin Patch
...
class TestParseInvalidPatch:
    async def test_empty_patch_returns_empty_ops(self):
...
class TestApplyUpdate:
    async def test_preserves_non_prefix_pipe_characters_in_unmodified_lines(self):
        patch = """\
*** Begin Patch
...
*** End Patch"""
        ...
        result = await apply_v4a_operations(operations, file_ops)
...
class TestAdditionOnlyHunks:
    ...
    async def test_addition_only_hunk_with_context_hint(self):
...
*** End Patch"""
        ...
        file_ops = FakeFileOps()
        result = await apply_v4a_operations(ops, file_ops)
        assert result.success is True
        assert "def helper():" in file_ops.written
        assert "return 42" in file_ops.written

    async def test_addition_only_hunk_without_context_hint(self):
...
class TestReadFileRaw:
    ...
    async def test_apply_update_file_over_2000_lines(self):
...
*** End Patch"""
    ...
    async def test_apply_update_preserves_long_lines(self):
...
class TestValidationPhase:
    ...
    async def test_validation_failure_writes_nothing(self):
...
*** End Patch"""
        ...
        result = await apply_v4a_operations(ops, FakeFileOps())
        assert result.success is False
        assert written == {}, f"No files should have been written, got: {list(written.keys())}"
        assert "validation failed" in result.error.lower()


    async def test_validation_error_identifies_hunk_number(self):
        patch = """\
*** Begin Patch
*** Update File: a.py
@@ first @@
...
class TestApplyDelete:
    ...
    async def test_delete_diff_contains_removed_lines(self):
...
*** End Patch"""
    ...
    async def test_delete_diff_fallback_on_empty_file(self):
...
class TestParseErrorSignalling:
    ...
    async def test_update_with_no_hunks_returns_error(self):
...
*** End Patch"""
    ...
    async def test_valid_patch_returns_no_error(self):
...
class TestV4ALspDiagnosticsPropagation:
    ...
    async def test_lsp_diagnostics_propagated_from_write_file_on_add(self):
        ...
        class FakeFileOps:
            ...
            async def _check_lint(self, path):
    ...
    async def test_lsp_diagnostics_propagated_from_write_file_on_update(self):
    ...
    async def test_lsp_diagnostics_none_when_no_blocks_emitted(self):
    ...
    async def test_lsp_diagnostics_combined_across_multiple_files(self):
...
class TestMoveThenUpdateSameFile:
    ...
    async def test_move_then_update_destination(self):
        patch = (
            "*** Begin Patch\n"
            "*** Move File: a.py -> b.py\n"
            "*** Update File: b.py\n"
...
class TestCrlfPatchBody:
    ...
    async def test_crlf_body_applied_to_lf_file(self):
        patch = (
            "*** Begin Patch\r\n"
            "*** Update File: f.py\r\n"
            "@@\r\n"
