...
class TestFalRouting:
    ...
    async def test_text_to_image_uses_base_endpoint(self, cfg_home, monkeypatch):
        ...
        raw = await image_tool.image_generate_tool(prompt="a cat", aspect_ratio="square")
        out = json.loads(raw)
...
class TestPluginDispatchImageToImage:
    @pytest.mark.asyncio
    async def test_dispatch_forwards_image_url(self, cfg_home, monkeypatch):
        import tools.image_generation_tool as image_tool
        ...
        monkeypatch.setattr(
            ...
            AsyncMock(return_value="editcap"),
        )
        monkeypatch.setattr(
            image_tool, "_read_configured_image_model", AsyncMock(return_value=None)
        )
        monkeypatch.setattr(
        ...
        raw = await image_tool._dispatch_to_plugin_provider(
            "make night", "square",
...
class TestDynamicSchema:
    ...
    @pytest.mark.asyncio
    async def test_fal_edit_model_advertises_both(self, cfg_home, monkeypatch):
        from tools.image_generation_tool import _build_dynamic_image_schema

        _write_cfg(cfg_home, {"image_gen": {"model": "fal-ai/nano-banana-pro"}})
        desc = (await _build_dynamic_image_schema())["description"]
