...
async def test_monster_line_clamped_in_shell(tmp_path, ops):
    ...
    result = await ops.read_file(str(monster))
...
async def test_offset_past_monster_returns_normal_lines(tmp_path, ops):
    monster = tmp_path / "monster.txt"
    with open(monster, "w") as f:
        f.write("y" * 1_000_000 + "\n")
        for i in range(5):
    ...
    result = await ops.read_file(str(monster), offset=2)
    assert result.error is None
...
async def test_normal_multiline_read_unchanged(tmp_path, ops):
    p = tmp_path / "plain.txt"
    p.write_text("alpha\nbeta\ngamma\n")
    result = await ops.read_file(str(p))
    assert result.error is None
    assert result.content.startswith("1|alpha\n2|beta\n3|gamma")


async def test_no_trailing_newline_preserved(tmp_path, ops):
...
async def test_utf8_multibyte_boundary_no_mojibake(tmp_path, ops):
...
async def test_multibyte_line_over_limit_still_marked_truncated(tmp_path, ops):
...
async def test_read_file_raw_not_clamped(tmp_path, ops):
    ...
    result = await ops.read_file_raw(str(p))
    assert result.error is None
