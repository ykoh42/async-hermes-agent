...
class TestUnicodeVariantRepair:
    async def test_nfc_plain_space_spelling_repairs(self, ws):
        result = json.loads(await read_file_tool(str(ws / "notes" / CLEAN)))
        assert "rotate the keys" in result.get("content", "")
        assert "unicode-equivalent" in result.get("hint", "")

    async def test_exact_spelling_no_note(self, ws):
        result = json.loads(await read_file_tool(str(ws / "notes" / HOSTILE)))
        assert "rotate the keys" in result.get("content", "")
        assert "unicode-equivalent" not in (result.get("hint") or "")

    async def test_visible_difference_not_repaired(self, ws):
        # Straight quote + accent-less = visibly different: suggest, don't repair
        result = json.loads(
            await read_file_tool(str(ws / "notes" / "Meeting notes' resume 3.04 PM.txt"))
        )
        assert result.get("error"), "visible diff must stay a not-found"
        assert "unicode-equivalent" not in (result.get("hint") or "")
        assert result.get("similar_files")

    async def test_ambiguous_twins_not_repaired(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TERMINAL_CWD", str(tmp_path))
        ...
        result = json.loads(await read_file_tool(str(tmp_path / "CAFE.txt")))
        assert "unicode-equivalent" not in (result.get("hint") or "")

    async def test_plain_missing_file_unchanged(self, ws):
        result = json.loads(await read_file_tool(str(ws / "missing.txt")))
        assert "not found" in result.get("error", "").lower()


class TestNearMissSuggestion:
    async def test_agent_md_suggests_agents_md(self, ws):
        result = json.loads(await read_file_tool(str(ws / "AGENT.md")))
        sims = result.get("similar_files") or []
        assert any("AGENTS.md" in s for s in sims), sims

    async def test_unrelated_name_no_suggestion_of_agents(self, ws):
        result = json.loads(await read_file_tool(str(ws / "zzz_qqq.bin")))
        sims = result.get("similar_files") or []
