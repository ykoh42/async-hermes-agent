...
class _StubChild:
    ...
    async def run_conversation(self, user_message, task_id=None, **_kwargs):
...
async def _run(child):
    return await _run_single_child(0, "produce the address", child, _StubParent())


class TestRunSingleChildSchemaValidation:
    @pytest.mark.asyncio
    async def test_valid_first_try_no_retry(self):
        child = _StubChild(['{"city": "Berlin"}'])
        child._delegate_output_schema = ADDRESS_SCHEMA
        entry = await _run(child)
    ...
    @pytest.mark.asyncio
    async def test_invalid_then_retry_then_valid(self):
    ...
    @pytest.mark.asyncio
    async def test_invalid_twice_surfaces_errors_and_stops(self):
    ...
    @pytest.mark.asyncio
    async def test_retry_exception_degrades_to_invalid(self):
        child = _StubChild(["nope"])
        child._delegate_output_schema = ADDRESS_SCHEMA

        original = child.run_conversation

        async def flaky(user_message, task_id=None, **kw):
            if child.calls:
                raise RuntimeError("child died on retry")
            return await original(user_message, task_id=task_id, **kw)
    ...
    @pytest.mark.asyncio
    async def test_no_schema_keeps_legacy_result_shape(self):
    ...
    @pytest.mark.asyncio
    async def test_failed_child_skips_validation(self):
...
class TestDelegateTaskDispatch:
    @pytest.mark.asyncio
    async def test_non_dict_output_schema_rejected(self):
        with (
    ...
    @pytest.mark.asyncio
    async def test_invalid_json_schema_rejected_at_dispatch(self):
        with (
            patch("tools.delegate_tool._load_config", return_value={}),
            patch(
                "tools.delegate_tool._resolve_delegation_credentials",
    ...
    @pytest.mark.asyncio
    async def test_child_receives_contract_and_schema_attr(self):
