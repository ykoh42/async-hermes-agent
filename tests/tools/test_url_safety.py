...
def _resolves_to(*ips):
    """Patch event-loop DNS so any hostname resolves to *ips*.
...
class TestIsSafeUrl:
    async def test_public_url_allowed(self):
        with _resolves_to("93.184.216.34"):
            assert await is_safe_url("https://example.com/image.png") is True
    ...
    async def test_unusable_urls_blocked(self, url):
    ...
    async def test_private_and_loopback_targets_blocked(self, ip, url):
    ...
    async def test_dns_failure_blocked(self, monkeypatch):
        ...
        with patch(
            "asyncio.BaseEventLoop.getaddrinfo",
            new=AsyncMock(side_effect=socket.gaierror("Name resolution failed")),
        ):
            assert await is_safe_url("https://nonexistent.example.com") is False
...
class TestProxyEnvironmentDnsDelegation:
    ...
    async def test_dns_failure_allowed_when_proxy_configured(self, monkeypatch):
        monkeypatch.setenv("HTTPS_PROXY", "http://host.docker.internal:9090")
        with patch(
            "asyncio.BaseEventLoop.getaddrinfo",
            new=AsyncMock(side_effect=socket.gaierror("blocked at network level")),
        ):
            assert await is_safe_url("https://api.openai.com/v1/models") is True

    async def test_metadata_hostname_still_blocked_with_proxy(self, monkeypatch):
    ...
    async def test_literal_metadata_ip_still_blocked_with_proxy(self, monkeypatch):
    ...
    async def test_ipv6_scope_id_link_local_blocked(self):
        ...
        with _resolves_to("fe80::1%eth0"):
            assert await is_safe_url("http://[fe80::1%eth0]/") is False

    async def test_unparseable_ip_after_scope_strip_fails_closed(self):
        ...
        with _resolves_to("not-an-ip%garbage"):
            assert await is_safe_url("http://example.invalid/") is False
    ...
    async def test_benchmark_ip_blocked_for_non_allowlisted_host(self):
        with _resolves_to("198.18.0.23"):
            assert await is_safe_url("https://example.com/file.jpg") is False
    ...
    async def test_qq_multimedia_hostname_exception(self, url, expected):
        with _resolves_to("198.18.0.23"):
            assert await is_safe_url(url) is expected


class TestSSRFGuardedHttpxClient:
    @pytest.mark.asyncio
    async def test_connect_resolution_checks_private_ip_beyond_candidate_cap(self):
        answers = [
        ...
        with patch(
            ...
            with pytest.raises(SSRFConnectionBlocked, match="metadata"):
                await _resolved_http_connect_ips("example.com", 80, "http")
    ...
    async def test_async_backend_blocks_unix_socket_connects(self):
        ...
        backend = _SSRFGuardedNetworkBackend(contextvars.ContextVar("test_schemes"))
...
class TestGlobalAllowPrivateUrls:
    ...
    async def test_default_is_false(self, monkeypatch):
    ...
    async def test_config_security_string_false_stays_disabled(self, monkeypatch):
    ...
    async def test_profile_scoped_config_does_not_reuse_another_profiles_opt_out(
        ...
        monkeypatch.setattr(
            "asyncio.BaseEventLoop.getaddrinfo",
            AsyncMock(return_value=[(2, 1, 6, "", ("10.0.0.8", 0))]),
        )

        async def under_profile(home):
            token = set_hermes_home_override(home)
            try:
                return await is_safe_url("http://profile-private.test/resource")
            finally:
...
class TestAllowPrivateUrlsIntegration:
    ...
    async def test_private_ip_allowed_when_toggle_on(self, monkeypatch, ip, url):
    ...
    async def test_metadata_ip_blocked_even_with_toggle(self, monkeypatch, ip, url):
        monkeypatch.setenv("HERMES_ALLOW_PRIVATE_URLS", "true")
        with _resolves_to(ip):
            assert await is_safe_url(url) is False

    async def test_metadata_hostname_blocked_even_with_toggle(self, monkeypatch):
        """metadata.google.internal is ALWAYS blocked."""
        monkeypatch.setenv("HERMES_ALLOW_PRIVATE_URLS", "true")
        assert await is_safe_url("http://metadata.google.internal/computeMetadata/v1/") is False

    async def test_dns_failure_still_blocked_with_toggle(self, monkeypatch):
        ...
        with patch(
            "asyncio.BaseEventLoop.getaddrinfo",
            new=AsyncMock(side_effect=socket.gaierror("fail")),
        ):
            assert await is_safe_url("https://nonexistent.example.com") is False
...
class TestIsAlwaysBlockedUrl:
    ...
    async def test_literal_imds_ips_always_blocked(self, url):
        assert await is_always_blocked_url(url) is True

    async def test_gcp_metadata_hostname_always_blocked_even_without_dns(self):
        """metadata.google.internal blocks by hostname, no DNS needed."""
        with patch("socket.getaddrinfo", side_effect=socket.gaierror("nope")):
            assert await is_always_blocked_url("http://metadata.google.internal/") is True

    async def test_scope_id_imds_in_floor_blocked(self):
        ...
        with _resolves_to("::ffff:169.254.169.254%eth0"):
            assert await is_always_blocked_url("http://attacker-controlled.example.com/") is True

    # -- Things the floor must NOT block ----------------------------------------

    async def test_public_url_not_blocked(self):
        with _resolves_to("93.184.216.34"):
            assert await is_always_blocked_url("https://example.com/path") is False

    async def test_ordinary_private_urls_not_in_floor(self):
        ...
        """
        assert await is_always_blocked_url("http://127.0.0.1:8080/") is False
        assert await is_always_blocked_url("http://100.64.0.1/") is False

    async def test_dns_failure_not_in_floor(self):
        ...
        with patch(
            ...
            assert await is_always_blocked_url("http://nonexistent.example.com/") is False

    async def test_malformed_url_not_in_floor(self):
        """Parse errors don't claim always-blocked status."""
        assert await is_always_blocked_url("not a url at all") is False

    async def test_floor_ignores_allow_private_urls_toggle(self, monkeypatch):
        """security.allow_private_urls can NOT unblock cloud metadata."""
        monkeypatch.setenv("HERMES_ALLOW_PRIVATE_URLS", "true")
        assert await is_always_blocked_url("http://169.254.169.254/") is True
...
class TestIPv4MappedIPv6SSRF:
    ...
    async def test_ipv4_mapped_metadata_blocked(self, ip, url):
        with _resolves_to(ip):
            assert await is_safe_url(url) is False
