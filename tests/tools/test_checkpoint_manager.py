...
class TestStorePath:
    async def test_store_is_single_shared_path(self, work_dir, checkpoint_base, monkeypatch):
        monkeypatch.setattr("tools.checkpoint_manager.CHECKPOINT_BASE", checkpoint_base)
    ...
    async def test_project_hash_identifies_dir_and_expands_tilde(self, fake_home):
        project = fake_home / "project"
        project.mkdir()
        assert await _project_hash(str(project)) == await _project_hash(str(project))
...
class TestStoreInit:
    async def test_creates_git_store(self, work_dir, checkpoint_base, monkeypatch):
        monkeypatch.setattr("tools.checkpoint_manager.CHECKPOINT_BASE", checkpoint_base)
        store = _store_path(checkpoint_base)
        err = await _init_store(store, str(work_dir))
        assert err is None
        ...
        # Idempotent.
        assert await _init_store(store, str(work_dir)) is None

    async def test_legacy_migration_archives_prev2_repos(
...
class TestDisabledManager:
    async def test_disabled_manager_is_inert(self, disabled_mgr, work_dir):
        disabled_mgr.new_turn()
        assert await disabled_mgr.ensure_checkpoint(str(work_dir)) is False
...
class TestTakeCheckpoint:
    async def test_first_checkpoint_dedups_and_skips_unsafe_dirs(self, mgr, work_dir):
        assert await mgr.ensure_checkpoint(str(work_dir), "first") is True
        assert await mgr.ensure_checkpoint(str(work_dir), "second") is False  # dedup'd
    ...
    async def test_new_turn_resets_dedup_but_needs_changes(self, mgr, work_dir):
        ...
        assert await mgr.ensure_checkpoint(str(work_dir), "no changes") is False
        mgr.new_turn()
        (work_dir / "main.py").write_text("print('modified')\n")
        assert await mgr.ensure_checkpoint(str(work_dir), "turn 2") is True
...
class TestListCheckpoints:
    async def test_list_reports_newest_first(self, mgr, work_dir):
        assert await mgr.list_checkpoints(str(work_dir)) == []

        await mgr.ensure_checkpoint(str(work_dir), "first")
        result = await mgr.list_checkpoints(str(work_dir))
        assert len(result) == 1
        ...
        await mgr.ensure_checkpoint(str(work_dir), "second")
        mgr.new_turn()
        (work_dir / "main.py").write_text("v3\n")
        await mgr.ensure_checkpoint(str(work_dir), "third")
    ...
    async def test_projects_share_one_store_but_list_separately(
        ...
        assert await mgr.ensure_checkpoint(str(a), "A-1") is True
        mgr.new_turn()
        assert await mgr.ensure_checkpoint(str(b), "B-1") is True

        # Exactly one store dir + two project metas.
        assert (checkpoint_base / "store" / "HEAD").exists()
        projects = checkpoint_base / "store" / "projects"
        assert (projects / f"{await _project_hash(str(a))}.json").exists()
        assert (projects / f"{await _project_hash(str(b))}.json").exists()

        # Listing one project doesn't leak the other's checkpoints.
        assert [c["reason"] for c in await mgr.list_checkpoints(str(a))] == ["A-1"]
        assert [c["reason"] for c in await mgr.list_checkpoints(str(b))] == ["B-1"]
...
class TestRealPruning:
    async def test_max_snapshots_trims_history(self, work_dir, checkpoint_base, monkeypatch):
        monkeypatch.setattr("tools.checkpoint_manager.CHECKPOINT_BASE", checkpoint_base)
        ...
        for i in range(6):
            ...
            m.new_turn()
            await m.ensure_checkpoint(str(work_dir), f"step-{i}")

        cps = await m.list_checkpoints(str(work_dir))
        assert len(cps) == 3
    ...
    async def test_max_file_size_mb_skips_large_files(
        ...
        assert await m.ensure_checkpoint(str(wd), "initial") is True

        store = _store_path(checkpoint_base)
        ok, files, _ = await _run_git(
            ["ls-tree", "-r", "--name-only", _ref_name(await _project_hash(str(wd)))],
            store, str(wd),
        )
        assert ok
...
class TestRestore:
    async def test_restore_unknown_hash_fails(self, mgr, work_dir):
        assert (await mgr.restore(str(work_dir), "abc123"))["success"] is False  # no checkpoints
        await mgr.ensure_checkpoint(str(work_dir), "initial")
        assert (await mgr.restore(str(work_dir), "deadbeef1234"))["success"] is False


    async def test_tilde_path_supports_diff_and_restore_flow(
        ...
        assert await m.ensure_checkpoint(tilde, "initial") is True
        m.new_turn()

        file_path.write_text("changed\n")
        cps = await m.list_checkpoints(str(project))
        assert len(cps) == 1
        diff_result = await m.diff(tilde, cps[0]["hash"])
        assert diff_result["success"] is True
        assert "main.py" in diff_result["diff"]

        restore_result = await m.restore(tilde, cps[0]["hash"])
...
class TestWorkingDirResolution:
    async def test_resolves_project_root_markers(self, tmp_path, fake_home):
        ...
        assert await m.get_working_dir_for_path(
    ...
    async def test_falls_back_to_parent(self, tmp_path, monkeypatch):
        m = CheckpointManager(enabled=True)
        ...
        assert await m.get_working_dir_for_path(str(filepath)) == str(filepath.parent)
...
class TestGitEnvIsolation:
    async def test_env_pins_store_worktree_and_ignores_ambient_git_state(
        ...
        env = await _git_env(store, str(work))
        ...
        tilde_work.mkdir()
        env = await _git_env(store, f"~/{tilde_work.name}")
...
class TestErrorResilience:
    async def test_run_git_allows_expected_nonzero_without_error_log(
        self, tmp_path, caplog,
    ):
        work = tmp_path / "work"
        work.mkdir()
        (work / "left").write_text("left\n")
        (work / "right").write_text("right\n")
    ...
    async def test_run_git_cancellation_reaps_subprocess(
        self,
        tmp_path,
        monkeypatch,
    ):
        ...
        async with no_task_leaks(action=LeakAction.RAISE):
            ...
            try:
                task = asyncio.create_task(
                    _run_git(["status"], tmp_path / "store", str(work))
                )
    ...
    async def test_checkpoint_failures_never_raise(self, mgr, work_dir, monkeypatch):
        ...
        assert await mgr.ensure_checkpoint(str(work_dir), "test") is False
...
class TestTouchProjectMalformedMeta:
    ...
    async def test_non_dict_meta_does_not_raise(self, tmp_path):
        store = tmp_path / "store"
        workdir = str(tmp_path / "project")
        await _init_store(store, workdir)

        dir_hash = await _project_hash(workdir)
        meta_path = _project_meta_path(store, dir_hash)
        meta_path.parent.mkdir(parents=True, exist_ok=True)
        meta_path.write_text("[]", encoding="utf-8")

        # Must not raise TypeError
        await _touch_project(store, workdir)
...
class TestSecurity:
    async def test_restore_rejects_argument_injection(self, mgr, work_dir):
        await mgr.ensure_checkpoint(str(work_dir), "initial")
        result = await mgr.restore(str(work_dir), "--patch")
        assert result["success"] is False
        assert "Invalid commit hash" in result["error"]
        assert "must not start with '-'" in result["error"]

        result = await mgr.restore(str(work_dir), "-p")
        assert result["success"] is False
        assert "Invalid commit hash" in result["error"]

    async def test_restore_rejects_invalid_hex_chars(self, mgr, work_dir):
        await mgr.ensure_checkpoint(str(work_dir), "initial")
        result = await mgr.restore(str(work_dir), "abc; rm -rf /")
        assert result["success"] is False
        assert "expected 4-64 hex characters" in result["error"]

        result = await mgr.diff(str(work_dir), "abc&def")
        assert result["success"] is False
        assert "expected 4-64 hex characters" in result["error"]

    async def test_restore_file_path_confined_to_working_dir(self, mgr, work_dir):
        await mgr.ensure_checkpoint(str(work_dir), "initial")
        cps = await mgr.list_checkpoints(str(work_dir))
        target_hash = cps[0]["hash"]

        result = await mgr.restore(str(work_dir), target_hash, file_path="/etc/passwd")
        assert result["success"] is False
        assert "got absolute path" in result["error"]

        result = await mgr.restore(str(work_dir), target_hash, file_path="../outside_file.txt")
        assert result["success"] is False
        assert "escapes the working directory" in result["error"]

        # Relative paths inside the working dir are accepted, nested included.
        result = await mgr.restore(str(work_dir), target_hash, file_path="main.py")
        ...
        await mgr.ensure_checkpoint(str(work_dir), "second")
        cps = await mgr.list_checkpoints(str(work_dir))
        result = await mgr.restore(str(work_dir), cps[0]["hash"], file_path="subdir/test.txt")
        assert result["success"] is True
...
class TestGpgIsolation:
    async def test_init_disables_commit_and_tag_signing(
        self, work_dir, checkpoint_base, monkeypatch,
    ):
        monkeypatch.setattr("tools.checkpoint_manager.CHECKPOINT_BASE", checkpoint_base)
        store = _store_path(checkpoint_base)
        await _init_store(store, str(work_dir))
        for key in ("commit.gpgsign", "tag.gpgSign"):
            ok, value, error = await _run_git(
                ["config", "--get", key],
                store,
                str(work_dir),
            )
    ...
    async def test_checkpoint_works_with_global_gpgsign_and_broken_gpg(
        ...
        m = CheckpointManager(enabled=True)
        assert await m.ensure_checkpoint(str(work_dir), reason="with-global-gpgsign") is True
        assert len(await m.list_checkpoints(str(work_dir))) == 1
...
class TestPruneCheckpointsLegacy:
    ...
    async def test_deletes_orphan_when_workdir_missing(self, tmp_path):
        ...
        result = await prune_checkpoints(retention_days=0, checkpoint_base=base)
    ...
    async def test_noop_cases_delete_nothing(self, tmp_path):
        # Missing base.
        result = await prune_checkpoints(checkpoint_base=tmp_path / "does-not-exist")
...
class TestPruneCheckpointsV2:
    ...
    async def test_deletes_orphan_project_entry(self, tmp_path, monkeypatch):
        base = tmp_path / "checkpoints"
        ...
        m = CheckpointManager(enabled=True)
        assert await m.ensure_checkpoint(str(alive), "alive") is True
        m.new_turn()
        assert await m.ensure_checkpoint(str(gone), "gone") is True
        ...
        alive_hash = await _project_hash(str(alive))
        assert (base / "store" / "projects" / f"{alive_hash}.json").exists()
        # Gone project metadata wiped
        gone_hash = await _project_hash(str(gone))
        assert not (base / "store" / "projects" / f"{gone_hash}.json").exists()

    async def test_retention_prunes_stale_projects_and_legacy_archives(
        ...
        m = CheckpointManager(enabled=True)
        await m.ensure_checkpoint(str(fresh), "fresh")
        m.new_turn()
        await m.ensure_checkpoint(str(stale), "stale")

        # Backdate stale's last_touch to 60 days ago
        stale_hash = await _project_hash(str(stale))
        ...
        fresh_hash = await _project_hash(str(fresh))
...
class TestPruneCheckpointsOrphanAllowlist:
    ...
    async def test_v2_allowlist_restricts_deletion_to_approved_hash(self, tmp_path, monkeypatch):
        base = tmp_path / "checkpoints"
        monkeypatch.setattr("tools.checkpoint_manager.CHECKPOINT_BASE", base)
        ...
        m = CheckpointManager(enabled=True)
        assert await m.ensure_checkpoint(str(previewed), "previewed") is True
        m.new_turn()
        assert await m.ensure_checkpoint(str(newly_gone), "newly-gone") is True

        shutil.rmtree(previewed)
        shutil.rmtree(newly_gone)

        previewed_hash = await _project_hash(str(previewed))
        newly_gone_hash = await _project_hash(str(newly_gone))
    ...
    async def test_pre_v2_allowlist_restricts_deletion_to_approved_path(self, tmp_path):
        base = tmp_path / "checkpoints"
...
class TestMaybeAutoPruneCheckpoints:
    async def test_prunes_once_then_skips_within_interval(self, tmp_path):
        ...
        out = await maybe_auto_prune_checkpoints(
        ...
        second = await maybe_auto_prune_checkpoints(
            checkpoint_base=base, min_interval_hours=24,
        )
...
class TestStoreStatus:
    async def test_reports_projects_and_legacy(self, tmp_path, monkeypatch, work_dir):
        base = tmp_path / "checkpoints"
        monkeypatch.setattr("tools.checkpoint_manager.CHECKPOINT_BASE", base)

        m = CheckpointManager(enabled=True)
        await m.ensure_checkpoint(str(work_dir), "initial")
...
class TestClearFunctions:
    async def test_clear_all_wipes_base_then_is_a_noop(self, tmp_path, monkeypatch, work_dir):
        base = tmp_path / "checkpoints"
        monkeypatch.setattr("tools.checkpoint_manager.CHECKPOINT_BASE", base)
        m = CheckpointManager(enabled=True)
        await m.ensure_checkpoint(str(work_dir), "initial")
    ...
    async def test_clear_legacy_only_removes_legacy_dirs(
...
class TestOrphanPruneRequiresObservableDeletion:
    ...
    async def _project_with_history(self, work_dir, checkpoint_base, monkeypatch):
        monkeypatch.setattr(
    ...
    async def _history_survives(self, checkpoint_base, work_dir):
        store = _store_path(checkpoint_base)
        return _project_meta_path(
            store, await _project_hash(str(work_dir))
        ).exists()

    async def test_absent_workdir_without_deletion_evidence_keeps_history(
        self, tmp_path, checkpoint_base, monkeypatch,
    ):
        ...
        await self._project_with_history(vanished, checkpoint_base, monkeypatch)
        ...
        await self._project_with_history(unmounted, checkpoint_base, monkeypatch)
        ...
        assert await self._history_survives(checkpoint_base, vanished), (
            "checkpoint history was deleted for a project whose volume was "
            "merely unmounted"
        )
        assert await self._history_survives(checkpoint_base, unmounted), (
            "checkpoint history was deleted for a project whose mount point "
            "merely survived the unmount as an empty directory"
        )


    async def test_populated_underlay_mountpoint_keeps_its_checkpoints(
        self, tmp_path, checkpoint_base, monkeypatch,
    ):
        ...
        await self._project_with_history(work_dir, checkpoint_base, monkeypatch)
        ...
        assert result["deleted_orphan"] == 0
        assert await self._history_survives(checkpoint_base, work_dir), (
            "checkpoint history was deleted because underlay entries exposed "
            "by an unmount were mistaken for attachment evidence"
        )

    async def test_recorded_parent_identity_survives_probe_failure_on_touch(
        self, tmp_path, checkpoint_base, monkeypatch,
    ):
        ...
        store = _store_path(checkpoint_base)
        meta_path = _project_meta_path(store, await _project_hash(str(work_dir)))
        ...
        async def no_volume_evidence(_workdir):
            return {}

        monkeypatch.setattr(
            "tools.checkpoint_manager._volume_evidence", no_volume_evidence,
        )
        await _register_project(store, str(work_dir))
...
class TestSessionDiff:
    async def test_empty_when_nothing_changed(self, mgr, work_dir):
        """No checkpoints, and a checkpoint with no later edits, both report empty."""
        result = await mgr.session_diff(str(work_dir))
        assert result["success"] is True
        assert result.get("empty") is True
        assert result["diff"] == ""

        await mgr.ensure_checkpoint(str(work_dir), "baseline")
        result = await mgr.session_diff(str(work_dir))
        assert result["success"] is True
        assert result.get("empty") is True
        assert result["diff"] == ""
    ...
    async def test_includes_newly_added_files(self, mgr, work_dir):
        await mgr.ensure_checkpoint(str(work_dir), "baseline")
