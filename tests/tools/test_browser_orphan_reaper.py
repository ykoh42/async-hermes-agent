...
class TestReapOrphanedBrowserSessions:
    ...
    async def test_no_socket_dirs_is_noop(self, fake_tmpdir):
        """No socket dirs => nothing happens, no errors."""
        from tools.browser_tool import _reap_orphaned_browser_sessions
        await _reap_orphaned_browser_sessions()  # should not raise

    async def test_stale_dir_without_pid_file_is_removed(self, fake_tmpdir):
    ...
    async def test_alive_legacy_daemon_is_reaped(self, fake_tmpdir):
    ...
    async def test_corrupt_pid_file_is_cleaned(self, fake_tmpdir):
...
class TestOwnerPidCrossProcess:
    ...
    async def test_alive_owner_is_not_reaped_even_when_untracked(self, fake_tmpdir):
    ...
    async def test_owner_pid_permission_error_treated_as_alive(self, fake_tmpdir):
    ...
    async def test_write_owner_pid_swallows_oserror(self, fake_tmpdir, monkeypatch):
        ...
        # Must not raise
        await bt._write_owner_pid(str(fake_tmpdir), "h_readonly123")

    async def test_run_browser_command_calls_write_owner_pid(
        ...
        monkeypatch.setattr(
            ...
            AsyncMock(side_effect=RuntimeError("short-circuit after owner_pid")),
        )
        monkeypatch.setattr(bt, "_find_agent_browser", AsyncMock(return_value="/bin/true"))
        monkeypatch.setattr(
            bt, "_requires_real_termux_browser_install", AsyncMock(return_value=False)
        )
        monkeypatch.setattr(bt, "_is_local_mode", AsyncMock(return_value=False))
        monkeypatch.setattr(
            bt, "_get_session_info",
            AsyncMock(return_value={"session_name": session_name}),
        )
        monkeypatch.setattr(bt, "_is_headed_mode", AsyncMock(return_value=False))
        monkeypatch.setattr(bt, "_get_browser_engine", AsyncMock(return_value="auto"))
        monkeypatch.setattr(bt, "_is_camofox_mode", AsyncMock(return_value=False))
        monkeypatch.setattr(
            bt, "_needs_chromium_sandbox_bypass", AsyncMock(return_value=False)
        )
...
class TestEmergencyCleanupRunsReaper:
    ...
    async def test_emergency_cleanup_calls_reaper(self, fake_tmpdir, monkeypatch):
