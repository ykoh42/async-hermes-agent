...
class TestOrchestratorRoleSchema(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_default_role_is_leaf(self):
        ...
        self.assertFalse(child._owns_session_db)
...
class TestFallbackModelInheritance(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_pinned_provider_disables_parent_fallback_chain(self):
    ...
    async def test_pinned_acp_command_missing_raises(self):
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            MockAgent.return_value = MagicMock()
            with patch("tools.delegate_tool._executable_on_path", return_value=False):
                with self.assertRaises(ValueError) as ctx:
                    await _build_child_agent(
                        task_index=0,
    ...
    async def test_resolve_credentials_rejects_missing_pinned_command(self):
