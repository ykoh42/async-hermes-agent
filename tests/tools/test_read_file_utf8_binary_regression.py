...
class TestReadFileBinaryClassification:
    ...
    async def test_empty_file_reads_as_text(self, ops, tmp_path):
        path = _write(tmp_path, "empty.txt", b"")
        r = await ops.read_file(path)
        assert r.is_binary is False and r.error is None

    async def test_png_magic_stays_binary(self, ops, tmp_path):
        path = _write(
    ...
    async def test_elf_magic_stays_binary(self, ops, tmp_path):
        path = _write(
    ...
    async def test_nul_byte_in_text_stays_binary(self, ops, tmp_path):
        path = _write(tmp_path, "nul.txt", b"hello\x00world" + b"a" * 128)
        r = await ops.read_file(path)
        assert r.is_binary is True

    @pytest.mark.parametrize("encoding", ["utf-16-le", "utf-16-be"])
    async def test_utf16_transcodes_to_readable_text(self, ops, tmp_path, encoding):
...
class TestSiblingSites:
    async def test_read_file_raw_cjk_cut_is_text(self, ops, tmp_path):
        # 80221: patch/V4A goes through read_file_raw — same sampling site.
        path = _write(tmp_path, "cjk.md", ("漢字テキスト" * 200).encode("utf-8"))
        r = await ops.read_file_raw(path)
        assert r.is_binary is False and r.error is None

    async def test_patch_replace_on_boundary_cut_cjk_file(self, ops, tmp_path):
        path = _write(
            tmp_path, "doc.md", ("漢字テキスト" * 200 + "\nEND-MARKER\n").encode("utf-8")
        )
        r = await ops.patch_replace(path, "END-MARKER", "END-PATCHED")
        assert r.success is True
        assert "END-PATCHED" in Path(path).read_text(encoding="utf-8")

    async def test_search_finds_cjk_content(self, ops, tmp_path):
        # 80308: content search must not skip valid CJK files.
        _write(tmp_path, "notes.md", ("漢字テキスト\n" * 400).encode("utf-8"))
        r = await ops.search("漢字", path=str(tmp_path), target="content")
        assert r.error is None
        assert r.matches
