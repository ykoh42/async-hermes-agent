...
class TestBrowserSnapshotPrivateNetworkGuard:
    ...
    async def test_blocks_private_url_after_eval_navigation(self, monkeypatch):
        ...
        monkeypatch.setattr(
            browser_tool, "_run_browser_command", AsyncMock(side_effect=mock_run_browser_command)
        )

        result = json.loads(await browser_browser_snapshot(task_id="test"))
    ...
    async def test_allows_public_url_after_eval_navigation(self, monkeypatch):
    ...
    async def test_skips_check_in_local_backend_mode(self, monkeypatch):
    ...
    async def test_skips_check_when_private_urls_allowed(self, monkeypatch):
    ...
    async def test_handles_eval_failure_gracefully(self, monkeypatch):
    ...
    async def test_handles_eval_exception(self, monkeypatch):
    ...
    async def test_blocks_loopback_url(self, monkeypatch):
    ...
    async def test_blocks_private_ip_range(self, monkeypatch):
        ...
        for private_ip in ["http://10.0.0.1/api", "http://172.16.0.1/admin", "http://192.168.1.1/config"]:
            ...
            monkeypatch.setattr(
                ...
                AsyncMock(side_effect=mock_run_browser_command),
            )
...
async def browser_browser_snapshot(**kwargs):
    from tools.browser_tool import browser_snapshot
    return await browser_snapshot(**kwargs)


async def browser_browser_vision(**kwargs):
    from tools.browser_tool import browser_vision
    return await browser_vision(**kwargs)
...
class TestBrowserVisionPrivateNetworkGuard:
    ...
    async def test_blocks_private_url_after_eval_navigation(self, monkeypatch):
        ...
        result = json.loads(await browser_browser_vision(question="what do you see", task_id="test"))
        assert result["success"] is False
        assert "private or internal address" in result["error"]
        assert self.PRIVATE_URL in result["error"]
    ...
    async def test_allows_public_url_after_eval_navigation(self, monkeypatch):
        ...
        result_raw = await browser_browser_vision(question="what do you see", task_id="test")
    ...
    async def test_skips_check_in_local_backend_mode(self, monkeypatch):
        ...
        result_raw = await browser_browser_vision(question="what", task_id="test")
        result = json.loads(result_raw)
        assert "private or internal address" not in result.get("error", "")
