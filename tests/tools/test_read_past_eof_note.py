...
class TestPastEofNote:
    async def test_offset_beyond_eof_names_recovery(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TERMINAL_CWD", str(tmp_path))
        p = tmp_path / "r.txt"
        p.write_text("\n".join(f"l{i}" for i in range(1, 51)) + "\n")
        result = json.loads(await read_file_tool(str(p), offset=900, limit=50))
    ...
    async def test_offset_at_last_line_still_reads(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TERMINAL_CWD", str(tmp_path))
        p = tmp_path / "r.txt"
        p.write_text("a\nb\nc\n")
        result = json.loads(await read_file_tool(str(p), offset=3, limit=10))
        assert "c" in result.get("content", "")
        assert "beyond" not in (result.get("hint") or "")

    async def test_empty_file_says_so(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TERMINAL_CWD", str(tmp_path))
        p = tmp_path / "e.txt"
        p.write_text("")
    ...
    async def test_normal_pagination_hint_unchanged(self, tmp_path, monkeypatch):
        monkeypatch.setenv("TERMINAL_CWD", str(tmp_path))
        p = tmp_path / "r.txt"
        p.write_text("\n".join(f"l{i}" for i in range(1, 300)) + "\n")
        result = json.loads(await read_file_tool(str(p), offset=1, limit=100))
