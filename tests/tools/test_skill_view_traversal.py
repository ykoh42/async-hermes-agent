...
class TestPathTraversalBlocked:
    async def test_dotdot_in_skill_name_blocked(self, fake_skills):
        ...
        result = json.loads(await skill_view("../outside-skill", file_path=".env"))

        assert result["success"] is False
        assert "traversal" in result["error"].lower()
        assert "do-not-leak" not in json.dumps(result)

    async def test_absolute_skill_name_blocked(self, fake_skills):
        ...
        result = json.loads(await skill_view(str(outside_skill), file_path=".env"))

        assert result["success"] is False
        assert "relative path" in result["error"].lower()
        assert "do-not-leak" not in json.dumps(result)

    async def test_legit_skill_name_still_works(self, fake_skills):
        """A normal skill name must still resolve after the name guard."""
        result = json.loads(await skill_view("test-skill"))
        assert result["success"] is True

    async def test_dotdot_in_file_path(self, fake_skills):
        """Direct .. traversal should be rejected."""
        result = json.loads(await skill_view("test-skill", file_path="../../.env"))
        assert result["success"] is False
        assert "traversal" in result["error"].lower()

    async def test_dotdot_nested(self, fake_skills):
        """Nested .. traversal should also be rejected."""
        result = json.loads(await skill_view("test-skill", file_path="references/../../../.env"))
        assert result["success"] is False
        assert "traversal" in result["error"].lower()

    async def test_legitimate_file_still_works(self, fake_skills):
        """Valid paths within the skill directory should work normally."""
        result = json.loads(await skill_view("test-skill", file_path="references/api.md"))
        assert result["success"] is True
        assert "API docs here" in result["content"]

    async def test_no_file_path_shows_skill(self, fake_skills):
        """Calling skill_view without file_path should return the SKILL.md."""
        result = json.loads(await skill_view("test-skill"))
        assert result["success"] is True

    async def test_symlink_escape_blocked(self, fake_skills):
        ...
        result = json.loads(await skill_view("test-skill", file_path="evil-link"))
    ...
    async def test_sensitive_file_not_leaked(self, fake_skills):
