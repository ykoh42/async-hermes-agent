...
class TestCloudProviderRuntimeFallback:
    ...
    async def test_cloud_failure_falls_back_to_local(self, monkeypatch):
        """When cloud provider.create_session raises, fall back to local."""
        _reset_session_state(monkeypatch)

        provider = Mock()
        provider.create_session = AsyncMock(side_effect=RuntimeError("401 Unauthorized"))
        monkeypatch.setattr(
    ...
    async def test_no_provider_uses_local_directly(self, monkeypatch):
        ...
        session = await browser_tool._get_session_info("task-4")

        assert session["features"]["local"] is True
        assert "fallback_from_cloud" not in session


    async def test_cloud_returns_invalid_session_triggers_fallback(self, monkeypatch):
        """Cloud provider returning None or empty dict triggers fallback."""
        _reset_session_state(monkeypatch)

        provider = Mock()
        provider.create_session = AsyncMock(return_value=None)
        monkeypatch.setattr(
            browser_tool, "_get_cloud_provider", AsyncMock(return_value=provider)
        )
        monkeypatch.setattr(
        ...
        session = await browser_tool._get_session_info("task-7")
