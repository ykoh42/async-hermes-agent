...
def _reset_routing_state(monkeypatch):
    ...
    monkeypatch.setattr(browser_tool, "_start_browser_cleanup_thread", AsyncMock())
    monkeypatch.setattr(browser_tool, "_update_session_activity", lambda t: None)
    # Default: no CDP override, no Camofox
    monkeypatch.setattr(browser_tool, "_get_cdp_override", AsyncMock(return_value=None))
...
class TestNavigationSessionKey:
    ...
    async def test_public_url_uses_bare_task_id(self, monkeypatch):
        """Public URL with cloud provider configured → bare task_id (cloud)."""
        monkeypatch.setattr(browser_tool, "_get_cloud_provider", AsyncMock(return_value=Mock()))
        key = await browser_tool._navigation_session_key("default", "https://github.com/x/y")
        assert key == "default"

    async def test_localhost_routes_to_local_sidecar(self, monkeypatch):
        """``localhost`` URL → ``::local`` suffix when cloud configured + flag on."""
        monkeypatch.setattr(browser_tool, "_get_cloud_provider", AsyncMock(return_value=Mock()))
        key = await browser_tool._navigation_session_key("default", "http://localhost:3000/")
        assert key == "default::local"


    async def test_rfc1918_lan_routes_to_local_sidecar(self, monkeypatch):
        monkeypatch.setattr(browser_tool, "_get_cloud_provider", AsyncMock(return_value=Mock()))
        key = await browser_tool._navigation_session_key("default", "http://192.168.1.50:8000/")
        assert key == "default::local"


    async def test_none_task_id_defaults(self, monkeypatch):
        """``None`` task_id resolves to 'default'."""
        monkeypatch.setattr(browser_tool, "_get_cloud_provider", AsyncMock(return_value=Mock()))
        key = await browser_tool._navigation_session_key(None, "http://localhost:3000/")
        assert key == "default::local"


class TestSessionKeyHelpers:
    async def test_is_local_sidecar_key(self):
    ...
    async def test_last_session_key_drops_mismatched_owner_metadata(self, monkeypatch):
...
class TestHybridRoutingSessionCreation:
    ...
    async def test_local_sidecar_key_skips_cloud_provider(self, monkeypatch):
        ...
        monkeypatch.setattr(browser_tool, "_get_cloud_provider", AsyncMock(return_value=provider))
        monkeypatch.setattr(browser_tool, "_ensure_cdp_supervisor", AsyncMock())

        session = await browser_tool._get_session_info("default::local")
    ...
    async def test_bare_task_id_with_cloud_provider_uses_cloud(self, monkeypatch):
        ...
        monkeypatch.setattr(browser_tool, "_get_cloud_provider", AsyncMock(return_value=provider))
        monkeypatch.setattr(browser_tool, "_ensure_cdp_supervisor", AsyncMock())
        monkeypatch.setattr(browser_tool, "_resolve_cdp_override", AsyncMock(side_effect=lambda u: u))

        session = await browser_tool._get_session_info("default")

        provider.create_session.assert_awaited_once_with("default")
...
class TestCleanupHybridSessions:
    ...
    async def test_cleanup_reaps_both_primary_and_sidecar(self, monkeypatch):
        """Given a bare task_id with both sessions alive, both get cleaned."""
        reaped = []

        async def _fake_cleanup_one(key):
            reaped.append(key)
        ...
        await browser_tool.cleanup_browser("default")
    ...
    async def test_cleanup_sidecar_directly_keeps_primary(self, monkeypatch):
        ...
        await browser_tool.cleanup_browser("default::local")
