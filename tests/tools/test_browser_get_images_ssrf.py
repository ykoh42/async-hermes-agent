...
@pytest.mark.asyncio
async def test_blocks_images_on_private_page(monkeypatch):
    ...
    result = json.loads(await browser_tool.browser_get_images(task_id="test"))
...
@pytest.mark.asyncio
async def test_allows_images_on_public_page(monkeypatch):
    _mock_run_success(monkeypatch)
    monkeypatch.setattr(
        browser_tool, "_eval_ssrf_guard_active", AsyncMock(return_value=True)
    )
...
@pytest.mark.asyncio
async def test_guard_does_not_block_on_failed_eval(monkeypatch):
