...
class TestVideoToBase64DataUrl:
    ...
    @pytest.mark.asyncio
    async def test_produces_data_url(self, tmp_path):
        p = tmp_path / "test.mp4"
        p.write_bytes(b"\x00\x01\x02\x03")
        result = await _video_to_base64_data_url(p)
        assert result.startswith("data:video/mp4;base64,")


    @pytest.mark.asyncio
    async def test_default_mime_for_unknown_ext(self, tmp_path):
        p = tmp_path / "test.xyz"
        p.write_bytes(b"\x00\x01\x02\x03")
        result = await _video_to_base64_data_url(p)
...
class TestVideoAnalyzeTool:
    ...
    @pytest.mark.asyncio
    async def test_local_file_success(self, tmp_path, monkeypatch):
        ...
        with patch("tools.vision_tools.call_llm", new_callable=AsyncMock, return_value=mock_response):
            with patch("tools.vision_tools.extract_content_or_reasoning", return_value="A short video showing a demo."):
                result = await video_analyze_tool(str(video), "What is this?")
    ...
    @pytest.mark.asyncio
    async def test_local_file_read_guard_blocks_env_via_video_extension(self, tmp_path):
        ...
        with patch("tools.vision_tools.call_llm", new_callable=AsyncMock) as mock_llm:
            result = await video_analyze_tool(str(disguised), "What is this?")
    ...
    @pytest.mark.asyncio
    async def test_api_message_format(self, tmp_path):
        ...
        with patch("tools.vision_tools.call_llm", side_effect=capture_llm):
            with patch("tools.vision_tools.extract_content_or_reasoning", return_value="OK"):
                await video_analyze_tool(str(video), "Describe this")
    ...
    @pytest.mark.asyncio
    async def test_non_local_backend_reads_video_from_terminal_backend(self, tmp_path, monkeypatch):
        ...
        def fake_get_active(task_id):
            ...
            async def execute(_cmd, **_kw):
                return {"returncode": 0, "output": remote_b64}

            return SimpleNamespace(
                execute=execute,
            )

        async def ensure_task_env(*_args, **_kwargs):
            return None

        monkeypatch.setattr(tt, "ensure_task_env", ensure_task_env)
        ...
        with (
            patch("tools.vision_tools.call_llm", side_effect=capture_llm),
            patch("tools.vision_tools.extract_content_or_reasoning", return_value="sandbox video"),
        ):
            result = await video_analyze_tool(
                str(host_video), "Describe this", task_id="task-123"
            )
