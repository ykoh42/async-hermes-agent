...
@pytest.mark.asyncio
async def test_xai_provider_registers():
    ...
    assert await provider.default_model() == "grok-imagine-video"


@pytest.mark.asyncio
async def test_xai_resolved_credentials_threaded_through_request(monkeypatch):
    ...
    api_key, base_url = await xai_plugin._resolve_xai_credentials()
...
@pytest.mark.asyncio
async def test_xai_video_image_input_blocks_credential_store_symlink(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="credential store"):
        await _image_ref_to_xai_input(str(image_link))
