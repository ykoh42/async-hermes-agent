...
@pytest.mark.asyncio
async def test_identity_and_availability(monkeypatch):
...
@pytest.mark.asyncio
async def test_list_models_filters_by_video_gen_tag(monkeypatch):
    """list_models() returns only ``video-gen``-tagged catalog entries."""
    import hermes_cli.models as _models_mod

    async def _fake_by_tag(tag, **kw):
    ...
    rows = await deepinfra_plugin.DeepInfraVideoGenProvider().list_models()
...
def _mock_url_download(captured: dict, raise_exc: Exception | None = None):
    ...
    async def _fake_save_url_video(url, *, prefix="video", **kw):
        captured["url"] = url
        if raise_exc:
            raise raise_exc
