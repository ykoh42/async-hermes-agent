...
class TestXAIEndpoint:
    ...
    async def test_text_to_video_hits_generations(self, xai_provider):
        provider, captured = xai_provider
        result = await provider.generate("a dog on a skateboard")
        assert result["success"] is True
        assert _last_post(captured)["url"].endswith("/videos/generations")
        assert result["modality"] == "text"

    async def test_image_to_video_hits_generations(self, xai_provider):
        provider, captured = xai_provider
...
class TestXAIPayload:
    ...
    async def test_local_image_path_is_sent_as_data_uri(self, xai_provider, tmp_path):
        provider, captured = xai_provider
        image_path = tmp_path / "frame.png"
        image_path.write_bytes(b"\x89PNG\r\n\x1a\nfake")

        await provider.generate("animate this", image_url=str(image_path))
    ...
    async def test_explicit_model_override_is_honored_for_image(self, xai_provider):
        provider, captured = xai_provider
    ...
    async def test_reference_images_payload(self, xai_provider):
        provider, captured = xai_provider
        await provider.generate(
            "keep this character",
...
class TestXAIValidation:
    async def test_missing_prompt_rejects(self, xai_provider):
        provider, captured = xai_provider
        result = await provider.generate("")
    ...
    async def test_too_many_references_rejects(self, xai_provider):
        provider, captured = xai_provider
        result = await provider.generate(
            "x",
...
class TestXAIClamping:
    async def test_duration_clamped_to_15(self, xai_provider):
        provider, captured = xai_provider
        await provider.generate("x", duration=30)
        assert _last_post(captured)["json"]["duration"] == 15


    async def test_invalid_aspect_ratio_soft_clamps(self, xai_provider):
        provider, captured = xai_provider
        await provider.generate("x", aspect_ratio="21:9")
