...
@pytest.mark.asyncio
async def test_fal_provider_registers():
    ...
    assert await provider.default_model() == DEFAULT_MODEL
...
class TestFamilyRouting:
    ...
    async def test_image_to_video_routes_to_image_endpoint(self, with_fake_fal):
        from plugins.video_gen.fal import FALVideoGenProvider
    ...
    async def test_default_family_text_routing(self, with_fake_fal):
        ...
        result = await FALVideoGenProvider().generate("a dog")
        assert result["success"] is True
        expected_endpoint = FAL_FAMILIES[DEFAULT_MODEL]["text_endpoint"]
        assert with_fake_fal["endpoint"] == expected_endpoint


    async def test_unknown_family_falls_back_to_default(self, with_fake_fal):
        from plugins.video_gen.fal import FALVideoGenProvider, FAL_FAMILIES, DEFAULT_MODEL
    ...
    async def test_premium_seedance_routing(self, with_fake_fal):
