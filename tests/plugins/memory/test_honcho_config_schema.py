...
async def test_honcho_is_declared():
    provider = await get_provider_config_schema("honcho")
...
async def test_inline_fields_are_the_curated_subset():
    provider = await get_provider_config_schema("honcho")
    assert provider is not None
...
async def test_declares_the_new_field_kinds():
    provider = await get_provider_config_schema("honcho")
    assert provider is not None
...
async def test_selects_constrain_their_values():
    provider = await get_provider_config_schema("honcho")
    assert provider is not None
...
async def test_root_scoped_fields_are_exactly_the_global_ones():
    provider = await get_provider_config_schema("honcho")
    assert provider is not None
