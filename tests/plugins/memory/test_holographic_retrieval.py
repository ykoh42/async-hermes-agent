...
async def retriever_with_facts(tmp_path):
...
@pytest.mark.asyncio
async def test_prefetch_recovers_prose_query(retriever_with_facts):
    ...
    """
    results = await retriever_with_facts.search(
...
@pytest_asyncio.fixture
async def hoisted_retriever(tmp_path):
...
@pytest.mark.asyncio
async def test_search_encodes_query_vector_once(hoisted_retriever, monkeypatch):
    calls = _counting_spy(monkeypatch, "encode_text")
    results = await hoisted_retriever.search("deploy target setting")
...
@pytest.mark.asyncio
async def test_search_results_bit_identical_to_unhoisted(hoisted_retriever):
    ...
    new_results = await r.search(query)

    # --- pre-fix reference ---
    candidates = await r._fts_candidates(query, None, 0.3, 10 * 3)
...
@pytest.mark.asyncio
async def test_related_encodes_role_atoms_once(hoisted_retriever, monkeypatch):
    calls = _counting_spy(monkeypatch, "encode_atom")
    results = await hoisted_retriever.related("entity_1")
    assert results
...
@pytest.mark.asyncio
async def test_probe_encodes_role_atom_once(hoisted_retriever, monkeypatch):
    calls = _counting_spy(monkeypatch, "encode_atom")
    results = await hoisted_retriever.probe("entity_1")
    assert results
...
@pytest.mark.asyncio
async def test_search_without_vectors_never_encodes(hoisted_retriever, monkeypatch):
    ...
    await store._conn.execute("UPDATE facts SET hrr_vector = NULL")
