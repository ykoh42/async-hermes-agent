...
async def test_project_dir_is_ignored_without_opt_in(tmp_path, monkeypatch):
    ...
    assert "projectmem" not in await memory_plugins.list_memory_provider_names()
    assert await memory_plugins.find_provider_dir("projectmem") is None


async def test_project_dir_is_discovered_when_opted_in(tmp_path, monkeypatch):
    provider = _write_provider_dir(tmp_path / ".hermes" / "plugins", "projectmem")
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("HERMES_ENABLE_PROJECT_PLUGINS", "1")

    assert "projectmem" in await memory_plugins.list_memory_provider_names()
    assert await memory_plugins.find_provider_dir("projectmem") == provider


async def test_bundled_still_wins_over_project(tmp_path, monkeypatch):
    ...
    resolved = await memory_plugins.find_provider_dir("honcho")
...
async def test_entry_point_provider_is_listed(entry_points, tmp_path, monkeypatch):
    ...
    assert "pipmem" in await memory_plugins.list_memory_provider_names()


async def test_find_provider_dir_resolves_a_package_entry_point(entry_points, tmp_path, monkeypatch):
    ...
    assert await memory_plugins.find_provider_dir("pipmem") == package


async def test_resolving_an_entry_point_does_not_import_it(entry_points, tmp_path, monkeypatch):
    ...
    assert await memory_plugins.find_provider_dir("sideeffect") == package
    assert not (package / "IMPORTED").exists()
    assert "sideeffect_pkg" not in sys.modules


async def test_bare_module_entry_point_has_no_directory(entry_points, tmp_path, monkeypatch):
    ...
    assert await memory_plugins.find_provider_dir("flatmem") is None
...
async def test_a_secondary_registration_cannot_cost_the_provider(tmp_path, monkeypatch):
    ...
    loaded = await memory_plugins.load_memory_provider("auxmem")
...
async def test_activation_is_not_gated_on_plugins_enabled(tmp_path, monkeypatch):
    ...
    assert await memory_plugins.load_memory_provider("gatedmem") is not None
