...
class TestMetadata:
    ...
    async def test_display_name(self, provider):
        assert provider.display_name == "OpenAI (Codex auth)"

    async def test_default_model(self, provider):
        assert await provider.default_model() == "gpt-image-2-medium"

    async def test_list_models_three_tiers(self, provider):
        ids = [m["id"] for m in await provider.list_models()]
        assert ids == ["gpt-image-2-low", "gpt-image-2-medium", "gpt-image-2-high"]

    async def test_setup_schema_has_no_required_env_vars(self, provider):
...
class TestAvailability:
    async def test_unavailable_without_codex_token(self, monkeypatch):
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.setattr(codex_plugin, "_read_codex_access_token", AsyncMock(return_value=None))
        assert await codex_plugin.OpenAICodexImageGenProvider().is_available() is False

    async def test_available_with_codex_token(self, monkeypatch):
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.setattr(
            codex_plugin, "_read_codex_access_token", AsyncMock(return_value="codex-token")
        )
        assert await codex_plugin.OpenAICodexImageGenProvider().is_available() is True

    async def test_openai_api_key_alone_is_not_enough(self, monkeypatch):
...
class TestGenerate:
    async def test_returns_auth_error_without_codex_token(self, provider, monkeypatch):
        monkeypatch.setattr(codex_plugin, "_read_codex_access_token", AsyncMock(return_value=None))
    ...
    async def test_generate_uses_codex_stream_path(self, provider, monkeypatch, tmp_path):
        monkeypatch.setattr(
            codex_plugin, "_read_codex_access_token", AsyncMock(return_value="codex-token")
        )
        monkeypatch.setattr(
            codex_plugin, "_collect_image_b64", AsyncMock(return_value=_b64_png())
        )

        result = await provider.generate("a cat", aspect_ratio="landscape")
    ...
    async def test_codex_stream_request_shape(self, provider, monkeypatch):
        monkeypatch.setattr(
            codex_plugin, "_read_codex_access_token", AsyncMock(return_value="codex-token")
        )
        ...
        async def _collect(token, *, prompt, size, quality, input_images=None):
        ...
        result = await provider.generate("a cat", aspect_ratio="portrait")
        assert result["success"] is True
    ...
    async def test_capabilities_advertise_image_inputs(self, provider):
        caps = await provider.capabilities()
        assert caps["modalities"] == ["text", "image"]
        assert caps["max_reference_images"] == 16


    async def test_rejects_non_image_local_source(self, provider, monkeypatch, tmp_path):
        monkeypatch.setattr(
            codex_plugin, "_read_codex_access_token", AsyncMock(return_value="codex-token")
        )
        ...
        result = await provider.generate("edit this", image_url=str(text_path))
    ...
    async def test_partial_image_event_used_when_done_missing(self):
    ...
    async def test_sse_parser_handles_event_and_data_lines(self):
        class _Response:
        ...
        events = [event async for event in codex_plugin._iter_sse_json(_Response())]
        assert events == [{
            "type": "response.output_item.done",
            "item": {"type": "image_generation_call", "result": "abc"},
        }]

    async def test_final_response_sweep_recovers_image(self):
    ...
    async def test_empty_response_returns_error(self, provider, monkeypatch):
        monkeypatch.setattr(
            codex_plugin, "_read_codex_access_token", AsyncMock(return_value="codex-token")
        )
        monkeypatch.setattr(codex_plugin, "_collect_image_b64", AsyncMock(return_value=None))

        result = await provider.generate("a cat")
        assert result["success"] is False
        assert result["error_type"] == "empty_response"

    async def test_stream_exception_returns_api_error(self, provider, monkeypatch):
        monkeypatch.setattr(
            codex_plugin, "_read_codex_access_token", AsyncMock(return_value="codex-token")
        )
    ...
    async def test_tool_choice_400_surfaces_verbatim_not_as_capability_error(
...
class TestRequestShape:
    async def test_payload_omits_tool_choice(self):
    ...
    async def test_http_error_body_is_truncated_but_preserved(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="HTTP 400") as excinfo:
            await codex_plugin._collect_image_b64(
                "codex-token",
                prompt="a cat",
                size="1024x1024",
                quality="low",
...
class TestRegistration:
    async def test_register_calls_register_image_gen_provider(self):
        registered = []
