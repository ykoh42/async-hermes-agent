...
class TestFalImageGenProviderSurface:
    ...
    async def test_default_model_matches_legacy(self):
        from plugins.image_gen.fal import FalImageGenProvider
        from tools.image_generation_tool import DEFAULT_MODEL

        assert await FalImageGenProvider().default_model() == DEFAULT_MODEL

    async def test_list_models_uses_legacy_catalog(self):
        from plugins.image_gen.fal import FalImageGenProvider
    ...
    async def test_setup_schema_advertises_fal_key(self):
        from plugins.image_gen.fal import FalImageGenProvider

        schema = await FalImageGenProvider().get_setup_schema()
...
class TestFalImageGenProviderAvailability:
    async def test_is_available_when_legacy_check_passes(self, monkeypatch):
...
class TestFalImageGenProviderGenerate:
    async def test_generate_delegates_to_legacy_image_generate_tool(self, monkeypatch):
...
class TestFalImageGenPluginRegistration:
    async def test_register_wires_provider_into_registry(self):
