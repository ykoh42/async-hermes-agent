...
class TestMetadata:
    ...
    async def test_catalog_entries_have_display_speed_strengths(self, provider):
...
class TestAvailability:
    async def test_no_api_key_unavailable(self, monkeypatch):
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        assert await openai_plugin.OpenAIImageGenProvider().is_available() is False

    async def test_api_key_set_available(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "test")
        assert await openai_plugin.OpenAIImageGenProvider().is_available() is True
...
class TestModelResolution:
    ...
    async def test_env_var_override(self, monkeypatch):
        monkeypatch.setenv("OPENAI_IMAGE_MODEL", "gpt-image-2-high")
        model_id, meta = await openai_plugin._resolve_model()
        assert model_id == "gpt-image-2-high"
        assert meta["quality"] == "high"


    async def test_config_openai_model(self, tmp_path):
        import yaml
...
class TestSourceImageLoading:
    async def test_load_image_bytes_blocks_credential_store(self, tmp_path, monkeypatch):
        ...
        with pytest.raises(ValueError, match="credential store"):
            await openai_plugin._load_image_bytes(str(auth_json))


    async def test_load_image_bytes_allows_legit_local_image(self, tmp_path, monkeypatch):
        ...
        data, name = await openai_plugin._load_image_bytes(str(img))
...
class TestGenerate:
    async def test_empty_prompt_rejected(self, provider):
        result = await provider.generate("", aspect_ratio="square")
        assert result["success"] is False
        assert result["error_type"] == "invalid_argument"

    async def test_missing_api_key(self, monkeypatch):
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        result = await openai_plugin.OpenAIImageGenProvider().generate("a cat")
        assert result["success"] is False
        assert result["error_type"] == "auth_required"

    async def test_b64_saves_to_cache(self, provider, tmp_path):
        png_bytes = bytes.fromhex(_PNG_HEX)
        fake_client = _fake_client(response=_fake_response(b64=_b64_png()))
    ...
    async def test_tier_maps_to_quality(self, provider, monkeypatch, tier, expected_quality):
    ...
    async def test_aspect_ratio_mapping(self, provider, aspect, expected_size):
        fake_client = _fake_client(response=_fake_response(b64=_b64_png()))

        with _patched_openai(fake_client):
            await provider.generate("a cat", aspect_ratio=aspect)

        assert fake_client.images.generate.call_args.kwargs["size"] == expected_size

    async def test_revised_prompt_passed_through(self, provider):
        fake_client = _fake_client(
            response=_fake_response(b64=_b64_png(), revised_prompt="A photo of a cat")
        )
    ...
    async def test_url_response_is_cached_locally(self, provider):
        ...
        """
        fake_client = _fake_client(
            response=_fake_response(b64=None, url="https://example.com/img.png")
        )
