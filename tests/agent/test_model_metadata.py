...
def _async_http_client(*, response=None, side_effect=None):
    client = MagicMock()
...
class TestEstimateTokensRough:
    ...
    async def test_known_length(self):
...
class TestEstimateMessagesTokensRough:
    ...
    async def test_message_with_list_content(self):
...
class TestEstimateRequestTokensRough:
    async def test_caches_tools_estimate(self):
    ...
    async def test_tools_cache_is_bounded(self):
...
class TestDefaultContextLengths:
    ...
    async def test_k3_context_is_scoped_to_confirmed_coding_endpoint(self):
        ...
        """
        with patch("agent.model_metadata.get_cached_context_length", new_callable=AsyncMock, return_value=None), \
             patch("agent.model_metadata.fetch_model_metadata", new_callable=AsyncMock, return_value={}), \
             patch("agent.model_metadata.fetch_endpoint_model_metadata", new_callable=AsyncMock, return_value={}), \
             patch("agent.model_metadata._query_ollama_api_show", new_callable=AsyncMock, return_value=None), \
             patch("agent.models_dev.lookup_models_dev_context", new_callable=AsyncMock, return_value=None):
            accepted_urls = (
            ...
            for base_url in accepted_urls:
                for model in ("k3", "kimi-k3", "kimi-k3-cot"):
                    assert await get_model_context_length(
    ...
    async def test_xai_oauth_grok_build_uses_xai_models_dev_context(self):
        ...
        with patch("agent.model_metadata.get_cached_context_length", new_callable=AsyncMock, return_value=None), \
             patch("agent.model_metadata._query_ollama_api_show", new_callable=AsyncMock, return_value=None), \
             patch("agent.models_dev.fetch_models_dev", new_callable=AsyncMock, return_value=registry):
            assert await get_model_context_length(
                "grok-build-0.1",
                provider="xai-oauth",
    ...
    async def test_deepseek_v4_models_1m_context(self):
        ...
        with mock_patch("agent.model_metadata.fetch_model_metadata", new_callable=AsyncMock, return_value={}), \
             mock_patch("agent.model_metadata.fetch_endpoint_model_metadata", new_callable=AsyncMock, return_value={}), \
             mock_patch("agent.model_metadata.get_cached_context_length", new_callable=AsyncMock, return_value=None):
            cases = [
                ("deepseek-v4-pro", 1_000_000),
                ("deepseek-v4-flash", 1_000_000),
            ...
            for model_id, expected_ctx in cases:
                actual = await get_model_context_length(model_id)
...
class TestCodexOAuthContextLength:
    ...
    async def test_live_catalogue_cache_is_scoped_to_access_token(self):
        ...
        clients = [
            _async_http_client(response=first_response),
            _async_http_client(response=second_response),
        ]
        with patch("httpx.AsyncClient", side_effect=clients) as mock_client, patch(
            "agent.model_metadata.save_context_length", new_callable=AsyncMock
        ) as mock_save:
            first = await get_model_context_length(
                "gpt-5.6-terra",
                base_url="https://chatgpt.com/backend-api/codex",
                api_key="token-account-a",
                provider="openai-codex",
            )
            first_again = await get_model_context_length(
                "gpt-5.6-terra",
                base_url="https://chatgpt.com/backend-api/codex",
                api_key="token-account-a",
                provider="openai-codex",
            )
            second = await get_model_context_length(
                "gpt-5.6-terra",
                base_url="https://chatgpt.com/backend-api/codex",
        ...
        assert mock_client.call_args_list[0].kwargs["headers"]["Authorization"] == "Bearer token-account-a"
        assert mock_client.call_args_list[1].kwargs["headers"]["Authorization"] == "Bearer token-account-b"
    ...
    async def test_probe_failure_falls_back_to_hardcoded(self):
        ...
        with patch("httpx.AsyncClient", return_value=_async_http_client(response=fake_response)), \
             patch("agent.model_metadata.get_cached_context_length", new_callable=AsyncMock, return_value=None), \
             patch("agent.model_metadata.save_context_length", new_callable=AsyncMock):
    ...
    async def test_live_codex_context_replaces_stale_cache_in_both_directions(
        ...
        client = _async_http_client(response=fake_response)
...
class TestNousPortalContextResolution:
    ...
    @patch("agent.model_metadata.fetch_endpoint_model_metadata", new_callable=AsyncMock)
    @patch("agent.model_metadata.fetch_model_metadata", new_callable=AsyncMock)
    async def test_openrouter_fallback_is_not_persisted(
    ...
    @patch("agent.model_metadata.fetch_endpoint_model_metadata", new_callable=AsyncMock)
    @patch("agent.model_metadata.fetch_model_metadata", new_callable=AsyncMock)
    async def test_stale_cache_is_bypassed_and_overwritten_by_portal(
        self, mock_or, mock_portal, tmp_path, monkeypatch
    ):
...
class TestGetModelContextLength:
    @patch("agent.model_metadata.fetch_model_metadata", new_callable=AsyncMock)
    async def test_known_model_from_api(self, mock_fetch):
        mock_fetch.return_value = {
            "test/model": {"context_length": 32000}
        }
        assert await get_model_context_length("test/model") == 32000
    ...
    @patch("agent.model_metadata.fetch_model_metadata", new_callable=AsyncMock)
    async def test_api_missing_context_length_key(self, mock_fetch):
        ...
        assert await get_model_context_length("test/model") == CONTEXT_PROBE_TIERS[0]


    @patch("agent.model_metadata.fetch_model_metadata", new_callable=AsyncMock)
    async def test_no_base_url_skips_cache(self, mock_fetch, tmp_path):
        """Without base_url, cache lookup is skipped."""
        mock_fetch.return_value = {}
        cache_file = tmp_path / "cache.yaml"
        with patch("agent.model_metadata._get_context_cache_path", return_value=cache_file):
            await save_context_length("custom/model", "http://local", 32768)
            # No base_url → cache skipped → falls to probe tier
            result = await get_model_context_length("custom/model")
            assert result == CONTEXT_PROBE_TIERS[0]


    @patch("agent.model_metadata.fetch_model_metadata", new_callable=AsyncMock)
    @patch("agent.model_metadata.fetch_endpoint_model_metadata", new_callable=AsyncMock)
    async def test_custom_endpoint_without_metadata_falls_back_to_catalog(self, mock_endpoint_fetch, mock_fetch):
    ...
    @patch("agent.model_metadata.fetch_model_metadata", new_callable=AsyncMock)
    async def test_custom_endpoint_falls_back_to_hardcoded_catalog(self, mock_fetch):
        ...
        with (
            ...
            # Another known model
            ctx2 = await get_model_context_length(
                "claude-sonnet-4-6",
                base_url="https://my-gateway.example.com/v1/claude",
            )
            ...
            ctx3 = await get_model_context_length(
    ...
    @patch("agent.model_metadata.get_cached_context_length", new_callable=AsyncMock, return_value=None)
    @patch("agent.model_metadata.fetch_model_metadata", new_callable=AsyncMock, return_value={})
    @patch("agent.model_metadata._resolve_endpoint_context_length", new_callable=AsyncMock, return_value=None)
    @patch("agent.model_metadata._query_ollama_api_show", new_callable=AsyncMock, return_value=131072)
    @patch("agent.model_metadata._query_local_context_length", new_callable=AsyncMock, return_value=32768)
    @patch("agent.model_metadata.is_local_endpoint", return_value=True)
    @patch("agent.model_metadata.save_context_length", new_callable=AsyncMock)
    @patch("agent.model_metadata._maybe_cache_local_context_length", new_callable=AsyncMock)
    async def test_local_ollama_prefers_num_ctx_over_gguf(
        self,
...
class TestBedrockContextResolution:
    ...
    @patch("agent.model_metadata.fetch_endpoint_model_metadata", new_callable=AsyncMock)
    async def test_bedrock_claude_4_6_ignores_stale_200k_cache(self, mock_fetch, tmp_path):
        """Old 200K Bedrock cache entries must not mask the 1M table entry."""
        cache_file = tmp_path / "context_length_cache.yaml"
        base_url = "https://bedrock-runtime.us-east-2.amazonaws.com"
        with patch("agent.model_metadata._get_context_cache_path", return_value=cache_file):
            await save_context_length("us.anthropic.claude-sonnet-4-6", base_url, 200_000)
            ctx = await get_model_context_length(
    ...
    @patch("agent.model_metadata.fetch_endpoint_model_metadata", new_callable=AsyncMock)
    async def test_non_bedrock_url_still_probes(self, mock_fetch):
...
class TestStripProviderPrefix:
    async def test_known_provider_prefix_is_stripped(self):
    ...
    async def test_http_urls_preserved(self):
    ...
    @patch("agent.model_metadata.fetch_model_metadata", new_callable=AsyncMock)
    async def test_ollama_model_tag_not_mangled_in_context_lookup(self, mock_fetch):
        ...
        """
        mock_fetch.return_value = {}
        with patch("agent.model_metadata.fetch_endpoint_model_metadata", new_callable=AsyncMock) as mock_ep, \
...
class TestFetchModelMetadata:
    ...
    async def test_network_success_writes_disk_cache(self, tmp_path, monkeypatch):
        self._reset_cache()
        ...
        with patch(
            "httpx.AsyncClient",
            return_value=_async_http_client(response=mock_response),
        ):
            await fetch_model_metadata(force_refresh=True)

        assert cache_path.exists()
        assert "live/model" in cache_path.read_text(encoding="utf-8")

    async def test_network_failure_falls_back_to_stale_disk_cache(self, tmp_path, monkeypatch):
        self._reset_cache()
        cache_path = self._isolate_disk_cache(monkeypatch, tmp_path)
        ...
        with patch(
            "httpx.AsyncClient",
            return_value=_async_http_client(side_effect=Exception("Network error")),
        ):
            result = await fetch_model_metadata(force_refresh=True)
    ...
    async def test_caches_result(self):
        ...
        mock_response.raise_for_status = MagicMock()
        client = _async_http_client(response=mock_response)

        with patch("httpx.AsyncClient", return_value=client):
            result1 = await fetch_model_metadata(force_refresh=True)
            result2 = await fetch_model_metadata()
    ...
    async def test_canonical_slug_aliasing(self):
...
class TestContextProbeTiers:
    async def test_tiers_descending(self):
        for i in range(len(CONTEXT_PROBE_TIERS) - 1):
            assert CONTEXT_PROBE_TIERS[i] > CONTEXT_PROBE_TIERS[i + 1]


class TestGetNextProbeTier:
    async def test_from_256k(self):
        assert get_next_probe_tier(256_000) == 128_000




    async def test_from_8k_returns_none(self):
...
class TestParseContextLimitFromError:
    ...
    async def test_vllm_delimiter_variants(self, msg, expected):
    ...
    async def test_get_context_length_from_vllm_max_model_len_error(self):
...
class TestContextLengthCache:
    ...
    async def test_null_context_lengths_key_returns_empty(self, tmp_path):
        ...
        with patch("agent.model_metadata._get_context_cache_path", return_value=cache_file):
            assert await get_cached_context_length("test/model", "http://x") is None
            # save must also survive the null key and repair the file
            await save_context_length("test/model", "http://x", 32768)
            assert await get_cached_context_length("test/model", "http://x") == 32768



    async def test_idempotent_save(self, tmp_path):
        cache_file = tmp_path / "cache.yaml"
        with patch("agent.model_metadata._get_context_cache_path", return_value=cache_file):
            await save_context_length("model", "http://x", 32768)
            await save_context_length("model", "http://x", 32768)
    ...
    @patch("agent.model_metadata.fetch_model_metadata", new_callable=AsyncMock)
    async def test_cached_value_takes_priority(self, mock_fetch, tmp_path):
        mock_fetch.return_value = {}
        cache_file = tmp_path / "cache.yaml"
        with patch("agent.model_metadata._get_context_cache_path", return_value=cache_file):
            await save_context_length("unknown/model", "http://local", 65536)
            assert await get_model_context_length("unknown/model", base_url="http://local") == 65536
...
class TestGrok43StaleCacheGuard:
    ...
    async def test_suggests_grok_4_3(self):
    ...
    async def test_stale_grok_4_3_dropped_and_reresolves_to_1m(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        import importlib
        import agent.model_metadata as mm
        importlib.reload(mm)
        base = "https://api.x.ai/v1"
        await mm.save_context_length("grok-4.3", base, 256_000)
        ctx = await mm.get_model_context_length(
    ...
    async def test_grok_4_not_clobbered(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        import importlib
        import agent.model_metadata as mm
        importlib.reload(mm)
        ...
        for slug in ("grok-4", "grok-4-0709"):
            await mm.save_context_length(slug, base, 256_000)
            ctx = await mm.get_model_context_length(
...
class TestMoAContextLength:
    ...
    async def test_moa_resolves_from_aggregator(self, tmp_path, monkeypatch):
        ...
        agg_ctx = await get_model_context_length(
            "anthropic/claude-opus-4.8", base_url="https://openrouter.ai/api/v1", provider="openrouter"
        )
        moa_ctx = await get_model_context_length("p", base_url="http://127.0.0.1/v1", provider="moa")
        assert moa_ctx == agg_ctx




    async def test_moa_custom_context_configures_compressor_threshold(
