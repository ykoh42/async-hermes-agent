...
async def skills(tmp_path, monkeypatch):
...
class TestDescribeSkillInvocation:
    ...
    async def test_ignores_non_string_content(self):
        assert describe_skill_invocation(None) is None
        assert describe_skill_invocation([{"type": "text", "text": "hi"}]) is None

    async def test_recovers_the_typed_instruction(self, skills):
    ...
    async def test_bundle_carries_its_typed_keys(self, skills):
        result = await skill_bundles.build_bundle_invocation_message(
...
class TestExcerptedScaffolding:
    ...
    async def test_excerpt_still_recovers_the_instruction(self, skills):
        message = await skill_commands.build_skill_invocation_message(
            "/work", user_instruction="fix the title leak"
        )
    ...
    async def test_description_never_runs_across_the_joint(self, skills):
...
class TestSqlLikePattern:
    async def test_matches_the_prefix_the_builders_emit(self, skills):
        message = await skill_commands.build_skill_invocation_message("/work")
        assert message.startswith(SKILL_SCAFFOLD_SQL_LIKE.rstrip("%"))

    async def test_carries_no_like_wildcards_needing_escape(self):
