...
class _FakeProvider(ImageGenProvider):
    ...
    async def generate(self, prompt, aspect_ratio="landscape", **kw):
...
class TestGetActiveProvider:
    ...
    @pytest.mark.asyncio
    async def test_explicit_config_wins(self, tmp_path, monkeypatch):
        import yaml
        ...
        active = await image_gen_registry.get_active_provider()
        assert active is not None and active.name == "openai"


    @pytest.mark.asyncio
    async def test_none_when_empty(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        assert await image_gen_registry.get_active_provider() is None
