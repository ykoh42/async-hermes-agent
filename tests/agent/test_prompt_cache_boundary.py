...
def skills(tmp_path, monkeypatch):
    ...
    asyncio.run(skill_commands.scan_skill_commands())
    return skills_dir
...
class TestRealBuilders:
    @pytest.mark.asyncio
    async def test_two_invocations_share_the_marked_scaffold(self, skills):
        first = await skill_commands.build_skill_invocation_message(
            "/triage", user_instruction="ticket=one time=10:00"
        )
        second = await skill_commands.build_skill_invocation_message(
    ...
    @pytest.mark.asyncio
    async def test_payload_quoting_the_marker_cannot_poison_the_stable_prefix(self, skills):
        ...
        hostile_blocks = apply_anthropic_cache_control(
            [
                {
                    "role": "user",
                    "content": await skill_commands.build_skill_invocation_message(
    ...
    @pytest.mark.asyncio
    async def test_bare_invocation_keeps_whole_block_layout(self, skills):
        message = await skill_commands.build_skill_invocation_message("/triage")
    ...
    @pytest.mark.asyncio
    async def test_builder_round_trip_survives_failover_strip(self, skills):
        original = [
            {
                "role": "user",
                "content": await skill_commands.build_skill_invocation_message(
