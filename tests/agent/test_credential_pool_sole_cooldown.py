...
async def _load(tmp_path, monkeypatch, entries: list[dict]):
...
@pytest.mark.asyncio
async def test_sole_credential_429_recovers_after_short_cooldown(tmp_path, monkeypatch):
    ...
    """
    pool = await _load(tmp_path, monkeypatch, [_entry(429, age_seconds=90)])
...
@pytest.mark.asyncio
async def test_sole_credential_403_recovers_after_short_cooldown(tmp_path, monkeypatch):
    """403 (edge-throttle variant, hits the catch-all default TTL) also recovers."""
    pool = await _load(tmp_path, monkeypatch, [_entry(403, age_seconds=90)])
    entry = await pool.select()
    assert entry is not None
    assert entry.last_status == "ok"
...
@pytest.mark.asyncio
async def test_sole_credential_billing_403_keeps_full_bench(tmp_path, monkeypatch):
...
@pytest.mark.asyncio
async def test_sole_credential_billing_403_survives_reload(tmp_path, monkeypatch):
    ...
    assert (
        _exhausted_ttl(403, sole_credential=True, failure_reason="billing")
        == 60 * 60
    )
...
@pytest.mark.asyncio
async def test_sole_credential_402_keeps_full_bench(tmp_path, monkeypatch):
    """402 (billing/quota) is genuine exhaustion — a quick retry can't help, so
    the sole-credential short cooldown must NOT apply."""
    pool = await _load(tmp_path, monkeypatch, [_entry(402, age_seconds=90)])
    assert await pool.has_available() is False
    assert await pool.select() is None


@pytest.mark.asyncio
async def test_sole_credential_next_available_at_uses_short_cooldown(
    tmp_path,
    monkeypatch,
):
    ...
    pool = await _load(tmp_path, monkeypatch, [_entry(429, age_seconds=10)])
    next_at = await pool.next_available_at()
...
@pytest.mark.asyncio
async def test_multi_key_429_keeps_full_bench(tmp_path, monkeypatch):
