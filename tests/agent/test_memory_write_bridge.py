...
async def test_notifies_remove_with_old_text_after_success():
    mgr, provider = _manager_with_provider()
    await mgr.notify_memory_tool_write(
...
async def test_skips_unrecognized_tool_result_shape(tool_result):
    mgr, provider = _manager_with_provider()
    await mgr.notify_memory_tool_write(
        tool_result,
...
async def test_build_metadata_callback_is_merged_per_op():
    mgr, provider = _manager_with_provider()
    await mgr.notify_memory_tool_write(
        json.dumps({"success": True}),
