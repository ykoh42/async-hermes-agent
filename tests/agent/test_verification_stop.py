...
async def test_verify_on_stop_env_can_enable(clear_verify_env):
    ...
    assert (await verify_on_stop_enabled({"agent": {}})) is True
...
async def test_verify_on_stop_auto_on_for_interactive_surfaces(clear_verify_env, source):
    # Under "auto", CLI/TUI/desktop coding surfaces resolve ON.
    clear_verify_env.setenv("HERMES_SESSION_SOURCE", source)
    assert (
        await verify_on_stop_enabled({"agent": {"verify_on_stop": "auto"}})
    ) is True
...
async def test_nudge_checks_all_edited_workspaces(tmp_path, monkeypatch):
    ...
    await mark_workspace_edited(session_id="s1", cwd=project_b, paths=[changed_b])
...
async def test_ad_hoc_pass_satisfies_no_suite_stop_loop(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / ".hermes"))
    ...
    assert (
        await build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    ) is None


async def test_nudge_attempts_are_bounded(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / ".hermes"))
    _node_project(tmp_path)
    changed = str(tmp_path / "src" / "app.ts")
    await mark_workspace_edited(session_id="s1", cwd=tmp_path, paths=[changed])

    assert await build_verify_on_stop_nudge(
        session_id="s1",
...
async def test_mixed_doc_and_code_edit_still_nudges(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / ".hermes"))
    _node_project(tmp_path)
    doc = str(tmp_path / "README.md")
    code = str(tmp_path / "src" / "app.ts")
    await mark_workspace_edited(session_id="s1", cwd=tmp_path, paths=[code])
...
async def test_is_non_code_path_classification():
