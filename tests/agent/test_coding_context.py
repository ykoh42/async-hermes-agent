...
class TestIsCodingContext:
    ...
    async def test_auto_bare_git_repo_without_code_stays_general(self, tmp_path):
        ...
        assert await cc.is_coding_context(platform="cli", cwd=tmp_path, config=cfg) is False
        # …but adding a manifest or source file makes it a code workspace.
        (tmp_path / "pyproject.toml").write_text("[project]\nname='x'\n")
        assert await cc.is_coding_context(platform="cli", cwd=tmp_path, config=cfg) is True


    async def test_default_mode_is_auto(self, tmp_path):
        # Unknown/missing value normalizes to auto.
        _git_init(tmp_path)
        assert await cc.is_coding_context(platform="cli", cwd=tmp_path, config={}) is True


# ── toolset substitution ────────────────────────────────────────────────────

class TestCodingSelection:
    async def test_selects_coding_under_focus(self, tmp_path):
        _git_init(tmp_path)
        cfg = {"agent": {"coding_context": "focus"}}
        out = await cc.coding_selection(platform="cli", cwd=tmp_path, config=cfg)
        assert out is not None
        assert out[0] == cc.CODING_TOOLSET

    async def test_auto_is_prompt_only(self, tmp_path):
        ...
        _git_init(tmp_path)
        cfg = {"agent": {"coding_context": "auto"}}
        assert await cc.coding_selection(platform="cli", cwd=tmp_path, config=cfg) is None
...
class TestWorkspaceBlock:
    async def test_empty_outside_repo(self, tmp_path):
        assert await cc.build_coding_workspace_block(tmp_path) == ""

    async def test_reports_branch_and_clean_status(self, tmp_path):
        _git_init(tmp_path)
        block = await cc.build_coding_workspace_block(tmp_path)
    ...
    async def test_reports_dirty_counts(self, tmp_path):
        _git_init(tmp_path)
...
class TestProjectFacts:
    ...
    async def test_worktree_detected_without_primary_path(self, tmp_path):
        ...
        block = await cc.build_coding_workspace_block(worktree)
    ...
    async def test_detect_project_facts_structured(self, tmp_path):
        (tmp_path / "package.json").write_text(
            json.dumps({"scripts": {"test": "vitest", "dev": "vite"}})
        )
        (tmp_path / "pnpm-lock.yaml").write_text("")
        facts = await cc.detect_project_facts(tmp_path)
    ...
    async def test_project_facts_for_matches_prompt_block(self, tmp_path):
        ...
        (tmp_path / "pnpm-lock.yaml").write_text("")
        facts = await cc.project_facts_for(tmp_path)
        assert facts is not None
        verify_line = (await cc.build_coding_workspace_block(tmp_path)).split(
            "Verify:"
        )[1].splitlines()[0]
...
class TestHomeDotfilesGuard:
    async def test_dotfiles_repo_at_home_is_not_coding(self, tmp_path, monkeypatch):
        home = tmp_path / "home"
        home.mkdir()
        _git_init(home)
        monkeypatch.setattr(Path, "home", lambda: home)
        cfg = {"agent": {"coding_context": "auto"}}
        assert await cc.is_coding_context(platform="cli", cwd=home, config=cfg) is False
        # …and a plain subdirectory of the dotfiles repo stays general too.
        docs = home / "Documents"
        docs.mkdir()
        assert await cc.is_coding_context(platform="cli", cwd=docs, config=cfg) is False


    async def test_real_project_under_dotfiles_home_still_detects(self, tmp_path, monkeypatch):
        home = tmp_path / "home"
        home.mkdir()
        _git_init(home)
        monkeypatch.setattr(Path, "home", lambda: home)
        ...
        cfg = {"agent": {"coding_context": "auto"}}
        assert await cc.is_coding_context(platform="cli", cwd=proj, config=cfg) is True
...
class TestStatusParsing:
    async def test_parse_status_counts_and_branch(self):
        porcelain = (
            "# branch.head feature\n"
...
class TestRuntimeMode:
    async def test_resolves_coding_in_repo(self, tmp_path):
        _git_init(tmp_path)
        mode = await cc.resolve_runtime_mode(platform="cli", cwd=tmp_path, config={})
        assert mode.is_coding is True
        assert mode.kind == "coding"
        assert mode.profile is cc.CODING_PROFILE

    async def test_resolves_general_outside_workspace(self, tmp_path):
        mode = await cc.resolve_runtime_mode(platform="cli", cwd=tmp_path, config={})
    ...
    async def test_no_instructions_block_when_unset(self, tmp_path):
        _git_init(tmp_path)
        mode = await cc.resolve_runtime_mode(platform="cli", cwd=tmp_path, config={"agent": {"coding_context": "on"}})
        assert not any("Operator instructions" in b for b in await mode.system_blocks())
...
class TestEditFormatSteering:
    ...
    async def test_openai_family_gets_v4a_nudge(self, tmp_path):
        _git_init(tmp_path)
        mode = await cc.resolve_runtime_mode(
            platform="cli", cwd=tmp_path,
            config={"agent": {"coding_context": "on"}}, model="openai/gpt-5.4",
        )
        brief = (await mode.system_blocks())[0]
...
class TestProfiles:
    ...
    async def test_coding_profile_shape(self):
    ...
    async def test_skill_demotion_gated_on_focus(self, tmp_path):
        ...
        focus = await cc.resolve_runtime_mode(
            platform="cli", cwd=tmp_path,
        ...
        general = await cc.resolve_runtime_mode(platform="telegram", cwd=tmp_path, config={})
...
class TestDetection:
    ...
    async def test_project_manifest_triggers_without_git(self, tmp_path, marker):
        (tmp_path / marker).write_text("x")
        cfg = {"agent": {"coding_context": "auto"}}
        assert await cc.is_coding_context(platform="cli", cwd=tmp_path, config=cfg) is True

    async def test_marker_in_parent_counts_from_subdir(self, tmp_path):
        (tmp_path / "pyproject.toml").write_text("x")
        sub = tmp_path / "src" / "pkg"
        sub.mkdir(parents=True)
        cfg = {"agent": {"coding_context": "auto"}}
        assert await cc.is_coding_context(platform="cli", cwd=sub, config=cfg) is True

    async def test_bare_dir_is_not_coding(self, tmp_path):
        cfg = {"agent": {"coding_context": "auto"}}
        assert await cc.is_coding_context(platform="cli", cwd=tmp_path, config=cfg) is False
