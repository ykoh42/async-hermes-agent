...
@pytest.mark.asyncio
async def test_explicit_base_anthropic_messages_keeps_anthropic_path():
...
@pytest.mark.asyncio
async def test_explicit_base_anthropic_messages_fails_without_native_transport():
    """A Messages endpoint must fail clearly when its native async transport is
    unavailable; it must not silently fall back to a synchronous OpenAI wire."""
    from agent.auxiliary_client import resolve_provider_client, AnthropicAuxiliaryClient
    ...
    with (
        ...
        pytest.raises(ImportError, match="Anthropic async transport"),
    ):
...
@pytest.mark.asyncio
async def test_explicit_base_without_anthropic_mode_preserves_v1_rewrite():
...
@pytest.mark.asyncio
async def test_explicit_base_unknown_host_keeps_anthropic_path():
