...
@pytest.mark.asyncio
async def test_preset_resolution_is_cached_across_create_calls(monkeypatch, tmp_path):
    ...
    async def load_config_readonly():
        return _make_preset_config()

    async def fake_call_llm(**k):
        return _fake_response()

    monkeypatch.setattr(cfg_mod, "load_config_readonly", load_config_readonly)
    monkeypatch.setattr(moa, "call_llm", fake_call_llm)

    cc = moa.MoAChatCompletions("demo")
    for _ in range(3):
        await cc.create(messages=[{"role": "user", "content": "hi"}])
...
@pytest.mark.asyncio
async def test_preset_cache_invalidates_on_config_edit(monkeypatch, tmp_path):
...
@pytest.mark.asyncio
async def test_no_config_file_fails_open(monkeypatch, tmp_path):
...
@pytest.mark.asyncio
async def test_slot_runtime_is_cached_across_create_calls(monkeypatch, tmp_path):
...
@pytest.mark.asyncio
async def test_slot_runtime_cache_expires_after_ttl(monkeypatch):
    ...
    first = await moa._slot_runtime(slot)
    assert calls["n"] == 1 and first["api_key"] == "key-1"

    # Within TTL: cached.
    assert (await moa._slot_runtime(slot))["api_key"] == "key-1"
    assert calls["n"] == 1
    ...
    assert (await moa._slot_runtime(slot))["api_key"] == "key-2"
    assert calls["n"] == 2


@pytest.mark.asyncio
async def test_slot_runtime_resolution_error_is_not_cached(monkeypatch):
