...
class TestRuntimeResolution:
    ...
    def _stub_portal_credentials(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "load_pool",
            AsyncMock(
                return_value=SimpleNamespace(has_credentials=lambda: False)
            ),
        )
        monkeypatch.setattr(
            rp.auth_mod,
    ...
    async def test_target_model_wins_over_the_persisted_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            rp,
            "_get_model_config",
            lambda *_args: {"provider": "nous", "default": "hermes-4-405b"},
        )
...
class TestPoolRuntimeResolution:
    ...
    async def test_pool_entry_honors_the_model_derived_mode(self, monkeypatch, portal_entry):
        ...
        monkeypatch.setattr(
            rp,
            "load_pool",
            AsyncMock(return_value=self._pool(portal_entry)),
        )
        monkeypatch.setattr(
            rp,
            "_get_model_config",
    ...
    async def test_non_anthropic_catalog_model_stays_on_chat_completions(self):
        ...
        with (
            patch(
                ...
                new=AsyncMock(return_value=(plain, "hermes-4-405b")),
            ),
            patch(
