...
class _FakeProvider(TTSProvider):
    ...
    async def synthesize(self, text: str, output_path: str, **kw):
...
class TestABCContract:
    ...
    @pytest.mark.asyncio
    async def test_stream_raises_not_implemented_by_default(self):
        p = _FakeProvider(name="cartesia")
        with pytest.raises(NotImplementedError, match="does not implement streaming"):
            await anext(p.stream("hello"))
...
class TestBuiltinSync:
    ...
    def test_registry_builtins_match_dispatcher_builtins(self):
        tts_tool = pytest.importorskip("tools.tts_tool")
        BUILTIN_TTS_PROVIDERS = tts_tool.BUILTIN_TTS_PROVIDERS
