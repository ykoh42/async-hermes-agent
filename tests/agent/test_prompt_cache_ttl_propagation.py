...
class TestPlanCacheSectionsThreadsTtlAndPrefix:
    async def test_cache_ttl_1h_reaches_markers(self):
    ...
    async def test_static_system_prefix_gets_early_breakpoint(self):
        from agent.agent_runtime_helpers import plan_cache_sections_for_destination
    ...
    async def test_qwen_1h_clamped_to_5m(self):
        from agent.agent_runtime_helpers import plan_cache_sections_for_destination
...
class TestMoACacheControlThreadsTtl:
    async def test_moa_decoration_uses_threaded_1h(self):
    ...
    async def test_moa_qwen_1h_clamped_to_5m(self):
        from agent.moa_loop import _maybe_apply_moa_cache_control
    ...
    async def test_moa_decoration_defaults_to_5m_without_ttl(self):
        from agent.moa_loop import _maybe_apply_moa_cache_control
...
class TestAuxFallbackReplanThreadsTtl:
    ...
    async def test_configured_cache_ttl_reads_valid_tiers(self, monkeypatch):
        import agent.agent_runtime_helpers as arh

        async def _config_1h():
            return {"prompt_caching": {"cache_ttl": "1h"}}

        monkeypatch.setattr(
            "hermes_cli.config.load_config_readonly",
            _config_1h,
        )
        assert await arh.configured_cache_ttl() == "1h"

        async def _config_5m():
            return {"prompt_caching": {"cache_ttl": "5m"}}

        monkeypatch.setattr(
            "hermes_cli.config.load_config_readonly",
            _config_5m,
        )
        assert await arh.configured_cache_ttl() == "5m"

    async def test_configured_cache_ttl_none_for_disabled_or_unknown(self, monkeypatch):
        import agent.agent_runtime_helpers as arh

        for value in ("off", False, None, "2h"):
            async def _config(value=value):
                return {"prompt_caching": {"cache_ttl": value}}

            monkeypatch.setattr(
                "hermes_cli.config.load_config_readonly",
                _config,
            )
            assert await arh.configured_cache_ttl() is None, value

    async def test_replan_threads_configured_ttl_to_markers(self, monkeypatch):
        ...
        out_msgs, _ = await auxiliary_client._replan_fallback_cache_sections(
            messages,
            None,
