...
def _build_agent_with_db(db: SessionDB, session_id: str):
    ...
    async def _async_compress(*args, **kwargs):
        return _compress(*args, **kwargs)

    compressor.compress = AsyncMock(side_effect=_async_compress)
...
async def _seed_drifted_session(db: SessionDB, session_id: str):
    ...
    """
    await db.create_session(session_id, source="desktop")
    await db.append_message(session_id, "user", "persisted question")
    await db.append_message(session_id, "assistant", "persisted answer")

    loaded = await db.get_messages_as_conversation(session_id)
    ...
    await db.append_message(session_id, "assistant", "concurrent row 1")
    await db.append_message(session_id, "assistant", "concurrent row 2")

    assert len(await db.get_messages_as_conversation(session_id)) > len(messages)
    return agent, messages


@pytest.mark.asyncio
async def test_adoption_preserves_unpersisted_live_user_tail(tmp_path: Path) -> None:
    ...
    agent, messages = await _seed_drifted_session(db, "PREFLIGHT_ADOPT_PARENT")
...
@pytest.mark.asyncio
async def test_adoption_skipped_when_preflush_fails_keeps_live_input(
    ...
    """
    db = SessionDB(db_path=tmp_path / "state.db")
    agent, messages = await _seed_drifted_session(db, "PREFLIGHT_ADOPT_FLUSH_FAIL")
    ...
    async def _async_recording_compress(first_arg, **_kw):
        return _recording_compress(first_arg, **_kw)

    agent.context_compressor.compress = AsyncMock(side_effect=_async_recording_compress)

    async def _failing_flush(*_a, **_kw):
