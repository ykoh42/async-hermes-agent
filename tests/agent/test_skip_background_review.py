...
async def _run_finalize(agent: AIAgent) -> None:
...
@pytest.mark.asyncio
async def test_finalize_turn_skips_review_when_flag_set() -> None:
    ...
    await _run_finalize(agent)
    agent._spawn_background_review.assert_not_called()


@pytest.mark.asyncio
async def test_finalize_turn_fires_review_when_flag_unset() -> None:
...
def test_cron_construction_sets_skip_background_review() -> None:
    ...
    if not scheduler_src.exists():
        pytest.skip("cron scheduler is an intentionally removed surface")
