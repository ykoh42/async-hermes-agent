...
@pytest.mark.asyncio
async def test_key_rotation_clears_exhausted_status(tmp_path, monkeypatch):
    ...
    rotated = next(entry for entry in pool._entries if entry.id == "cred-1")
    assert rotated.access_token == "new-rotated-key"
    assert rotated.last_status is None, "last_status should be cleared after key rotation"
    assert rotated.last_status_at is None
    assert rotated.last_error_code is None
    assert rotated.last_error_reason is None
    assert rotated.last_error_message is None
    assert rotated.last_error_reset_at is None
