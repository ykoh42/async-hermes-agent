...
@pytest.mark.asyncio
async def test_binary_reference_block_maps_host_attachment_to_container_path(
    tmp_path: Path, monkeypatch,
):
    """Docker attachments use the staged path visible inside the container.

    This is the async/native equivalent of the upstream #76577 regression
    test: a gateway host path is not readable from a Docker tool process, so
    the prompt must advertise the existing cache mount path instead.
    """
...
@pytest.mark.asyncio
async def test_binary_reference_block_keeps_host_path_on_local_backend(
    tmp_path: Path, monkeypatch,
):
    """Local tools continue to receive the real host attachment path."""
    from agent.context_references import preprocess_context_references
