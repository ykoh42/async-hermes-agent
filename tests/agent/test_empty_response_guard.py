...
async def _record_streak(agent, responses, finish_reasons=None):
    ...
    for resp, reason in zip(responses, finish_reasons):
        await guard.record_empty_attempt(agent, finish_reason=reason, response=resp)
        agent._empty_content_retries += 1


class TestDeterministicEmpty:
    @pytest.mark.asyncio
    async def test_two_zero_output_attempts_same_signature_is_deterministic(self):
        agent = _agent()
        await _record_streak(agent, [_response(), _response()])
        assert guard.deterministic_empty(agent) is True

    @pytest.mark.asyncio
    async def test_single_attempt_is_never_deterministic(self):
        """One empty could be a transient blip — retry #1 must always run."""
        agent = _agent()
        await _record_streak(agent, [_response()])
        assert guard.deterministic_empty(agent) is False

    @pytest.mark.asyncio
    async def test_missing_usage_fails_open(self):
        agent = _agent()
        await _record_streak(
            agent,
    ...
    @pytest.mark.asyncio
    async def test_mixed_usage_presence_fails_open(self):
        agent = _agent()
        await _record_streak(agent, [_response(), _response(usage_present=False)])
        assert guard.deterministic_empty(agent) is False

    @pytest.mark.asyncio
    async def test_nonzero_output_tokens_fails_open(self):
    ...
    @pytest.mark.asyncio
    async def test_zero_then_nonzero_fails_open(self):
        agent = _agent()
        await _record_streak(
            agent,
    ...
    @pytest.mark.asyncio
    async def test_signature_change_resets_determinism(self):
        """Fallback switched model mid-streak — new model deserves retries."""
        agent = _agent()
        await guard.record_empty_attempt(agent, finish_reason="stop", response=_response())
        agent._empty_content_retries += 1
    ...
    @pytest.mark.asyncio
    async def test_finish_reason_change_fails_open(self):
        agent = _agent()
        await _record_streak(
            agent,
            [_response(), _response()],
    ...
    @pytest.mark.asyncio
    async def test_new_streak_clears_history(self):
    ...
    @pytest.mark.asyncio
    async def test_reasoning_tokens_count_as_generation(self, monkeypatch):
...
class TestEmptyRetryBudget:
    @pytest.mark.asyncio
    async def test_default_budget_when_cost_unknown(self, monkeypatch):
        monkeypatch.setattr(guard, "_estimate_attempt_cost", lambda a, r: None)
        assert (
            await guard.empty_retry_budget(_agent(), _response())
            == guard.DEFAULT_EMPTY_RETRY_BUDGET
        )

    @pytest.mark.asyncio
    async def test_reduced_budget_above_threshold(self, monkeypatch):
        monkeypatch.setattr(
    ...
    @pytest.mark.asyncio
    async def test_default_budget_below_threshold(self, monkeypatch):
        monkeypatch.setattr(
    ...
    @pytest.mark.asyncio
    async def test_custom_threshold_config(self, monkeypatch):
        monkeypatch.setattr(
            guard, "_estimate_attempt_cost", lambda a, r: Decimal("0.80")
        )
        assert (
            await guard.empty_retry_budget(
    ...
    @pytest.mark.asyncio
    async def test_guard_disabled_keeps_default_budget(self, monkeypatch):
        monkeypatch.setattr(
            guard, "_estimate_attempt_cost", lambda a, r: Decimal("9.99")
        )
        assert (
            await guard.empty_retry_budget(_agent(_empty_guard_enabled=False), _response())
            == guard.DEFAULT_EMPTY_RETRY_BUDGET
        )

    @pytest.mark.asyncio
    async def test_pricing_exception_fails_open(self, monkeypatch):
        def _boom(a, r):
        ...
        assert (
            await guard.empty_retry_budget(agent, resp)
            == guard.DEFAULT_EMPTY_RETRY_BUDGET
        )
...
class TestStreakCost:
    @pytest.mark.asyncio
    async def test_streak_cost_accumulates(self, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_streak_cost_none_when_unknown(self, monkeypatch):
        monkeypatch.setattr(guard, "_estimate_attempt_cost", lambda a, r: None)
        agent = _agent()
        await _record_streak(agent, [_response(), _response()])
    ...
    @pytest.mark.asyncio
    async def test_streak_cost_resets_on_new_streak(self, monkeypatch):
        monkeypatch.setattr(
