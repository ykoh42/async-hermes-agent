...
class TestRunReferenceSlotMaxTokens:
    ...
    @pytest.mark.asyncio
    async def test_slot_max_tokens_overrides_preset_level(self):
        ...
        async def passthrough(messages, runtime, **kwargs):
            return messages
        ...
        with patch("agent.moa_loop._slot_runtime", return_value={"provider": "openrouter", "model": "deepseek/deepseek-v4-pro"}), \
             ...
             patch("agent.moa_loop._maybe_apply_moa_cache_control", side_effect=passthrough):
            await _run_reference(slot, [{"role": "user", "content": "hi"}], max_tokens=2000)

        assert captured_kwargs.get("max_tokens") == 600

    @pytest.mark.asyncio
    async def test_slot_max_tokens_absent_falls_back_to_preset(self):
    ...
    @pytest.mark.asyncio
    async def test_both_none_means_uncapped(self):
        ...
        with patch("agent.moa_loop._slot_runtime", return_value={"provider": "openrouter", "model": "deepseek/deepseek-v4-pro"}), \
             patch("agent.moa_loop.call_llm", side_effect=fake_call_llm), \
             patch("agent.moa_loop._maybe_apply_moa_cache_control", side_effect=passthrough):
            await _run_reference(slot, [{"role": "user", "content": "hi"}], max_tokens=None)
