...
@pytest.mark.asyncio
async def test_write_file_skips_lsp_when_syntax_failed(tmp_path):
    ...
    with patch.object(fops, "_maybe_lsp_diagnostics") as mock_lsp:
        res = await fops.write_file(str(target), "def x(:\n")  # syntax error
...
@pytest.mark.asyncio
async def test_patch_replace_propagates_lsp_diagnostics(tmp_path):
    ...
    with patch.object(fops, "_maybe_lsp_diagnostics", return_value=block):
        res = await fops.patch_replace(str(target), "x = 1", "x = 2")
