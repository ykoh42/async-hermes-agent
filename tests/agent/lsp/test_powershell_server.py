...
@pytest.mark.asyncio
async def test_spawn_builds_command_with_bundle_via_env(monkeypatch, tmp_path):
    async def which(*_names):
        return "/usr/bin/pwsh"
    ...
    spec = await srv._spawn_powershell_es(str(tmp_path), ctx)
...
@pytest.mark.asyncio
async def test_bundle_path_init_override_not_leaked_into_init_options(monkeypatch, tmp_path):
    async def which(*_names):
        return "/usr/bin/pwsh"
