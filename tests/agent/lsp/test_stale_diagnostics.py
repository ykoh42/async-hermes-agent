...
@pytest.mark.asyncio
async def test_service_reports_no_data_not_stale_errors(stale_repo):
    ...
    try:
        ...
        first = await svc.get_diagnostics_sync(str(f), delta=False)
        assert len(first) == 1

        # Edit the file — mock never re-publishes (slow tsserver model).
        f.write_text("good code\n")
        ghost = await svc.get_diagnostics_sync(str(f), delta=False)
