...
@pytest.mark.asyncio
async def test_shell_linter_skipped_when_lsp_will_handle(ext, tmp_path):
    ...
    with patch.object(fops, "_lsp_will_handle", return_value=True), \
        ...
        result = await fops._check_lint(str(src))
...
@pytest.mark.asyncio
async def test_lsp_will_handle_swallows_enabled_for_exception(tmp_path):
    ...
    fake_svc = MagicMock()
    fake_svc.enabled_for = AsyncMock()
    fake_svc.enabled_for.side_effect = RuntimeError("server crashed")

    with patch.object(fops, "_lsp_local_only", return_value=True), \
         patch("agent.lsp.get_service", new=AsyncMock(return_value=fake_svc)):
        assert await fops._lsp_will_handle(str(src)) is False




@pytest.mark.asyncio
async def test_tsx_default_check_lint_returns_skipped(tmp_path):
