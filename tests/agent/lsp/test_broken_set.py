...
@pytest.mark.asyncio
async def test_unrelated_project_not_affected_by_broken(tmp_path, monkeypatch):
    ...
    try:
        await svc._mark_broken_for_file(str(a_src), RuntimeError("simulated"))
        # Project A skipped.
        assert await svc.enabled_for(str(a_src)) is False
        # Project B still enabled — the broken key is per-project.
        monkeypatch.chdir(str(repo_b))
        assert await svc.enabled_for(str(b_src)) is True
    finally:
        await svc.shutdown()
...
@pytest.mark.asyncio
async def test_mark_broken_handles_no_workspace_silently(tmp_path):
    ...
    try:
        await svc._mark_broken_for_file(str(src), RuntimeError("x"))
...
@pytest.mark.asyncio
async def test_snapshot_failure_marks_broken_via_outer_timeout(tmp_path, monkeypatch):
    ...
    try:
        ...
        with patch.object(svc, "_snapshot_async", boom):
            assert await svc.enabled_for(str(src)) is True
            await svc.snapshot_baseline(str(src))

        # After the failure, the file's pair is in the broken-set and
        # ``enabled_for`` skips it.
        assert ("pyright", str(repo)) in svc._broken
        assert await svc.enabled_for(str(src)) is False
    finally:
        await svc.shutdown()
