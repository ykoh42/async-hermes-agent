...
@pytest.mark.asyncio
async def test_maybe_lsp_diagnostics_returns_empty_for_non_local(monkeypatch):
    ...
    class FakeService:
        async def enabled_for(self, path):
            called.append(("enabled_for", path))
            return True
        async def get_diagnostics_sync(self, path, **kw):
    ...
    result = await fops._maybe_lsp_diagnostics("/sandbox/x.py")
    assert result == ""
    assert called == [], "service must not be queried for non-local backends"


@pytest.mark.asyncio
async def test_snapshot_baseline_called_for_local_env(tmp_path, monkeypatch):
    ...
    class FakeService:
        async def snapshot_baseline(self, path):
    ...
    await fops._snapshot_lsp_baseline(str(tmp_path / "x.py"))
