...
@pytest.mark.asyncio
async def test_real_aiagent_builds_section_once_and_keeps_it_out_of_static_prefix(monkeypatch):
    calls = []
    ...
    first = await build_system_prompt(agent)
    await invalidate_system_prompt(agent)
    rebuilt = await build_system_prompt(agent)
...
def test_fresh_process_resume_restores_identical_full_prompt_without_callback(tmp_path):
    ...
    child = textwrap.dedent(
        ...
        async def main():
            ...
            await db.ensure_session(session_id, source="cli", model="test/model")
            agent = AIAgent(
                api_key="test-key",
                base_url="https://openrouter.ai/api/v1",
                model="test/model",
