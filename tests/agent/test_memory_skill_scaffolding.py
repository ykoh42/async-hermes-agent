...
class _RecordingProvider(MemoryProvider):
    ...
    async def prefetch(self, query, *, session_id: str = "") -> str:
        self.prefetched.append(query)
        return ""

    async def queue_prefetch(self, query, *, session_id: str = "") -> None:
        self.queued.append(query)

    async def sync_turn(self, user_content, assistant_content, *, session_id: str = "", messages=None) -> None:
...
class TestExtractUserInstruction:
    ...
    async def test_bundle_with_instruction(self):
        assert (
...
class TestMemoryManagerStripsScaffolding:
    ...
    async def test_prefetch_all_skips_bare_skill(self):
        mgr, provider = _manager_with_recorder()
        result = await mgr.prefetch_all(_BARE_SKILL_TURN)
        assert result == ""
        assert provider.prefetched == []

    async def test_queue_prefetch_all_strips_bundle(self):
        mgr, provider = _manager_with_recorder()
        await mgr.queue_prefetch_all(_BUNDLE_TURN)
        await mgr.flush_pending(timeout=5.0)
        assert provider.queued == ["fix the failing retrieval test"]



    async def test_sync_all_skips_bare_skill(self):
        mgr, provider = _manager_with_recorder()
        await mgr.sync_all(_BARE_SKILL_TURN, "Done.")
        await mgr.flush_pending(timeout=5.0)
