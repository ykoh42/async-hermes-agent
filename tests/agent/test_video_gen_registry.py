...
class TestGetActiveProvider:
    ...
    @pytest.mark.asyncio
    async def test_single_available_among_many_autoresolves(self, tmp_path, monkeypatch):
        ...
        active = await video_gen_registry.get_active_provider()
        assert active is not None and active.name == "deepinfra"


    @pytest.mark.asyncio
    async def test_unknown_explicit_config_fails_closed(self, tmp_path, monkeypatch):
        ...
        assert await video_gen_registry.get_active_provider() is None
