...
def test_quarantine_paths_hold_the_pool_lock():
    """The native refresh path must guard terminal-entry read-modify-write.

    Upstream uses a synchronous ``_refresh_entry_impl`` seam.  The retained
    fork has one awaited ``_refresh_entry`` instead, so verify its lock-aware
    quarantine helper rather than requiring the removed synchronous private
    method.
    """
    import inspect
    import textwrap

    target = getattr(CredentialPool, "_refresh_entry_impl", None)
    if target is None:
        src = textwrap.dedent(inspect.getsource(CredentialPool._refresh_entry))
        assert "_quarantine_entries_async" in src
        assert "lock_held=lock_held" in src
        return
