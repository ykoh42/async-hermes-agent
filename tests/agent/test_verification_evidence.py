...
async def test_lint_and_typecheck_are_not_reported_as_full_tests(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / ".hermes"))
    _node_project(tmp_path)

    lint = await classify_verification_command(
        "pnpm run lint",
        cwd=tmp_path,
        session_id="s1",
        exit_code=0,
    )
    test = await classify_verification_command(
...
async def test_shell_wrappers_match_but_echo_does_not(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / ".hermes"))
    _node_project(tmp_path)

    wrapped = await classify_verification_command(
        "env CI=1 bash scripts/run_tests.sh tests/test_widget.py",
        cwd=tmp_path,
        session_id="s1",
        exit_code=0,
    )
    echoed = await classify_verification_command(
...
async def test_file_tool_stales_evidence_by_session_id_for_absolute_edit(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / ".hermes"))
    _node_project(tmp_path)
    ...
    assert (await verification_status(session_id="conversation", cwd=tmp_path))["status"] == "stale"
    assert (await verification_status(session_id="turn", cwd=tmp_path))["status"] == "unverified"
...
async def test_recording_expires_old_edit_only_state(tmp_path, monkeypatch):
...
async def test_windows_backslash_ad_hoc_script_path_is_matched(tmp_path, monkeypatch):
    ...
    async def mock_is_temp_script(token, root):
    ...
    result = await _find_ad_hoc_match(f"python {win_script}", tmp_path)
