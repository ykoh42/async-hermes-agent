...
class FakeClient:
    ...
    async def respond_error(self, request_id, code, message, data=None):
        self.error_responses.append((request_id, code, message))

    async def take_notification(self, timeout: float = 0.0):
    ...
    async def take_server_request(self, timeout: float = 0.0):
...
class TestApprovalChoiceMapping:
    ...
    async def test_mapping(self, choice, expected):
        assert _approval_choice_to_codex_decision(choice) == expected


class TestTurnInputCoercion:
    async def test_list_content_keeps_text_and_marks_images(self):
...
class TestLifecycle:
    async def test_ensure_started_is_idempotent(self):
        ...
        tid_a = await s.ensure_started()
        tid_b = await s.ensure_started()
        assert tid_a == tid_b == "thread-fake-001"
        # thread/start should be called exactly once
        method_calls = [m for (m, _) in client.requests if m == "thread/start"]
        assert len(method_calls) == 1

    async def test_thread_start_passes_cwd_only(self):
...
class TestRunTurn:
    async def test_simple_text_turn_returns_final_message(self):
        client = FakeClient()
        ...
        s = make_session(client)
        r = await s.run_turn("hi", turn_timeout=2.0)
    ...
    async def test_foreign_completion_in_server_request_drain_is_ignored(self):
        ...
        async def respond_and_release_parent(request_id, response):
            await original_respond(request_id, response)
            client.queue_notification(
                "item/completed",
        ...
        result = await session.run_turn("delegate then continue", turn_timeout=2.0)
    ...
    async def test_tool_iteration_counter_ticks(self):
        client = FakeClient()
        ...
        s = make_session(client)
        r = await s.run_turn("do stuff", turn_timeout=2.0)
    ...
    async def test_turn_start_failure_attaches_redacted_stderr_tail(self):
    ...
    async def test_turn_start_timeout_attaches_redacted_stderr_tail(self):
    ...
    async def test_steer_appends_input_to_active_turn(self):
        client = FakeClient()
        s = make_session(client)
        await s.ensure_started()
        async with s._active_turn_lock:
            s._active_turn_id = "turn-live-123"

        assert await s.request_steer("Use Postgres instead") is True
...
class TestCompactThread:
    async def test_compact_thread_sends_rpc_and_waits_for_completion(self):
        client = FakeClient()
        client.queue_notification(
            "turn/started",
            threadId="thread-fake-001",
        ...
        r = await make_session(client).compact_thread(turn_timeout=2.0)
    ...
    async def test_compact_thread_ignores_foreign_child_completion(self):
        client = FakeClient()
        client.queue_notification(
            "turn/started",
            threadId="thread-child-001",
        ...
        result = await make_session(client).compact_thread(turn_timeout=2.0)
...
class TestServerRequestRouting:
    ...
    async def test_unknown_server_request_replied_with_error(self):
        client = FakeClient()
        ...
        s = make_session(client)
        await s.run_turn("hi", turn_timeout=1.0)
        assert any(
            rid == "req-3" and code == -32601
            for (rid, code, _msg) in client.error_responses
        )

    async def test_on_event_fires_during_approval_drain(self):
    ...
    async def test_routing_auto_approve_bypass(self):
        client = FakeClient()
...
class TestApprovalPromptEnrichment:
    ...
    async def test_exec_falls_back_to_session_cwd(self):
    ...
    async def test_apply_patch_prompt_summarizes_pending_changes(self):
    ...
    async def test_apply_patch_prompt_works_without_cached_summary(self):
...
class TestSessionRetirement:
    ...
    async def test_final_agent_message_without_turn_completed_is_recovered(self):
    ...
    async def test_post_tool_watchdog_uses_monotonic_clock(self):
        client = FakeClient()
        client.queue_notification(
            "item/completed",
            item={
    ...
    async def test_post_tool_watchdog_resets_on_further_activity(self):
    ...
    async def test_dead_subprocess_detected_between_iterations(self):
        ...
        r = await s.run_turn("x", turn_timeout=2.0,
                       notification_poll_timeout=0.01)
        assert r.should_retire is True
...
class TestThreadStartCrossFill:
    ...
    async def test_thread_id_under_thread_key(self):
        client = FakeClient()
        s = make_session(client)
        tid = await s.ensure_started()
...
class TestClassifyOAuthFailure:
    ...
    async def test_401_classified(self):
        from agent.transports.codex_app_server_session import (
