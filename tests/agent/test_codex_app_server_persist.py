...
def _make_agent(session_db=None, session_id="sess-codex"):
    ...
    agent._codex_session.run_turn = AsyncMock(return_value=_make_turn())
    agent._codex_session.close = AsyncMock()
    agent._sync_external_memory_for_turn = AsyncMock()
...
@pytest.mark.asyncio
async def test_codex_success_flushes_and_reports_persisted():
    ...
    result = await run_codex_app_server_turn(
        agent,
        user_message="hello",
...
@pytest.mark.asyncio
async def test_codex_user_interrupt_is_reported_and_cleared():
...
async def test_codex_turn_persists_each_message_exactly_once():
    ...
    try:
        ...
        sid = "sess-codex-once"
        await db.create_session(session_id=sid, source="telegram", model="codex")
        ...
        messages = [user_msg]
        await agent._flush_messages_to_session_db(messages)  # turn-start flush
        ...
        rows = await db.get_messages(sid, include_inactive=True)
        ...
        hits = {
            r["session_id"]
            for r in await db.search_messages("CODEX_ASSISTANT")
        }
        assert sid in hits
    finally:
        if "agent" in locals():
