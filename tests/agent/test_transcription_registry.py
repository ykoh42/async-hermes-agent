...
class _FakeProvider(TranscriptionProvider):
    ...
    async def transcribe(self, file_path: str, **kw):
...
class TestABCContract:
    ...
    def test_must_implement_name(self):
        class Incomplete(TranscriptionProvider):
            async def transcribe(self, file_path, **kw):
...
class TestBuiltinSync:
    ...
    def test_registry_builtins_match_dispatcher_builtins(self):
        transcription_tools = pytest.importorskip("tools.transcription_tools")
        BUILTIN_STT_PROVIDERS = transcription_tools.BUILTIN_STT_PROVIDERS
