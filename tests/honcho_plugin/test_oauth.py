...
class TestTokenDetection:
    async def test_access_token_prefix(self):
...
class TestCredentialModel:
    ...
    async def test_incomplete_block_returns_none(self):
    ...
    async def test_is_expired_respects_skew(self):
...
class TestEnsureFreshToken:
    async def test_no_oauth_credential_is_noop(self, tmp_path):
        path = tmp_path / "honcho.json"
        _write(path, {"hosts": {"hermes": {"apiKey": "hch-v3-static"}}})
        token, refreshed = await oauth.ensure_fresh_token(path, "hermes", now=0)
        assert token is None and refreshed is False

    async def test_fresh_token_skips_refresh(self, tmp_path, monkeypatch):
        path = tmp_path / "honcho.json"
    ...
    async def test_expired_token_refreshes_and_persists_rotation(self, tmp_path, monkeypatch):
        path = tmp_path / "honcho.json"
        ...
        token, refreshed = await oauth.ensure_fresh_token(path, "hermes", now=1000)
    ...
    async def test_refresh_failure_fails_open(self, tmp_path, monkeypatch):
        path = tmp_path / "honcho.json"
        _write(path, {"hosts": {"hermes": _host_block(expires_at=100)}})
    ...
    async def test_double_check_uses_disk_when_already_rotated(self, tmp_path, monkeypatch):
        ...
        token, refreshed = await oauth.ensure_fresh_token(path, "hermes", stale_raw, now=1000)
        assert token == "hch-at-old"  # the on-disk fresh credential's access token


class TestInstallGrant:
    async def test_deep_merges_config_and_preserves_other_hosts(self, tmp_path):
        path = tmp_path / "honcho.json"
        _write(path, {
        ...
        cred = await oauth.install_grant(
            path, "hermes", grant,
    ...
    async def test_rejects_grant_without_tokens(self, tmp_path):
        path = tmp_path / "honcho.json"
        _write(path, {})
        with pytest.raises(ValueError):
            await oauth.install_grant(
...
class TestApplyTokenToClient:
    async def test_mutates_live_bearer(self):
        class FakeHttp:
            api_key = "hch-at-old"

        class FakeClient:
            _async_http = FakeHttp()

        client = FakeClient()
        assert oauth.apply_token_to_client(client, "hch-at-new") is True
        assert client._async_http.api_key == "hch-at-new"

    async def test_returns_false_when_shape_unknown(self):
