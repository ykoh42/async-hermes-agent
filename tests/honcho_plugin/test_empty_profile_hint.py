...
def _make_provider(**cfg_overrides) -> HonchoMemoryProvider:
    ...
    provider._manager.get_peer_card = AsyncMock(return_value=[])
...
class TestEmptyProfileHint:
    @pytest.mark.asyncio
    async def test_returns_hint_not_bare_error_message(self):
        provider = _make_provider()
        raw = await provider.handle_tool_call("honcho_profile", {})
    ...
    @pytest.mark.asyncio
    async def test_hint_mentions_warmup_when_turn_count_below_cadence(self):
    ...
    @pytest.mark.asyncio
    async def test_populated_card_returns_card_without_hint(self):
