...
class TestPinPeerNameConfigParsing:
    ...
    async def test_root_level_true(self, tmp_path, monkeypatch):
    ...
    async def test_host_block_true(self, tmp_path, monkeypatch):
    ...
    async def test_explicit_false_parses(self, tmp_path, monkeypatch):
        config_file = tmp_path / "honcho.json"
        config_file.write_text(json.dumps({
            "apiKey": "k",
            "peerName": "Igor",
...
class TestRuntimePeerMappingConfigParsing:
    async def test_defaults_are_empty(self):
        config = HonchoClientConfig()
        assert config.user_peer_aliases == {}
        assert config.runtime_peer_prefix == ""


    async def test_malformed_alias_config_is_ignored(self, tmp_path):
        config_file = tmp_path / "honcho.json"
        config_file.write_text(json.dumps({
            "apiKey": "k",
...
def _patch_manager_for_resolution_test(mgr: HonchoSessionManager) -> None:
    ...
    """
    fake_peer = MagicMock()
    mgr._get_or_create_peer = AsyncMock(return_value=fake_peer)
    mgr._get_or_create_honcho_session = AsyncMock(
...
class TestPeerResolutionOrder:
    ...
    async def test_runtime_wins_when_pin_is_false(self):
        ...
        session = await mgr.get_or_create("telegram:7654321")
    ...
    async def test_alias_wins_for_known_runtime_id(self):
    ...
    async def test_unknown_runtime_id_uses_prefix(self):
    ...
    async def test_prefixed_runtime_id_hashes_when_sanitization_is_lossy(self):
        ...
        session = await mgr.get_or_create("telegram:user:42")
        assert session.user_peer_id == f"telegram_user-42-{expected_hash}"

    async def test_prefixed_runtime_id_hashes_when_it_collides_with_peer_name(self):
    ...
    async def test_alias_value_is_sanitized_after_selection(self):
        mgr = HonchoSessionManager(
            honcho=MagicMock(),
            config=self._config(
                peer_name=None,
    ...
    async def test_alias_keys_match_raw_runtime_id_before_sanitization(self):
    ...
    async def test_session_peer_prefix_is_orthogonal_to_runtime_peer_prefix(self):
    ...
    async def test_config_wins_when_pin_is_true(self):
    ...
    async def test_pin_noop_when_peer_name_missing(self):
    ...
    async def test_alt_runtime_id_can_match_alias_without_changing_raw_fallback(self):
        ...
        session = await mgr.get_or_create("feishu:chat")
        assert session.user_peer_id == "Igor"


    async def test_everything_missing_falls_back_to_session_key(self):
        ...
        session = await mgr.get_or_create("telegram:123")
...
class TestCrossPlatformMemoryUnification:
    ...
    async def test_telegram_and_discord_collapse_to_one_peer_when_pinned(self):
        ...
        telegram_session = await mgr_telegram.get_or_create("telegram:7654321")
        ...
        discord_session = await mgr_discord.get_or_create("discord:1348750102029926454")
    ...
    async def test_multiuser_default_keeps_platforms_separate(self):
        ...
        sess_a = await mgr_a.get_or_create("telegram:a")
        sess_b = await mgr_b.get_or_create("telegram:b")
...
class TestPinUserPeerAlias:
    ...
    async def test_pinPeerName_still_works_unchanged(self, tmp_path):
        from plugins.memory.honcho.client import HonchoClientConfig
        import json
        config_file = tmp_path / "honcho.json"
        config_file.write_text(json.dumps({
...
class TestPinTransition:
    ...
    async def test_fresh_manager_after_flip_resolves_to_runtime(self):
        ...
        before = await pinned_mgr.get_or_create("telegram:7654321")
        ...
        after = await unpinned_mgr.get_or_create("telegram:7654321")
    ...
    async def test_cached_session_survives_config_flip_in_same_manager(self):
        mgr = HonchoSessionManager(
            honcho=MagicMock(),
            config=self._pinned(),
            runtime_user_peer_name="7654321",
        ...
        _patch_manager_for_resolution_test(mgr)
        first = await mgr.get_or_create("telegram:7654321")
        assert first.user_peer_id == "Igor"

        mgr._config = self._unpinned()
        second = await mgr.get_or_create("telegram:7654321")
...
class TestProfilePeerUniqueness:
    ...
    async def test_two_profiles_pinned_to_different_peer_names_resolve_distinctly(self):
        mgr_a = HonchoSessionManager(
            honcho=MagicMock(),
        ...
        _patch_manager_for_resolution_test(mgr_a)
        sess_a = await mgr_a.get_or_create("telegram:7654321")
        ...
        _patch_manager_for_resolution_test(mgr_b)
        sess_b = await mgr_b.get_or_create("telegram:7654321")
