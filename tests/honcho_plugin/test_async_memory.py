...
async def make_manager(monkeypatch):
    ...
    client = MagicMock()
    monkeypatch.setattr(session_module, "get_honcho_client", AsyncMock(return_value=client))
    created = []
...
class TestWriteFrequencyParsing:
    async def test_string_async(self, tmp_path):
    ...
    async def test_integer_frequency(self, tmp_path):
        cfg_file = tmp_path / "config.json"
    ...
    async def test_defaults_to_async(self, tmp_path):
        cfg_file = tmp_path / "config.json"
...
class TestResolveSessionNameTitle:
    async def test_manual_override_beats_title(self):
        cfg = HonchoClientConfig(sessions={"/my/project": "manual-name"})
        result = await cfg.resolve_session_name("/my/project", session_title="the-title")
        assert result == "manual-name"

    async def test_title_beats_dirname(self):
        cfg = HonchoClientConfig()
        result = await cfg.resolve_session_name("/some/dir", session_title="my-project")
        assert result == "my-project"


    async def test_title_sanitized(self):
        cfg = HonchoClientConfig()
        result = await cfg.resolve_session_name("/some/dir", session_title="my project/name!")
        # trailing dashes stripped by .strip('-')
        assert result == "my-project-name"


    async def test_none_title_falls_back_to_dirname(self):
        cfg = HonchoClientConfig()
        result = await cfg.resolve_session_name("/some/dir", session_title=None)
        assert result == "dir"

    async def test_empty_title_falls_back_to_dirname(self):
        cfg = HonchoClientConfig()
        result = await cfg.resolve_session_name("/some/dir", session_title="")
        assert result == "dir"

    async def test_per_session_uses_session_id(self):
        cfg = HonchoClientConfig(session_strategy="per-session")
        result = await cfg.resolve_session_name("/some/dir", session_id="20260309_175514_9797dd")
        assert result == "20260309_175514_9797dd"


    async def test_gateway_key_beats_per_session_id(self):
        # Gateways keep per-chat isolation even in per-session.
        cfg = HonchoClientConfig(session_strategy="per-session")
        result = await cfg.resolve_session_name("/some/dir", gateway_session_key="agent:main:telegram:dm:42", session_id="20260309_175514_9797dd")
        assert result == "agent-main-telegram-dm-42"

    async def test_global_strategy_returns_workspace(self):
        cfg = HonchoClientConfig(session_strategy="global", workspace_id="my-workspace")
        result = await cfg.resolve_session_name("/some/dir")
...
class TestSaveRouting:
    ...
    async def test_turn_flushes_immediately(self, make_manager):
        mgr = make_manager(write_frequency="turn")
        sess = self._make_session_with_message(mgr)
        with patch.object(mgr, "_flush_session", new_callable=AsyncMock) as mock_flush:
            await mgr.save(sess)
            mock_flush.assert_awaited_once_with(sess)

    async def test_session_mode_does_not_flush(self, make_manager):
    ...
    async def test_async_mode_enqueues(self, make_manager):
    ...
    async def test_int_frequency_flushes_on_nth_turn(self, make_manager):
        mgr = make_manager(write_frequency=3)
        sess = self._make_session_with_message(mgr)
        with patch.object(mgr, "_flush_session", new_callable=AsyncMock) as mock_flush:
            await mgr.save(sess)  # turn 1
            await mgr.save(sess)  # turn 2
            assert mock_flush.await_count == 0
            await mgr.save(sess)  # turn 3
            assert mock_flush.await_count == 1

    async def test_int_frequency_skips_other_turns(self, make_manager):
...
class TestFlushAll:
    async def test_flushes_all_cached_sessions(self, make_manager):
        mgr = make_manager(write_frequency="session")
    ...
    async def test_flush_all_drains_async_queue(self, make_manager):
        mgr = make_manager(write_frequency="async")
        sess = _make_session()
        ...
        with patch.object(mgr, "_flush_session", new_callable=AsyncMock) as mock_flush:
            ...
            await mgr._async_queue.put(sess)
            await mgr._ensure_async_writer()
            await mgr.flush_all()
            # Called at least once for the queued item
            assert mock_flush.await_count >= 1

    async def test_flush_all_tolerates_errors(self, make_manager):
        mgr = make_manager(write_frequency="session")
        sess = _make_session()
        mgr._cache = {"key": sess}
        with patch.object(
...
class TestAsyncWriterTask:
    async def test_task_starts_lazily_on_first_enqueue(self, make_manager):
        ...
        await mgr.save(_make_session())
        assert mgr._async_task is not None
        assert not mgr._async_task.done()
        await mgr.shutdown()

    async def test_no_task_for_turn_mode(self, make_manager):
        mgr = make_manager(write_frequency="turn")
        assert mgr._async_task is None
        assert mgr._async_queue is None

    async def test_shutdown_joins_task(self, make_manager):
        mgr = make_manager(write_frequency="async")
        await mgr._ensure_async_writer()
    ...
    async def test_async_writer_calls_flush(self, make_manager):
        mgr = make_manager(write_frequency="async")
        await mgr._ensure_async_writer()
        sess = _make_session()
    ...
    async def test_shutdown_sentinel_stops_loop(self, make_manager):
        mgr = make_manager(write_frequency="async")
        await mgr._ensure_async_writer()
        task = mgr._async_task
        await mgr.shutdown()
    ...
    async def test_shutdown_without_started_task_is_noop(self, make_manager):
        mgr = make_manager(write_frequency="async")
        await mgr.shutdown()
        assert mgr._async_task is None

    async def test_cancelled_shutdown_cancels_owned_writer(self, make_manager):
        mgr = make_manager(write_frequency="async")
        sess = _make_session()
        sess.add_message("user", "pending")
        ...
        async def blocked_flush(session):
            flush_started.set()
        ...
        assert writer_task is not None and writer_task.done()
        assert mgr._async_task is None
...
class TestAsyncWriterRetry:
    async def test_retries_once_on_failure(self, make_manager):
        mgr = make_manager(write_frequency="async")
        await mgr._ensure_async_writer()
        sess = _make_session()
    ...
    async def test_drops_after_two_failures(self, make_manager):
        mgr = make_manager(write_frequency="async")
        await mgr._ensure_async_writer()
        sess = _make_session()
        sess.add_message("user", "msg")
    ...
    async def test_retries_when_flush_reports_failure(self, make_manager):
        mgr = make_manager(write_frequency="async")
        await mgr._ensure_async_writer()
        sess = _make_session()
        sess.add_message("user", "msg")
        ...
        async def fail_then_succeed(session):
            call_count[0] += 1
            if call_count[0] >= 2:
                retry_done.set()
...
class TestMemoryFileMigrationTargets:
    async def test_soul_upload_targets_ai_peer(self, tmp_path, make_manager):
        mgr = make_manager(write_frequency="turn")
        ...
        honcho_session.aio.upload_file = AsyncMock()
        ...
        uploaded = await mgr.migrate_memory_files(session.key, str(tmp_path))

        assert uploaded is True
        assert honcho_session.aio.upload_file.await_count == 3

        peer_by_upload_name = {}
        for call_args in honcho_session.aio.upload_file.await_args_list:
...
class TestNewConfigFieldDefaults:
    async def test_write_frequency_default(self):
        cfg = HonchoClientConfig()
        assert cfg.write_frequency == "async"


class TestPrefetchCacheAccessors:
    async def test_set_and_pop_context_result(self, make_manager):
        mgr = make_manager(write_frequency="turn")
        payload = {"representation": "Known user", "card": "prefers concise replies"}

        await mgr.set_context_result("cli:test", payload)

        assert await mgr.pop_context_result("cli:test") == payload
        assert await mgr.pop_context_result("cli:test") == {}
