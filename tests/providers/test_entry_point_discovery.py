...
async def _restore_real_discovery():
    ...
    await providers._discover_providers()
...
async def test_entry_point_callable_and_module_targets(monkeypatch):
    ...
    _clear_provider_caches()
    try:
        assert await providers.get_provider_profile("ep-callable") is not None
        assert await providers.get_provider_profile("epc") is not None  # alias
        assert await providers.get_provider_profile("ep-module") is not None
    finally:
        _clear_provider_caches()


async def test_entry_point_not_enabled_is_skipped(monkeypatch):
    ...
    _clear_provider_caches()
    try:
        assert await providers.get_provider_profile("ep-callable") is None
    finally:
        _clear_provider_caches()


async def test_entry_point_disabled_wins_over_enabled(monkeypatch):
...
async def test_general_plugin_register_ctx_not_invoked(monkeypatch):
...
async def test_entry_point_failure_is_isolated(monkeypatch):
    def _boom():
...
async def test_filesystem_plugins_win_over_entry_points(monkeypatch):
    ...
    _clear_provider_caches()
    try:
        p = await providers.get_provider_profile("openrouter")
