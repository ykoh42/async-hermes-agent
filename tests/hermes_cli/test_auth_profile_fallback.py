...
async def test_missing_global_auth_file_is_safe(profile_env):
...
async def test_write_credential_pool_targets_profile_not_global(profile_env):
    ...
    assert [e["id"] for e in await read_credential_pool("openrouter")] == ["prof-new"]
...
async def test_write_pool_never_merges_cooldown_onto_reauthed_entry(classic_env):
    ...
    await write_credential_pool("openrouter", [_pool_entry(access_token="sk-new")])
