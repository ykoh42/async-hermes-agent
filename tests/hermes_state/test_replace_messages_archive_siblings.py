...
async def _seed_compacted_session(db, session_id: str) -> None:
    """Create a session with archived (active=0/compacted=1) + live rows."""
    await db.create_session(session_id, "test")
    msgs = [
    ...
    await db.append_messages_batch(session_id, msgs)
...
async def _archived_count(db, session_id: str) -> int:
    return sum(
        1
        for m in await db.get_messages(session_id, include_inactive=True)
        if not m["active"]
    )
...
class TestAcpPersistPreservesArchives:
    @pytest.mark.asyncio
    async def test_persist_nonowned_branch_keeps_archived_rows(self, state_db):
        """The ACP _persist fallback replace must not delete archived rows."""
        sid = "acp-compacted"
        await _seed_compacted_session(state_db, sid)
        assert await _archived_count(state_db, sid) == 4
        ...
        await state_db.replace_messages(sid, new_history, active_only=True)

        assert await _archived_count(state_db, sid) == 4
        live = [
            m for m in await state_db.get_messages_as_conversation(sid)
    ...
    def test_persist_source_has_no_failopen_probe(self):
        ...
        """
        import inspect
        acp_session = pytest.importorskip("acp_adapter.session")
    ...
    @pytest.mark.asyncio
    async def test_fresh_session_active_only_equals_full_replace(self, state_db):
        ...
        switch rests on)."""
        sid = "acp-fresh"
        await state_db.create_session(sid, "test")
        await state_db.append_messages_batch(
            sid,
            [
                {"role": "user", "content": "q"},
        ...
        await state_db.replace_messages(
        ...
        assert await _archived_count(state_db, sid) == 0
...
class TestTuiPromptTruncationPreservesArchives:
    def test_truncation_source_uses_active_only(self):
        ...
        import inspect
        mp = pytest.importorskip("tui_gateway.methods_prompt")
    ...
    @pytest.mark.asyncio
    async def test_truncation_write_keeps_archived_rows(self, state_db):
        ...
        await state_db.replace_messages(sid, truncated, active_only=True)
...
class TestArchiveDroppedIsRecoverable:
    ...
    @pytest.mark.asyncio
    async def test_dropped_turns_survive_as_inactive_rows(self, state_db):
        sid = "archive-dropped"
        await state_db.create_session(sid, "test")
        await state_db.append_messages_batch(
            sid,
        ...
        recovered = [
            m["content"]
            for m in await state_db.get_messages(sid, include_inactive=True)
            if not m["active"]
        ]
    ...
    @pytest.mark.asyncio
    async def test_archived_rows_use_rewind_marking_not_compaction(self, state_db):
        ...
        archived = [
            m for m in await state_db.get_messages(sid, include_inactive=True)
            if not m["active"]
        ]
    ...
    @pytest.mark.asyncio
    async def test_default_stays_destructive(self, state_db):
        ...
        await state_db.replace_messages(sid, [{"role": "user", "content": "fresh"}])
