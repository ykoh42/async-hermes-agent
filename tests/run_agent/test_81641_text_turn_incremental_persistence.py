...
async def _run_text_turn(agent, answer: str, *, flush_side_effect=None):
    ...
    *at call time*, so the assertion cannot be satisfied by a later mutation
    ...
    async def _record_flush(messages, conversation_history=None):
        events.append((
            "flush",
            [
    ...
    async def _record_persist(messages, conversation_history=None):
        events.append(("persist_session", None))

    with (
        patch.object(
            agent, "_flush_messages_to_session_db", new=AsyncMock(side_effect=_record_flush)
        ),
        patch.object(agent, "_persist_session", new=AsyncMock(side_effect=_record_persist)),
        patch.object(agent, "_save_trajectory", new=AsyncMock()),
        patch.object(agent, "_cleanup_task_resources", new=AsyncMock()),
    ):
        result = await agent.run_conversation("what is 2 + 2?")
...
class TestCompletedTextTurnIncrementalPersistence:
    @pytest.mark.asyncio
    async def test_completed_text_turn_is_flushed_before_finalization(self, loop_agent):
        ...
        """
        answer = "2 + 2 is 4."
        result, events = await _run_text_turn(loop_agent, answer)
    ...
    @pytest.mark.asyncio
    async def test_flush_failure_does_not_abort_the_completed_turn(self, loop_agent):
        ...
        result, events = await _run_text_turn(
