...
def agent(tmp_path, monkeypatch):
    ...
    instance._cleanup_task_resources = AsyncMock()
    instance._save_trajectory = AsyncMock()
    return instance
...
@pytest.mark.asyncio
async def test_verify_on_stop_preserves_composed_report_at_budget_limit(agent, monkeypatch):
    async def model_call(_api_kwargs, **_kwargs):
        agent._turn_file_mutation_paths = {"changed.py"}
        return _response()

    agent._execute_model_request = model_call
    agent._handle_max_iterations = AsyncMock(return_value="replacement summary")
    ...
    with (
        ...
        result = await agent.run_conversation("edit changed.py")
...
@pytest.mark.asyncio
async def test_pre_verify_preserves_composed_report_at_budget_limit(agent, monkeypatch):
    async def model_call(_api_kwargs, **_kwargs):
        agent._turn_file_mutation_paths = {"changed.py"}
        return _response()
...
@pytest.mark.asyncio
async def test_intermediate_ack_uses_summary_instead_of_premature_text(agent, monkeypatch):
    ...
    agent._execute_model_request = AsyncMock(
        return_value=_response("I'll inspect the files now")
    )
    agent._handle_max_iterations = AsyncMock(return_value="verified summary.")
    monkeypatch.setenv("HERMES_VERIFY_ON_STOP", "0")
    ...
    with (
        ...
        result = await agent.run_conversation("inspect /tmp/project")
...
@pytest.mark.asyncio
async def test_later_verified_response_supersedes_pending_report(agent, monkeypatch):
    agent.max_iterations = 2
    agent.iteration_budget.max_total = 2
    answers = iter([_response("premature report"), _response("verified final report")])
    agent._execute_model_request = AsyncMock(side_effect=lambda *_a, **_kw: next(answers))
    agent._handle_max_iterations = AsyncMock(return_value="replacement summary")
    monkeypatch.setenv("HERMES_VERIFY_ON_STOP", "1")
...
@pytest.mark.asyncio
async def test_multiple_verification_retries_publish_each_candidate_once(agent, monkeypatch):
...
@pytest.mark.asyncio
async def test_verify_on_stop_emits_interim_response_to_ui(agent, monkeypatch):
    ...
    """
    agent._execute_model_request = AsyncMock(return_value=_response("composed report"))
    agent._handle_max_iterations = AsyncMock(return_value="replacement summary")
    monkeypatch.setenv("HERMES_VERIFY_ON_STOP", "1")
...
@pytest.mark.asyncio
async def test_streamed_interim_then_different_summary_not_marked_previewed(agent, monkeypatch):
    ...
    agent._handle_max_iterations = AsyncMock(
