...
def _set_responses(agent, responses):
    agent._execute_model_request = AsyncMock(side_effect=list(responses))


async def _run(agent, message, history=None):
    with (
        ...
        return await agent.run_conversation(message, conversation_history=history)


class TestContinuationCeilingWedge:
    async def _exhaust_ceiling(self, agent):
        _set_responses(agent, [
            _stub("part one "), _stub("part two "),
            _stub("part three "), _stub("part four."),
        ])
        return await _run(agent, "write me a long report")

    @pytest.mark.asyncio
    async def test_partial_text_surfaced_at_ceiling(self, loop_agent):
        result = await self._exhaust_ceiling(loop_agent)
    ...
    @pytest.mark.asyncio
    async def test_new_user_message_issues_fresh_request(self, loop_agent):
        """Core regression: after the ceiling, a new user turn must reach
        the provider instead of replaying wedge state."""
        from tests.run_agent.test_run_agent import _mock_response

        result1 = await self._exhaust_ceiling(loop_agent)
        assert "truncated after 4 continuation attempts" in (result1.get("error") or "")
        calls_after_turn1 = loop_agent._execute_model_request.await_count
        assert calls_after_turn1 == 4

        loop_agent._execute_model_request.side_effect = [
            _mock_response(content="Hello! How can I help?", finish_reason="stop"),
        ]
        result2 = await _run(loop_agent, "hi", history=result1["messages"])

        assert loop_agent._execute_model_request.await_count == calls_after_turn1 + 1, (
    ...
    @pytest.mark.asyncio
    async def test_ceiling_replaces_scaffolding_with_settled_turn(self, loop_agent):
    ...
    @pytest.mark.asyncio
    async def test_ceiling_not_labeled_network_error(self, loop_agent):
        ...
        with patch.object(loop_agent, "_vprint", side_effect=_capture):
            await self._exhaust_ceiling(loop_agent)
    ...
    @pytest.mark.asyncio
    async def test_continuation_requests_carry_no_marks(self, loop_agent):
    ...
    @pytest.mark.asyncio
    async def test_prior_turn_marked_message_survives_later_ceiling(self, loop_agent):
        ...
        result = await _run(loop_agent, "another long report", history=reloaded_history)
    ...
    @pytest.mark.asyncio
    async def test_new_turn_does_not_inherit_continuation_counter(self, loop_agent):
        ...
        result2 = await _run(loop_agent, "try again", history=result1["messages"])
