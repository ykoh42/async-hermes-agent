...
def env(monkeypatch):
    ...
    values = {}
    async def _get_env(key):
        return values.get(key, "")

    monkeypatch.setattr(
        "hermes_cli.config.get_env_value_prefer_dotenv", _get_env
    )
    return values


class TestAdoptsEnvEdits:
    async def test_boot_default_adopts_override_on_first_look(self, env):
        ...
        assert await agent._try_refresh_env_client_credentials() is True
    ...
    async def test_edit_between_turns_is_adopted(self, env):
        """No-op first turn, then the user saves an override → next turn
        rebuilds the client onto the new endpoint."""
        agent = _make_agent()
        env["OPENAI_API_KEY"] = "sk-old"

        assert await agent._try_refresh_env_client_credentials() is False

        env["OPENAI_BASE_URL"] = LOCAL_BASE
        assert await agent._try_refresh_env_client_credentials() is True
        assert agent.base_url == LOCAL_BASE

    async def test_key_rotation_in_env_is_adopted(self, env):
        agent = _make_agent()
        env["OPENAI_API_KEY"] = "sk-old"
...
class TestLeavesNonEnvStateAlone:
    async def test_unchanged_env_is_a_noop(self, env):
        agent = _make_agent()
        env["OPENAI_API_KEY"] = "sk-old"
    ...
    async def test_pool_rotation_is_not_stomped(self, env):
    ...
    async def test_first_look_does_not_stomp_rotated_pool_key(self, env):
    ...
    async def test_adopted_key_rebinds_pool_entry_id(self, env, monkeypatch):
    ...
    async def test_custom_endpoint_wins_over_env_edit(self, env):
    ...
    async def test_skipped_while_failed_over(self, env):
        agent = _make_agent()
    ...
    async def test_skipped_for_non_api_key_provider(self, env):
        agent = _make_agent(provider="openai-codex")
        assert await agent._try_refresh_env_client_credentials() is False

    async def test_skipped_for_non_chat_completions_api_mode(self, env):
        agent = _make_agent()
        agent.api_mode = "anthropic_messages"
        assert await agent._try_refresh_env_client_credentials() is False

    async def test_skipped_when_no_key_resolves(self, env):
        agent = _make_agent()
        env["OPENAI_BASE_URL"] = LOCAL_BASE
...
class TestFailedRebuildRetries:
    async def test_failed_rebuild_rolls_back_and_retries_next_turn(self, env):
...
class TestRouteConfigRefresh:
    async def test_base_url_change_recomputes_route_tls_and_headers(self, env):
        ...
        assert await agent._try_refresh_env_client_credentials() is True
        agent._apply_client_headers_for_base_url.assert_called_once_with(LOCAL_BASE)

    async def test_key_only_change_keeps_route_config(self, env):
        agent = _make_agent()
        env["OPENAI_API_KEY"] = "sk-old"
        assert await agent._try_refresh_env_client_credentials() is False
        ...
        env["OPENAI_API_KEY"] = "sk-new"
        assert await agent._try_refresh_env_client_credentials() is True
        agent._apply_client_headers_for_base_url.assert_called_once_with(DEFAULT_BASE)
...
class TestNamedCustomProviders:
    ...
    async def test_key_added_mid_session_is_adopted(self, env, named_custom_provider):
    ...
    async def test_key_rotation_between_turns_is_adopted(self, env, named_custom_provider):
    ...
    async def test_unchanged_env_is_a_noop(self, env, named_custom_provider):
        agent = self._make_custom_agent(api_key="lc-old")
        env["LONGCAT_API_KEY"] = "lc-old"
    ...
    async def test_skipped_without_key_env(self, env, named_custom_provider):
    ...
    async def test_skipped_for_unknown_custom_provider(self, env, named_custom_provider):
        agent = self._make_custom_agent()
