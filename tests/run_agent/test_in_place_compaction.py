...
async def _seed(db, sid, title, n=8):
...
class TestInPlaceCompaction:
    ...
    async def test_in_place_keeps_same_session_id(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            await _seed(db, sid, "my-research")
            ...
            reloaded = await db.get_messages_as_conversation(sid)
            ...
            all_rows = await db.get_messages(sid, include_inactive=True)
...
class TestRotationFallbackWhenFlagOff:
    ...
    async def test_rotation_when_flag_off(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert (await db.get_session(sid))["end_reason"] == "compression"
            connection = await db._get_connection()
            child = await (
                await connection.execute(
            ...
            resumed = await db.get_messages_as_conversation(agent.session_id)
            assert [m.get("content") for m in resumed] == [
                "[CONTEXT COMPACTION] summary of prior turns",
                "recent reply",
            ]
