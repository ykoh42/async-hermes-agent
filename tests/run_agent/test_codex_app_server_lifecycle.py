...
@pytest.mark.asyncio
async def test_agent_close_releases_codex_app_server_session(monkeypatch):
    agent = _bare_agent("test-codex-lifecycle")
    codex_session = _FakeCodexSession()
    agent._codex_session = codex_session

    monkeypatch.setattr("run_agent.cleanup_vm", _async_noop)
...
@pytest.mark.asyncio
async def test_close_clears_reference_even_when_session_close_raises(monkeypatch):
...
@pytest.mark.asyncio
async def test_close_without_codex_session_is_a_noop(monkeypatch):
...
async def _async_noop(_task_id):
    return None
