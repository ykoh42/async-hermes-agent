...
def _make_agent(monkeypatch, api_mode, provider, response_fn):
    ...
    class _A(run_agent.AIAgent):
        def __init__(self, *a, **kw):
            ...
            super().__init__(*a, **kw)
            self._cleanup_task_resources = self._persist_session = AsyncMock()
            self._save_trajectory = AsyncMock()

        async def run_conversation(self, msg, conversation_history=None, task_id=None):
            self._execute_model_request = AsyncMock(return_value=response_fn())
...
@pytest.mark.asyncio
async def test_anthropic_cache_read_and_creation_added(monkeypatch):
...
@pytest.mark.asyncio
async def test_anthropic_no_cache_fields(monkeypatch):
    agent = _make_agent(monkeypatch, "anthropic_messages", "anthropic",
...
@pytest.mark.asyncio
async def test_anthropic_cache_read_only(monkeypatch):
    agent = _make_agent(monkeypatch, "anthropic_messages", "anthropic",
...
@pytest.mark.asyncio
async def test_openai_prompt_tokens_unchanged(monkeypatch):
...
@pytest.mark.asyncio
async def test_codex_no_cache_fields(monkeypatch):
    resp = lambda: SimpleNamespace(
