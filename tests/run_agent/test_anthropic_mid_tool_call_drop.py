...
class TestAnthropicMidToolCallStreamDrop:
    @pytest.mark.asyncio
    async def test_tool_use_without_stop_reason_raises_empty_stream(self, monkeypatch):
        ...
        agent._execute_model_request = AsyncMock(return_value=dropped)

        with pytest.raises(EmptyStreamError, match="tool_use"):
            await _call(agent)

    @pytest.mark.asyncio
    async def test_completed_tool_use_with_stop_reason_passes(self, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_STREAM_RETRIES", "0")
        agent = _make_anthropic_agent()
        agent._execute_model_request = AsyncMock(return_value=done)

        response = await _call(agent)
        assert response is done

    @pytest.mark.asyncio
    async def test_text_only_message_without_stop_reason_passes(self, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_STREAM_RETRIES", "0")
        agent = _make_anthropic_agent()
        agent._execute_model_request = AsyncMock(return_value=text_only)
