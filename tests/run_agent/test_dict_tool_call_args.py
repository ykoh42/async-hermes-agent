...
@pytest.mark.asyncio
async def test_tool_call_validation_accepts_dict_arguments(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=json.dumps({"ok": True, "args": {"path": "README.md"}})),
    )
    ...
    result = await agent.run_conversation("read the file")
