...
def _install_responses(agent, responses):
    sent = []

    async def _execute(api_kwargs, **_kwargs):
        sent.append(api_kwargs.get("messages") or [])
        return responses.pop(0)

    agent._execute_model_request = AsyncMock(side_effect=_execute)
    return sent
...
class TestThinkingPrefillTrailingTurn:
    ...
    @pytest.mark.asyncio
    async def test_request_after_prefills_does_not_end_on_assistant(self, loop_agent):
        ...
        sent = _install_responses(loop_agent, [
        ...
        with (
            patch.object(loop_agent, "_persist_session", new=AsyncMock()),
            patch.object(loop_agent, "_save_trajectory", new=AsyncMock()),
            patch.object(loop_agent, "_cleanup_task_resources", new=AsyncMock()),
        ):
            await loop_agent.run_conversation("do the thing")

        assert len(sent) >= 3, (
            "Two thinking-only responses should each trigger a prefill retry."
        )

        final_request = sent[2]
    ...
    @pytest.mark.asyncio
    async def test_prefill_stubs_are_absent_from_the_wire_payload(self, loop_agent):
    ...
    @pytest.mark.asyncio
    async def test_internal_marker_never_reaches_the_wire(self, loop_agent):
