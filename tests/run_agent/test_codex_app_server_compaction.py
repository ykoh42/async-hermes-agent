...
class SlowCodexSession(FakeCodexSession):
    ...
    async def compact_thread(self):
        ...
        await _wait_for_touch(
            self.touch_calls, "context compression in progress"
        )
        return self.result


async def _wait_for_touch(touch_calls, desc, timeout=1.0):
...
@pytest.mark.asyncio
async def test_codex_app_server_native_auto_mode_leaves_thread_compaction_to_codex():
    agent = DummyAgent(
        TurnResult(thread_id="thread-1", turn_id="compact-turn-1")
    )
    messages = [{"role": "user", "content": "hi"}]

    returned, prompt = await compress_context(
        agent,
        messages,
        "system",
...
@pytest.mark.asyncio
async def test_codex_app_server_compaction_heartbeat_refreshes_activity_while_waiting():
    agent = DummyAgent(
        TurnResult(thread_id="thread-1", turn_id="compact-turn-1")
    )
