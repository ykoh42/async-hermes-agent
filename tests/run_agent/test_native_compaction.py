...
def test_retry_state_has_one_shot_native_rejection_guard():
    state = TurnRetryState()
    assert state.native_compaction_reject_retry_attempted is False
    state.native_compaction_reject_retry_attempted = True
    assert dict(state)["native_compaction_reject_retry_attempted"] is True
...
class TestAgentInitConfig:
    ...
    @pytest.mark.asyncio
    async def test_kwargs_have_no_context_management_by_default(self):
        from run_agent import AIAgent
    ...
    @pytest.mark.asyncio
    async def test_kwargs_include_field_when_enabled_on_eligible_route(self):
        from run_agent import AIAgent
    ...
    @pytest.mark.asyncio
    async def test_kwargs_omit_field_for_ineligible_model_even_when_enabled(self):
        from run_agent import AIAgent
