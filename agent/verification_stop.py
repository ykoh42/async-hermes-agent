...
async def _candidate_cwds(paths: Iterable[str]) -> list[Path]:
    ...
    for raw in paths:
        ...
        try:
            ...
            candidate = path if await aiofiles.os.path.isdir(path) else path.parent
            realpath = aiofiles.os.wrap(os.path.realpath)
            resolved = str(await realpath(candidate))
        except Exception:
            continue
...
async def _verification_snapshot(
    *,
    ...
    for cwd in await _candidate_cwds(changed_paths):
        facts = await project_facts_for(cwd)
        if not facts:
            continue
        status = await verification_status(session_id=session_id, cwd=cwd)
...
async def build_verify_on_stop_nudge(
    *,
    session_id: str | None,
    ...
    snapshot = await _verification_snapshot(
        session_id=session_id, changed_paths=paths
    )
    if snapshot is None:
        return None
    ...
    try:
        ...
        guidance = await coding_verify_guidance()
    except Exception:
        guidance = None
