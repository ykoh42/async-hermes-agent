...
The :class:`agent.lsp.manager.LSPService` and file-operations layer await
it directly on that loop.
...
class LSPClient:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            self._state = "running"
        except BaseException as exc:  # noqa: ASYNC103 - cleanup precedes re-raise below
            failure = exc
        if failure is not None:
            self._state = "error"
            cleanup = asyncio.create_task(self._cleanup_process())
    ...
    async def shutdown(self) -> None:
        ...
        await _await_owned_task(self._shutdown_task)
    ...
    async def _cleanup_process(self) -> None:
        owned_tasks = [
            task
            for task in [self._reader_task, self._stderr_task, *self._request_tasks]
