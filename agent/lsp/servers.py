...
async def _spawn_pyright(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        ...
        bin_path = await try_install("pyright", ctx.install_strategy)
        if bin_path is None:
            return None
...
async def _spawn_typescript(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from agent.lsp.install import try_install
        bin_path = await try_install("typescript-language-server", ctx.install_strategy)
        if bin_path is None:
            return None
    return SpawnSpec(
        command=[bin_path, "--stdio"],
...
async def _spawn_gopls(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from agent.lsp.install import try_install
        bin_path = await try_install("gopls", ctx.install_strategy)
        if bin_path is None:
            return None
    return SpawnSpec(
        command=[bin_path],
...
async def _spawn_rust_analyzer(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from agent.lsp.install import try_install
        bin_path = await try_install("rust-analyzer", ctx.install_strategy)
        if bin_path is None:
            return None
    return SpawnSpec(
        command=[bin_path],
...
async def _spawn_clangd(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from agent.lsp.install import try_install
        bin_path = await try_install("clangd", ctx.install_strategy)
        if bin_path is None:
            return None
    return SpawnSpec(
...
async def _spawn_bash_ls(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from agent.lsp.install import try_install
        bin_path = await try_install("bash-language-server", ctx.install_strategy)
        if bin_path is None:
            return None
    ...
    if not _BASH_SHELLCHECK_WARNED and await _which("shellcheck") is None:
...
async def _spawn_yaml_ls(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from agent.lsp.install import try_install
        bin_path = await try_install("yaml-language-server", ctx.install_strategy)
        if bin_path is None:
            return None
    return SpawnSpec(
        command=[bin_path, "--stdio"],
...
async def _spawn_lua_ls(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from agent.lsp.install import try_install
        bin_path = await try_install("lua-language-server", ctx.install_strategy)
        if bin_path is None:
            return None
    return SpawnSpec(
        command=[bin_path],
...
async def _spawn_intelephense(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from agent.lsp.install import try_install
        bin_path = await try_install("intelephense", ctx.install_strategy)
        if bin_path is None:
            return None
...
async def _spawn_dockerfile_ls(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from agent.lsp.install import try_install
        bin_path = await try_install("dockerfile-language-server-nodejs", ctx.install_strategy)
        if bin_path is None:
            return None
    return SpawnSpec(
        command=[bin_path, "--stdio"],
...
async def _spawn_jdtls(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    # produces.
    bin_path = await _resolve_override(ctx, "jdtls") or await _which("jdtls")
    if bin_path is None:
        return None
    return SpawnSpec(
        command=[bin_path],
...
async def _spawn_vue(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from agent.lsp.install import try_install
        bin_path = await try_install("@vue/language-server", ctx.install_strategy)
        if bin_path is None:
            return None
    return SpawnSpec(
        command=[bin_path, "--stdio"],
...
async def _spawn_svelte(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from agent.lsp.install import try_install
        bin_path = await try_install("svelte-language-server", ctx.install_strategy)
        if bin_path is None:
            return None
    return SpawnSpec(
        command=[bin_path, "--stdio"],
...
async def _spawn_astro(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bin_path is None:
        from agent.lsp.install import try_install
        bin_path = await try_install("@astrojs/language-server", ctx.install_strategy)
        if bin_path is None:
            return None
    return SpawnSpec(
        command=[bin_path, "--stdio"],
...
async def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    for cand in candidates:
        ...
        if await aiofiles.os.path.isfile(start_script):
            return cand
        inner = os.path.join(cand, "Start-EditorServices.ps1")
        if await aiofiles.os.path.isfile(inner):
...
async def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    """
    pwsh = await _which("pwsh", "powershell")
    if pwsh is None:
        return None
    bundle = await _find_pses_bundle(ctx)
    if bundle is None:
    ...
    session_path = os.path.join(
        await hermes_lsp_session_dir(), f"pses-session-{os.getpid()}.json"
    )
    log_path = os.path.join(await hermes_lsp_session_dir(), "pses.log")
    inner = (
        f"& '{start_script}' "
...
async def hermes_lsp_session_dir() -> str:
...
async def _root_bash(file_path: str, workspace: str) -> str:
    return workspace


async def _root_yaml(file_path: str, workspace: str) -> str:
    return workspace
...
async def _root_docker(file_path: str, workspace: str) -> str:
    return workspace
...
async def _root_nix(file_path: str, workspace: str) -> str:
    found = await nearest_root(file_path, ["flake.nix"])
    return found or workspace
