...
async def _run_helper(
    ...
    try:
        proc = await asyncio.create_subprocess_exec(
            ...
            stderr=asyncio.subprocess.PIPE,  # captured and discarded
    ...
    if returncode != 0:
        ...
        if returncode < 0:
            ...
            code, signame = str(returncode), "none"
        print(
...
async def get_command_secret(
    *,
    ...
    if not command:
        return None
    stdout = await _run_helper(command, key, timeout_seconds, max_output_bytes)
    if stdout is None:
        return None
    return parse_secret_output(stdout, key)


async def list_command_secrets(
    *,
    command: str,
    timeout_seconds: float = _COMMAND_TIMEOUT_SECONDS,
    max_output_bytes: int = _MAX_OUTPUT_BYTES,
    ...
    if not command:
        return {}
    stdout = await _run_helper(command, "", timeout_seconds, max_output_bytes)
    if stdout is None:
        return {}
...
async def apply_command_secrets(
    *,
    command: str,
    ...
    The startup path normally goes through :class:`CommandSource` and the
    registry orchestrator, which owns precedence and environment writes.
    """
    result = FetchResult()
...
class CommandSource(SecretSource):
    ...
    async def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        stdout = await _run_helper(command, "", timeout, _MAX_OUTPUT_BYTES)
        if stdout is None:
            # _run_helper already logged structured fields to stderr.
            result.error = (
