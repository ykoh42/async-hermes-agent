...
async def find_op(binary_path: str = "") -> Optional[Path]:
    ...
    if binary_path:
        ...
        if await aiofiles.os.path.isfile(pinned) and await aiofiles.os.access(
            pinned, os.X_OK
        ):
            return pinned
        return None
    ...
    if os.name == "nt":
        names = [
            f"op{suffix}" for suffix in os.getenv("PATHEXT", ".EXE").split(os.pathsep)
        ]
...
async def _run_op_read(
    ...
    *,
    ...
    value = stdout_bytes.decode("utf-8", errors="replace").rstrip("\r\n")
    if not value.strip():
...
async def fetch_onepassword_secrets(
    *,
    ...
    op = binary or await find_op(binary_path)
    if op is None:
        raise RuntimeError(
    ...
    for name in sorted(valid):
        try:
            secrets[name] = await _run_op_read(
...
async def apply_onepassword_secrets(
    ...
    *before* fetching, so ``op`` is never invoked for a value that would be
    ...
    binary = await find_op(binary_path)
    ...
    try:
        secrets, fetch_warnings = await fetch_onepassword_secrets(
