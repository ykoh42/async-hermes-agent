...
async def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    if await aiofiles.os.path.isfile(managed) and await aiofiles.os.access(
        managed, os.X_OK
    ):
        return managed
    ...
    if os.name == "nt":
        names = [
            f"bws{suffix}" for suffix in os.getenv("PATHEXT", ".EXE").split(os.pathsep)
        ]
...
async def _platform_asset_name() -> str:
...
async def install_bws(*, force: bool = False) -> Path:
    ...
    await aiofiles.os.makedirs(bin_dir, exist_ok=True)
    target = bin_dir / _platform_binary_name()

    if await aiofiles.os.path.exists(target) and not force:
        return target

    asset_name = await _platform_asset_name()
    asset_url = f"{_BWS_RELEASE_BASE}/{asset_name}"
    checksum_url = f"{_BWS_RELEASE_BASE}/{_BWS_CHECKSUM_NAME}"

    async with aiofiles.tempfile.TemporaryDirectory(prefix="hermes-bws-") as tmpdir:
        tmp = Path(tmpdir)
        zip_path = tmp / asset_name
        checksum_path = tmp / _BWS_CHECKSUM_NAME

        logger.info("Downloading %s", asset_url)
        await _http_download(asset_url, zip_path)
        await _http_download(checksum_url, checksum_path)

        expected = await _expected_sha256(checksum_path, asset_name)
        actual = await _sha256_file(zip_path)
        ...
        with zipfile.ZipFile(io.BytesIO(archive)) as zf:
            ...
            extracted = await _safe_extract_member(zf, member, tmp)

        # Move into place atomically.  We write to a sibling tempfile in
        # the final directory so the rename can't cross filesystems.
        staged = bin_dir / f".bws_{uuid.uuid4().hex}.tmp"
        async with aiofiles.open(extracted, "rb") as source:
            async with aiofiles.open(staged, "xb") as destination:
        ...
        await aiofiles.os.replace(staged, target)
...
async def _expected_sha256(checksum_file: Path, asset_name: str) -> str:
    """Parse the upstream ``bws-sha256-checksums-X.Y.Z.txt`` file.

    Format is the standard ``sha256sum`` output: ``<hex>  <filename>``,
    one per line.
    """
    async with aiofiles.open(checksum_file, encoding="utf-8", errors="replace") as file:
...
async def _safe_extract_member(
    ...
    await aiofiles.os.makedirs(Path(target).parent, exist_ok=True)
    async with aiofiles.open(target, "wb") as file:
        await file.write(zf.read(member))
    return Path(target)
...
async def _write_encrypted_disk_cache(
    *,
    cache_key: _CacheKey,
    access_token: str,
    entry: _CachedFetch,
    ...
    try:
        ...
        tmp = cache_dir / f".bws_cache_enc_{uuid.uuid4().hex}.tmp"
        try:
            ...
            try:
                await aiofiles.os.remove(_disk_cache_path(home_path))
...
async def _read_encrypted_disk_cache(
    *,
    cache_key: _CacheKey,
    access_token: str,
...
async def fetch_bitwarden_secrets(
    *,
    access_token: str,
    project_id: str,
    ...
    if use_cache and cache_ttl_seconds > 0:
        ...
        if encrypted_cache_enabled:
            disk_cached = await _read_encrypted_disk_cache(
                cache_key=cache_key,
    ...
    bws = binary or await find_bws(install_if_missing=True)
    if bws is None:
        raise RuntimeError(
...
async def _run_bws_list(
    ...
    if proc.returncode != 0:
        ...
        err = _summarize_bws_stderr(stderr or stdout)
...
async def apply_bitwarden_secrets(
    *,
    enabled: bool,
    ...
    binary = await find_bws(install_if_missing=auto_install)
...
class BitwardenSource(SecretSource):
    ...
    *fetches* — precedence, override semantics, conflict warnings, and
    ...
    async def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
...
async def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    try:
        await aiofiles.os.remove(_encrypted_disk_cache_path(home_path))
