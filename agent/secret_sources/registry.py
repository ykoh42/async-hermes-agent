...
def register_source(source: SecretSource, *, replace: bool = False) -> bool:
    ...
    if getattr(source, "api_version", None) != SECRET_SOURCE_API_VERSION:
        ...
        return False
    if not inspect.iscoroutinefunction(inspect.unwrap(source.fetch)):
        logger.warning(
            "Ignoring secret source '%s': fetch() must be async",
            name,
        )
        return False
...
async def _fetch_with_timeout(
    ...
    try:
        async with asyncio.timeout(timeout):
            result = await source.fetch(cfg, home_path)
    except TimeoutError:
        res = FetchResult()
        res.error = (
...
async def apply_all(
    ...
    for source in ordered:
        ...
        cfg = cfg if isinstance(cfg, dict) else {}
        result = await _fetch_with_timeout(source, cfg, home_path, env)
