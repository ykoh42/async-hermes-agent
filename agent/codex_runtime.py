...
async def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    ...
    if not isinstance(usage, dict) or not usage:
        ...
        if agent._session_db and agent.session_id:
            try:
                ...
                await agent._session_db.queue_token_counts(
                    agent.session_id,
                    model=agent.model,
...
async def run_codex_app_server_turn(
    ...
    *,
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        cwd = getattr(agent, "session_cwd", None) or str(
    ...
    try:
        turn = await agent._codex_session.run_turn(user_input=user_message)
    except asyncio.CancelledError:
        try:
            agent._codex_session.request_interrupt()
            await agent._codex_session.close()
        finally:
    ...
    if turn.projected_messages:
        ...
        if getattr(agent, "_session_db", None) is not None:
            try:
                _codex_flush_ok = await agent._flush_messages_to_session_db(messages)
            except Exception:
...
async def run_codex_stream(
    ...
    try:
        ...
        close = async_close or getattr(stream, "close", None)
        if callable(close) and (
            ...
            try:
                ...
                if hasattr(close_task, "set_name"):
                    close_task.set_name("codex-response-stream-close")
...
async def run_codex_create_stream_fallback(
    agent,
    api_kwargs: dict,
    client: Any = None,
):
    """Backward-compatible name for the unified event-driven stream path."""
    return await run_codex_stream(agent, api_kwargs, client=client)
