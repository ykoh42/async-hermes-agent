...
Lifecycle:
    ...
    await session.ensure_started()                        # spawns + handshake + thread/start
    result = await session.run_turn(user_input="hello")   # waits until turn/completed
    ...
    await session.close()                                 # tears down subprocess

The adapter and wire client run entirely on the caller's event loop. A turn is
single-owner, while notifications, approvals, steering, and interruption are
coordinated with asyncio primitives.
"""
...
class CodexAppServerSession:
    ...
    One coroutine drives a turn at a time, matching AIAgent's serialized
    run_conversation() contract. The client can interleave reads and writes on
    the event loop, while the adapter owns projector and turn state.
    """
    ...
    async def ensure_started(self) -> str:
        ...
        result = await self._client.request("thread/start", params, timeout=15)
    ...
    async def request_steer(self, text: str) -> bool:
    ...
    async def run_turn(
        ...
        *,
        ...
        """Send a user message and wait until turn/completed, while
        ...
        while time.monotonic() < deadline and not turn_complete:
            ...
            sreq = await self._client.take_server_request(timeout=0)
            if sreq is not None:
                ...
                for _ in range(8):
                    pending = await self._client.take_notification(timeout=0)
                    if pending is None:
                        break
    ...
    async def _handle_server_request(self, req: dict) -> None:
        ...
        if method == "item/commandExecution/requestApproval":
            decision = await self._decide_exec_approval(params)
            await self._client.respond(rid, {"decision": decision})
        elif method == "item/fileChange/requestApproval":
            decision = await self._decide_apply_patch_approval(params)
            await self._client.respond(rid, {"decision": decision})
            ...
            await self._client.respond(rid, {"decision": "decline"})
    ...
    async def _decide_exec_approval(self, params: dict) -> str:
        ...
        if self._approval_callback is not None:
            try:
                choice = await self._approval_callback(
    ...
    async def _decide_apply_patch_approval(self, params: dict) -> str:
