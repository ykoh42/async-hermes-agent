...
def _usage_get(obj: Any, name: str, default: Any = 0) -> Any:
    """Read typed SDK usage objects and JSON-deserialized dicts alike."""
...
def _usage_count(value: Any) -> int:
    """Coerce malformed provider counters to a non-negative integer."""
...
def normalize_usage(
    ...
    *,
    ...
    if mode == "anthropic_messages" or provider_name == "anthropic":
        ...
        prompt_total = _usage_count(_usage_get(response_usage, "prompt_tokens", 0))
        if not prompt_total:
            prompt_total = _usage_count(_usage_get(response_usage, "input_tokens", 0))
