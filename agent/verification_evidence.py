...
_DB_LOCKS: WeakKeyDictionary[Any, asyncio.Lock] = WeakKeyDictionary()
...
async def _transaction() -> AsyncIterator[aiosqlite.Connection]:
    """Serialize a transaction per loop and always close its connection."""
    async with _db_lock():
        conn = await _connect()
        try:
...
def _split_shell_segments(
    ...
    """Tokenize top-level shell commands while preserving control operators."""
...
def _exit_status_is_attributable(
    ...
    """Whether the shell status proves the matched segment's own status."""
...
async def _is_under_root(token: str, root: str | Path | None) -> bool:
    if not root:
        return False
    try:
        path = await _resolved_path(token)
        root_path = await _resolved_path(root)
...
async def _is_temp_script_path(token: str, root: str | Path | None) -> bool:
    ...
    return (
        ...
        and await _is_under_temp_dir(token)
        and not await _is_under_root(token, root)
    )
...
async def _ad_hoc_script_args(
    ...
    if await _is_temp_script_path(command, root):
    ...
    if command in {"python", "python3", "node", "bash", "sh", "ruby", "perl"}:
        for idx, token in enumerate(candidate_tokens[1:], start=1):
            if token == "--":
                continue
            if await _is_temp_script_path(token, root):
...
async def _find_ad_hoc_match(
    command: str, root: str | Path | None, exit_code: int = 0
) -> list[str] | None:
    ...
    for posix in (True, False):
        ...
        for index, segment in enumerate(segments):
            trailing_args = await _ad_hoc_script_args(segment.tokens, root)
...
async def _prune_old_events(
    conn: aiosqlite.Connection, *, session_id: str, root: str
) -> None:
...
async def classify_verification_command(
    command: str,
    *,
    ...
    try:
        ...
        facts = await project_facts_for(cwd)
    except Exception:
        facts = None
    if not facts:
        return None
    ...
    if match is None and not verify_commands:
        ad_hoc_args = await _find_ad_hoc_match(
            command, facts.get("root"), int(exit_code)
        )
    ...
    return VerificationEvidence(
        ...
        cwd=str(await _resolved_path(cwd or ".")),
        root=str(facts.get("root") or await _resolved_path(cwd or ".")),
...
async def _insert_evidence(evidence: VerificationEvidence) -> dict[str, Any]:
    ...
    async with _transaction() as conn:
        ...
        await _prune_old_events(
            conn, session_id=evidence.session_id, root=evidence.root
        )

    return {"id": event_id, **evidence.__dict__, "created_at": created_at}


async def mark_workspace_edited(
    *,
    session_id: str | None,
    cwd: str | Path | None,
