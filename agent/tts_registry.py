...
def register_provider(provider: TTSProvider) -> None:
    ...
    if not inspect.iscoroutinefunction(provider.synthesize):
        raise TypeError("TTS provider .synthesize must be async")
    if not inspect.isasyncgenfunction(provider.stream):
        raise TypeError("TTS provider .stream must be an async iterator")
