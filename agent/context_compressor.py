...
def _reasoning_details_text_chars(value: Any) -> int:
    """Return the textual thinking size without provider envelope blobs."""
    if not value:
        return 0
    if isinstance(value, str):
        return len(value)
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
        ...
        # The deterministic fallback remains synchronous so existing callers
        # and tests keep their upstream helper contract. Its async digest
        # section is added only on the normal awaited summarizer path.
        summary = self._augment_summary_lean_sync(summary, turns_to_summarize)
        return summary
    ...
    async def _build_chunk_digests(self, turns: list[dict[str, Any]]) -> str:
    ...
    def _augment_summary_lean_sync(
        self, summary: str, turns_to_summarize: list[dict[str, Any]],
    ) -> str:
        """Append the deterministic, non-I/O lean sections to a summary.

        This is also used by the synchronous static fallback. The chunked
        digest is deliberately added by the awaited companion below.
        """
        if getattr(self, "tail_mode", "legacy") != "lean":
            return summary
    ...
    async def _augment_summary_lean(
        self, summary: str, turns_to_summarize: list[dict[str, Any]],
    ) -> str:
        """Append all lean sections at the native-async summary boundary."""
        summary = self._augment_summary_lean_sync(summary, turns_to_summarize)
        if (
            getattr(self, "tail_mode", "legacy") == "lean"
            and _LEAN_DIGESTS_HEADING not in summary
        ):
            summary += _redact_compaction_text(
                await self._build_chunk_digests(turns_to_summarize)
            )
        return summary
...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
        ...
        try:
            ...
            summary = await self._augment_summary_lean(summary, turns_to_summarize)
    ...
    def _find_tail_cut_by_tokens(
        ...
        return min(
            n,
            self._align_boundary_forward(messages, max(cut_idx, head_end + 1)),
        )
    ...
    async def compress(
        ...
        if tail_messages:
            first_tail_visible_idx, first_tail_role = next(
                (
                    (idx, role)
                    for idx, role in (
                        (idx, _template_visible_role(message))
                        for idx, message in enumerate(tail_messages)
                    )
                    if role is not None
                ),
                (None, None),
        ...
        if _force_user_leading and first_tail_visible_idx is not None:
            merge_target_idx = first_tail_visible_idx
        for tail_idx, msg in enumerate(tail_messages):
            if _merge_summary_into_tail and tail_idx == merge_target_idx:
