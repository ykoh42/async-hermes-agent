...
async def _videos_cache_dir() -> Path:
...
async def save_b64_video(
    ...
    *,
    ...
    cache_dir = await _videos_cache_dir()
...
class OpenAICompatibleVideoGenProvider(VideoGenProvider):
    ...
    # requests and a stuck job occupies its calling task indefinitely. We
    # hand-roll a bounded poll instead: a coarse interval plus a hard
    ...
    async def _create_and_poll(self, client: Any, call_kwargs: Dict[str, Any]) -> Any:
        ...
        video = await client.videos.create(**call_kwargs)
        ...
        try:
            ...
            try:
                if url:
                    ...
                    video_ref = str(await save_url_video(url, prefix=self.name))
                else:
                    # OpenAI/Sora style: no public URL — pull bytes via the SDK.
                    response = await client.videos.download_content(video.id)
                    raw = await response.aread()
                    video_ref = str(await save_bytes_video(raw, prefix=self.name))
