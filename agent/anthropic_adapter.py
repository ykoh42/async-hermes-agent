...
def _get_claude_code_version() -> str:
    ...
    call :func:`_ensure_claude_code_version` first; direct synchronous callers
...
async def _ensure_claude_code_version() -> str:
...
async def build_anthropic_client(
    ...
    * a ``Callable[[], Awaitable[str]]`` — an Entra ID token provider used by
    ...
    if _is_kimi_coding_endpoint(base_url):
        # Kimi's /coding endpoint requires a non-empty client identity.  Use
        # the same Hermes attribution headers as the OpenAI-wire provider
        # profile; this branch runs for the native Anthropic route.
...
async def _read_claude_code_credentials_from_keychain() -> dict[str, Any] | None:
...
async def _read_claude_code_credentials_from_file() -> dict[str, Any] | None:
...
async def read_claude_code_credentials() -> dict[str, Any] | None:
...
async def _refresh_oauth_token(creds: dict[str, Any]) -> str | None:
...
async def resolve_anthropic_token() -> str | None:
...
async def run_oauth_setup_token() -> str | None:
...
async def read_hermes_oauth_credentials() -> dict[str, Any] | None:
...
def _to_plain_data(value: Any, *, _depth: int = 0, _path: set | None = None) -> Any:
    ...
    if hasattr(value, "model_dump"):
        _path.add(obj_id)
        try:
            # Parsed streaming blocks can trigger pydantic's generic-union
            # serializer warning even though their dumped payload is valid.
            dumped = value.model_dump(warnings=False)
        except TypeError:
            # Preserve support for duck-typed model_dump() implementations.
